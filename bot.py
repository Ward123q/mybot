    ...
    async def __call__(self, handler, event: Message, data):
        if isinstance(event, Message) and event.from_user and event.chat.type in ("group","supergroup"):
            ...
            if new_level > old_level:
                ...
                # Красивое aesthetic-сообщение
                lvl_text = (
                    f"{emoji} <b>Новый уровень</b>\n"
                    f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
                    f"🌸 уровень — <b>{new_level}</b>\n"
                    f"🍃 титул — <b>{title}</b>\n"
                    f"✨ репутация — <b>+{bonus_rep}</b>\n"
                    f"☁️ чат — <i>{event.chat.title}</i>"
                )

                # 1) Сначала пробуем в ЛС
                dm_ok = False
                try:
                    await bot.send_message(uid, lvl_text, parse_mode="HTML")
                    dm_ok = True
                except: pass

                # 2) Если не получилось (не писал боту) — в чат с автоудалением через 30с
                if not dm_ok:
                    try:
                        chat_text = (
                            f"{emoji} {event.from_user.mention_html()} · новый уровень\n"
                            f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
                            f"🌸 уровень — <b>{new_level}</b>\n"
                            f"🍃 титул — <b>{title}</b>\n"
                            f"✨ репутация — <b>+{bonus_rep}</b>\n"
                            f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
                            f"<i>напиши /start в ЛС боту — буду присылать туда 🤍</i>"
                        )
                        sent = await event.answer(chat_text, parse_mode="HTML")
                        # Автоудаление через 30 сек
                        async def _auto_del():
                            try:
                                await asyncio.sleep(30)
                                await bot.delete_message(cid, sent.message_id)
                            except: pass
