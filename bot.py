...
LEVEL_TITLES = {
    # ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧
    #  15 эстетичных рангов · Aesthetic
    # ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧
    1:   ("🌱", "Росток"),
    5:   ("🍃", "Листок"),
    10:  ("🌴", "Веточка"),
...
RULES_TEXT = (
    ...
    "🔞 <b>Контент 18+</b>\n"
    "Запрещены любые материалы для взрослых — стикеры, видео, гифки, ссылки, насилие.\n\n"

    "🥴 <b>Наркотики</b>\n"
    "Запрещён любой контент, связанный с наркотическими веществами.\n\n"

    "📢 <b>Реклама</b>\n"
    "Запрещена реклама соцсетей, брендов, услуг и любых проектов.\n\n"

    "🛡 <b>Уважение к администрации</b>\n"
    "Оскорбления и грубость в адрес модераторов — варн.\n"
    "Повторное нарушение — бан.\n\n"

    "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
    "🌴 <b>Система наказаний</b>\n"
    "1️⃣ Первое нарушение → предупреждение (варн)\n"
    "2️⃣ Повторное → мут или бан\n\n"
    "<i>Соблюдай правила — будь частью комьюнити 🤝</i>\n"
)

MUTE_MESSAGES = [
    "🔇 <b>Мут выдан</b>\n<code>─</code>\n👤 {name}\n⏱ Длительность: <b>{time}</b>\n\n<i>Нарушение правил чата.</i>",
    "🔕 {name} временно отключён\n⏱ Срок: <b>{time}</b>",
    "🤫 {name} помолчит <b>{time}</b>\n<i>За нарушение правил.</i>",
]
BAN_MESSAGES = [
...
WARN_MESSAGES = [
    ...
    "⚡ {name} получил варн\n📋 <b>{reason}</b> · <b>{count}/{max}</b>",
    "🌴 Варн #{count} · {name}\n📝 <b>{reason}</b>\n<i>Осталось: {max} — {count}</i>",
]
AUTOBAN_MESSAGES = [
    "🤖 <b>Автобан</b>\n<code>─</code>\n👤 {name}\n⚡ Лимит варнов достигнут: <b>{max}/{max}</b>\n\n<i>Заблокирован автоматически.</i>",
    "🔨 {name} автоматически заблокирован\n⚡ Исчерпан лимит: <b>{max}</b> варнов",
]
RANDOM_BAN_REASONS = [
...
async def require_admin(message: Message) -> bool:
    ...
    if not await check_admin(message):
        await reply_auto_delete(message, "🍃 Эта команда — для администраторов чата", parse_mode="HTML")
        return False
    return True
    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(
                ...
                dl    = desc.lower()
                emoji = "☀️" if "ясно" in dl else ("🌞" if "дождь" in dl else ("🍉" if "снег" in dl else "⛅"))
...
def kb_user_panel(tid: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🎭  Приколы",            callback_data=f"panel:fun:{tid}"),
         InlineKeyboardButton(text="🎰  Казино",             callback_data=f"game:casino:{tid}")],
...
def kb_members(tid: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🔇  Мут 24ч реклама",    callback_data=f"members:warn24:{tid}"),
         InlineKeyboardButton(text="🌴  Варны участника",    callback_data=f"members:warninfo:{tid}")],
        [InlineKeyboardButton(text="🚫  Список банов",       callback_data=f"members:banlist:{tid}"),
...
def kb_games(tid: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="🖐 КНБ — Бумага",       callback_data=f"game:rps_b:{tid}"),
         InlineKeyboardButton(text="🌤 Погода",             callback_data=f"game:weather_Москва:{tid}")],
            ...
            try:
                if action == "mute_custom":
                    ...
                    if len(parts) >= 3:
                        ...
                        await event.reply("🌴 Формат: Вопрос|Вариант1|Вариант2")
...
class StatsMiddleware(BaseMiddleware):
    ...
    CASCADE_STEPS = [
        ...
        ("mute",  720,   "мут 12 часов",         "🍉"),   # 9-е
        ("mute",  1380,  "мут 23 часа",          "🍉"),   # 10-е
    ...
    async def _check_flood(self, uid: int, cid: int, name: str) -> bool:
        ...
        try:
            ...
            threshold = chat_cfg.get("flood_msgs", 15)
            window_sec = chat_cfg.get("flood_window", 20)
            now = _time_module.time()

            # ─── Трекер сообщений в скользящем окне ───
            ...
            self._flood_tracker[cid][uid] = [
                t for t in self._flood_tracker[cid][uid] if now - t < window_sec
            ]
            ...
            try:
                if action == "warn":
                    ...
                    add_mod_history(cid, uid, f"🍃 Предупреждение (флуд · ступень {human_step})",
                                    f"{count} сообщений за {window_sec}с", "AutoMod · Cascade")
                    ...
                    if human_step < len(self.CASCADE_STEPS):
                        ...
                        suffix_lines.append(f"🍉 это максимальная ступень")
                    ...
                    await bot.send_message(
                        ...
                        f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
                        f"🌸 нарушение №{human_step} ‧ {count} сообщ/{window_sec}с\n"
                        + "\n".join(suffix_lines),
                        parse_mode="HTML"
                    )
    ...
    async def __call__(self, handler, event: Message, data):
        if isinstance(event, Message) and event.from_user and event.chat.type in ("group","supergroup"):
            ...
            if now_ts - xp_cooldowns.get(xp_cd_key, 0) >= 60:
                ...
                # 🌊 СИСТЕМА XP/УРОВНЕЙ/РЕПЫ ВЫПИЛЕНА
                # xp_data[cid][uid] += _xp
                pass

            # ── Стрик ── (оставлен ‧ используется только в welcome-bonus)
            ...
            # ── Уровень / репа / левел-ап ── ВЫПИЛЕНО
            # (старая логика начисления уровней и репы удалена)
            if False:  # выпилено
                pass
            if event.text:
...
async def cb_captcha_skip(call: CallbackQuery):
    ...
    if not pending:
        await call.answer("🌴 Капча уже завершена.", show_alert=True)
        return
...
# ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧
#  🛡 АНТИБУЛЛИНГ ‧ SOS · анон-жалоба · щит над юзером
# ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧
...
_AGGRO_WORDS = [
    ...
    'тупенз', 'тупарь', 'отупел', 'тупоголов',
    'дебил', 'дебиль', 'дебильн', 'дебилоид',
    'идиот', 'идиотск', 'идиётск', 'идиотизм',
    'кретин', 'кретинизм', 'кретинск', 'имбецил',
    'имбицил', 'даун', 'даунизм', 'даунён',
    'даунен', 'даунч', 'олигофрен', 'олигоф',
    'дегенерат', 'дегенер', 'дебилушк', 'умственно отстал',
    'недоразвит', 'слабоумн', 'скудоумн', 'безмозгл',
    'безголов', 'безмозг', 'мозгов нет', 'без мозгов',
    'мозги где', 'мозга нет', 'тормоз', 'тормознут',
    'тормозн', 'заторможен', 'недалёк', 'недалек',
    'примитив', 'узколоб', 'недоумок', 'недоумк',
    'недоумен', 'дурак', 'дурочк', 'дурын',
    'дурак набит', 'дурбецел', 'придурок', 'придурк',
    'придурош', 'придурочн', 'балбес', 'болван',
    'болванк', 'остолоп', 'деревянн башк', 'дуб дуб',
    'дубина', 'дубьё', 'дубин', 'пенёк',
    'пенек', 'валенок', 'валенк', 'отсталый',
    'отсталая', 'даунёныш', 'еблан', 'ебланск',
    'ебланист', 'долбоёб', 'долбоеб', 'долбаёб',
    'долбаеб', 'долбоящер', 'долбак', 'долбень',
    'долбоклюй', 'выродок', 'недочеловек', 'убогий',
    'убогая', 'убог', 'убогост', 'тупое создан',
    'мозг отсутств', 'интеллект ноль', 'ноль интеллект', 'безмозглое',
    'думать не умеет', 'дебилизм', 'кретинище', 'идиотина',
    'тупица', 'пустоголов', 'дуролом', 'дурачьё',
    'дурачина', 'балда', 'оболтус', 'недоразвитость',
    'умалишён', 'умалишен', 'полудурок', 'полудур',
    'недоумочн', 'тупорогий', 'дуб дубом', 'дерево дерев',
    'клиника ходяч', 'диагноз ходяч', 'справка нужна', 'психбольн тебе',
    'на учёте', 'ничтожеств', 'ничтожн', 'ничтожк',
    'никчём', 'никчем', 'никчемн', 'пустое мест',
    'пустышк', 'пустой человек', 'ноль без палочк', 'мраз',
    'мразот', 'мразь конч', 'мразен', 'сволоч',
    'сволочь', 'подонок', 'подонк', 'подонч',
    'гнид', 'гнида', 'гнидин', 'гнусн',
    'гнусь', 'мерзав', 'мерзк', 'мерзот',
    'мерзопакост', 'урод', 'уродин', 'уродлив',
    'уродец', 'уродищ', 'уёбищ', 'уебищ',
    'уёбок', 'уебок', 'убожеств', 'убожищ',
    'чмо', 'чмошник', 'чмошн', 'чмырь',
    'чмыр', 'чмошниц', 'ублюдок', 'ублюдк',
    'ублюдочн', 'выблядок', 'выблядк', 'отброс',
    'отребье', 'отрепье', 'шваль', 'шушер',
    'шушера', 'гнилой', 'гнилая', 'гнильё',
    'гнилуш', 'падаль', 'падла', 'падлюк',
    'паскуд', 'паскудин', 'паскудник', 'тварь',
    'твар', 'тварин', 'тварюг', 'гадин',
    'гадюк', 'гадость', 'гадёныш', 'скотин',
    'скотоб', 'скотобаз', 'быдло', 'быдлан',
    'быдляк', 'быдлота', 'быдлокот', 'хам',
    'хамло', 'хамьё', 'хамоват', 'жлобьё',
    'жлобск', 'жлобин', 'деревня тупая', 'колхозник',
    'нищеброд', 'нищук', 'нищенк', 'голодранец',
    'оборванец', 'босяк', 'голытьб', 'бомж',
    'бомжар', 'бомжов', 'бомжих', 'помойн',
    'помоечн', 'помойка ходяч', 'клоун', 'клоунад',
    'клован', 'шут гороховый', 'шут', 'посмешищ',
    'паяц', 'фигляр', 'мразотн', 'выблядин',
    'гнойн', 'падальщик', 'стервятник', 'крыса позорн',
    'крысёныш', 'крысятник', 'шкет', 'шпана',
    'гопник', 'гопота', 'гопар', 'урка',
    'зэчара', 'сявка', 'шныр', 'фуфел',
    'фуфло', 'барыг', 'спекулянт', 'торгаш',
    'живоглот', 'кровопийц', 'паразит', 'трус',
    'трусиха', 'трусли', 'трусоват', 'трусишк',
    'ссыкло', 'ссыкун', 'ссыкл', 'ссыкух',
    'зассал', 'зассыш', 'зассыкл', 'очко играет',
    'очко жим', 'слабак', 'слабачок', 'слабачьё',
    'слабохарактер', 'тряпк', 'тряпич', 'тряпьё',
    'размазн', 'размазал', 'мямл', 'мямля',
    'нытик', 'нытьё', 'ноющ', 'нытьик',
    'плакса', 'плаксив', 'рёва', 'рева',
    'рёва корова', 'сопляк', 'соплив', 'сопляч',
    'малохольн', 'хлюпик', 'хлюпич', 'дрищ',
    'дрищав', 'дохляк', 'дохлятин', 'заморыш',
    'задохлик', 'задрот', 'задрач', 'задротск',
    'ботан тупой', 'ботаник зад', 'лошар', 'лошок',
    'лошпед', 'лошадь тупая', 'лохматрос', 'лоховск',
    'лошьё', 'терпил', 'терпильск', 'терпильн',
    'петух', 'петушар', 'петушин', 'петушок гей',
    'опущ', 'опущенец', 'опущенн', 'зашкварен',
    'зашквар', 'зашкварн', 'шестёрк', 'шестерк',
    'шестёрит', 'холуй', 'холуйск', 'прихвостень',
    'подхалим', 'подхалимск', 'лизоблюд', 'подлиз',
    'подлизыв', 'жополиз', 'жоп лиз', 'жополизн',
    'подстилк', 'шавк', 'шавка', 'шавкин',
    'псин', 'псина блох', 'шакал', 'шакальё',
    'шакалит', 'слизняк', 'слизняч', 'амёб',
    'амёба бесхар', 'медуз бесхреб', 'бесхребетн', 'безвольн',
    'рохля', 'размазня', 'кисель бесхар', 'овца тупая',
    'баран безвольн', 'стадо', 'тюфяк', 'недомужик',
    'не мужик', 'полумужик', 'бабёнк', 'тютя',
    'рохл', 'слюнтяй', 'слабонервн', 'истеричк',
    'психопат слаб', 'нюня', 'распустил сопл', 'разнюнил',
    'скуксил', 'сдрейфил', 'струсил', 'поджал хвост',
    'хвост поджал', 'спрятался как', 'пидор', 'пидар',
    'пидр', 'пидорас', 'пидорг', 'пидрила',
    'педик', 'педрил', 'педер', 'педофил',
    'гомик', 'гомосек', 'гомосятин', 'гей тупой',
    'гей опущ', 'голубой опущ', 'голубок гей', 'шлюх',
    'шлюшк', 'шлюхен', 'шлюховск', 'шлюшар',
    'блядин', 'блядск', 'блядун', 'блядов',
    'блядищ', 'блятство', 'проститут', 'проститук',
    'потаскух', 'потаскун', 'потаскушк', 'шалав',
    'шалашовк', 'шалашн', 'давалк', 'давала',
    'давалов', 'мандавошк', 'мандовошк', 'сучк',
    'сука', 'сукин', 'сучар', 'сученыш',
    'сучий', 'курв', 'курва', 'курвин',
    'шкур', 'шкура', 'шкурех', 'шкурён',
    'тёлк дешёв', 'телка деш', 'целка', 'целочк',
    'мандавош', 'мандень', 'манденыш', 'пизда',
    'пиздюк', 'пиздабол', 'пиздёнк', 'пиздоват',
    'залупа', 'залуп', 'залупон', 'хуесос',
    'хуесоск', 'хуепутал', 'хуеплёт', 'хуеплет',
    'членосос', 'членодевк', 'спермоглот', 'спермобак',
    'анус твой', 'очко твоё', 'очкан', 'анальн раб',
    'сперматозоид', 'гондон штопан', 'резинка пользов', 'блудниц',
    'распутн', 'развратн', 'гулящ', 'шваль панельн',
    'панельн дев', 'ночн бабочк', 'жрица любв', 'путан',
    'эскортниц', 'шмар', 'шмара', 'марух',
    'метрес', 'сожительниц', 'любовниц дешёв', 'самка',
    'сучка течн', 'кобель', 'озабоч', 'извращен',
    'извращ', 'эксгибицион', 'педрильск', 'гомосятск',
    'трансуха', 'транвестит', 'шим', 'оральн раб',
    'ротожоп', 'ротоблуд', 'спермоприём', 'залупоконь',
    'елдак', 'сдохни', 'сдохн', 'сдохнеш',
    'подохни', 'подохн', 'подохнеш', 'здохни',
    'здохн', 'убей себя', 'убейся', 'убейс',
    'убилб', 'убил бы', 'убить себя', 'удавись',
    'удавис', 'удавил бы', 'повесься', 'повес',
    'повесил бы', 'вешайся', 'сожги себя', 'сожги себ',
    'спрыгни', 'прыгни с', 'прыгай с', 'выпили себя',
    'выпилис', 'выпилил', 'выпили себ', 'самоубий',
    'суициднись', 'вскрой вены', 'вены вскр', 'вскройся',
    'убить тебя', 'убью тебя', 'убью нахуй', 'зарежу',
    'зарежем', 'режу теб', 'прирежу', 'порешу',
    'порешаю', 'порешим', 'грохну', 'грохнем',
    'грохну теб', 'замочу', 'замочим', 'мочи его',
    'закопаю', 'закопаем', 'закопаю теб', 'урою',
    'уроем', 'урою теб', 'размажу', 'размажем',
    'размажу по', 'изобью', 'изобьём', 'изобьем',
    'отпизж', 'отпижд', 'отхуяч', 'ебало разобью',
    'ебло разобью', 'рожу набью', 'морду набью', 'морду расквас',
    'рыло начищу', 'рыло сверну', 'табло набью', 'голову оторву',
    'башку оторву', 'руки оторву', 'ноги переломаю', 'кости переломаю',
    'рёбра перелом', 'хребет сломаю', 'шею сверну', 'придушу',
    'задушу', 'задавлю', 'задавим', 'сломаю тебе',
    'сломаю теб', 'покалечу', 'изувечу', 'искалечу',
    'инвалидом сдел', 'на костыл', 'в больниц попад', 'в реанимац',
    'в морг', 'на тот свет', 'закопают теб', 'гроб заколоч',
    'могил вырою', 'пожалеешь', 'ты пожалееш', 'найду тебя',
    'найду и убью', 'достану тебя', 'достану теб', 'выследю',
    'выслеж', 'вычислю теб', 'адрес узнаю', 'адрес пробью',
    'знаю где живёшь', 'знаю где живеш', 'знаю где ты', 'за тобой придут',
    'придём за теб', 'ты труп', 'ты покойник', 'тебе конец',
    'тебе пизд', 'тебе хана', 'тебе кранты', 'тебе крышка',
    'конец тебе', 'закончишь плохо', 'плохо кончиш', 'домой не дойдёшь',
    'домой не дойдеш', 'живым не уйдёш', 'ноги вырву', 'никому не нужен',
    'никому не нужна', 'никому ненуж', 'всем похуй на теб', 'всем плевать на теб',
    'всем насрать на теб', 'тебя никто не любит', 'тебя никто не лю', 'тебя все ненавид',
    'все тебя ненавид', 'тебя ненавиж', 'ненавижу теб', 'тебя терпеть',
    'терпеть тебя не', 'ты ничто', 'ты никто', 'ты пустое',
    'ты пустышк', 'лучше б ты не родил', 'лучше бы не родил', 'зря тебя родил',
    'зря родил', 'зачем ты родил', 'зачем родил', 'ошибка природы',
    'ошибка природ', 'ошибка рожден', 'выкидыш', 'недоношен',
    'аборт надо был', 'тебя надо был абортир', 'зря живёшь', 'зря живеш',
    'зря коптишь', 'коптишь небо', 'небо коптиш', 'место на кладбищ',
    'тебе место на кладб', 'тебе не место', 'тут тебе не место', 'вали отсюда',
    'вали отсюд', 'катись отсюда', 'катись отсюд', 'катись колбас',
    'уёбывай', 'уебывай', 'уёбыв', 'уебыв',
    'съебись', 'съебис', 'съёбывай', 'съебывай',
    'съёбыв', 'съебыв', 'свали уже', 'свали нахуй',
    'исчезни', 'исчезни нахуй', 'сгинь', 'проваливай',
    'проваливай нахуй', 'пшёл вон', 'пошёл вон', 'пошел вон',
    'выметайся', 'убирайся', 'заткни пасть', 'завали ебало',
    'завали ебло', 'завали пасть', 'завали хлебал', 'закрой рот',
    'закрой свой рот', 'рот закрой', 'рот завали', 'рот прикрой',
    'молчи в тряпочк', 'в тряпочк молчи', 'не вякай', 'не тявкай',
    'не пизди', 'хорош пиздет', 'хватит пиздет', 'не выёбывайся',
    'не выебывайся', 'ты обуза', 'ты груз', 'ты балласт',
    'от тебя одни проблем', 'от тебя вред', 'от тебя толку', 'без тебя лучше',
    'без тебя обойд', 'лучше б тебя', 'тебя забудут', 'тебя не вспомн',
    'о тебе никто', 'ты позор', 'позор семьи', 'позор рода',
    'позорищ', 'ходячий позор', 'стыдно за теб', 'стыд один',
    'убожество ходяч', 'ходячее недоразумен', 'ходячая ошибк', 'днище',
    'днищ', 'ты на дне', 'на самом дне', 'ниже плинтус',
    'днищенск', 'днярь', 'зашкварн ты', 'ты конч как человек',
    'как человек ты ноль', 'человек-ошибка', 'биомусор', 'биоотход',
    'генетич мусор', 'мусор генет', 'отработанн материал', 'расходник',
    'пушечн мясо', 'никчёмн существ', 'жалкое подоб', 'жалкое зрелищ',
    'жалок ты', 'жирн', 'жируха', 'жиртрест',
    'жиробас', 'жиробан', 'жирдяй', 'жирок наел',
    'свинья жирн', 'свиноматк', 'свиномордый', 'хряк',
    'боров', 'кабан жирн', 'тушка жир', 'толстуха',
    'толстяк жирн', 'толстопуз', 'пузан', 'пузатый',
    'брюхан', 'брюхо отрастил', 'ожирел', 'разъел рожу',
    'рожу разъел', 'ряху разъел', 'ряху наел', 'харю наел',
    'щёки отъел', 'урод страшн', 'страшил', 'страшила',
    'страшнее', 'страшен как', 'рожа кривая', 'морда кривая',
    'морда кирпич', 'кривомордый', 'криворыл', 'косоглаз',
    'косорыл', 'косоглазик', 'лысый череп', 'лысина блестит',
    'лысина', 'плешив', 'плешак', 'лысый чёрт',
    'зубы гнилые', 'зубы кривые', 'щербат', 'беззубый',
    'гнилозуб', 'вонючк', 'вонюч', 'воняешь',
    'вонища от теб', 'потн', 'пропотел', 'потлив',
    'потные подмыш', 'немыт', 'грязнуля', 'грязный как',
    'чумаз', 'чушок', 'чухан', 'нищенск вид',
    'оборванск вид', 'анорексичк', 'анорекс', 'скелет ходяч',
    'скелетин', 'кощей', 'кащей', 'дрищ худой',
    'глист', 'глиста бледн', 'шпала', 'каланча',
    'дылда', 'оглобл', 'коротыш', 'коротышк',
    'недомерок', 'метр с кепк', 'карлик', 'карлан',
    'гном тупой', 'лилипут', 'пигмей', 'прыщав',
    'прыщеват', 'угреват', 'угрёв', 'рябой',
    'конопат', 'веснушч урод', 'очкарик', 'очкарь',
    'четырёхглаз', 'четырехглаз', 'слепошар', 'косая морда',
    'бледная немочь', 'синяк под глаз', 'мешки под глаз', 'рожа опух',
    'опухшая рожа', 'морщин старая', 'старпёр', 'старпер',
    'старая карга', 'карга', 'ведьма старая', 'бабка дряхл',
    'дед древний', 'развалин', 'песок сыпл', 'обрюзг',
    'обвисл', 'целлюлит', 'жирные склад', 'складки жира',
    'трёхэтажн подбород', 'второй подбород', 'задница как', 'жопа толст',
    'жопа разъел', 'suka', 'suchka', 'blyat',
    'blyad', 'blya', 'pidor', 'pidoras',
    'pidr', 'pidaras', 'mraz', '",\\\\\\\\\\\\\\\\n    ',
    ', ', ',\\\\\\\\\\\\\\\\n    ', ', "you', 'ur trash',
    'dumbass', 'dumbfuck', 'retard', 'retarded',
    'moron', 'imbecile', 'asshole', 'bastard',
    'motherf', 'fuck you', 'fuck off', 'fuck u',
    'gtfo', 'stfu', 'cunt', 'whore',
    'slut', 'bitch', 'faggot', 'fagot',
    'fag', 'dickhead', 'piece of shit', 'pos',
    'kys urself', 'go hang', 'hang yourself', 'drink bleach',
    'slit', 'cut yourself', 'nobody loves', 'nobody likes',
    'everyone hates', 'ur worthless', 'you worthless', 'уебіще',
    'сабака', 'пайдёш', 'здохні', 'паміраеш',
    'тупіца', 'дзяун', 'ідыёт', 'вырадак',
    'гандон', 'гондон', 'гандошник', 'мудак',
    'мудачьё', 'мудил', 'мудозвон', 'мудень',
    'мудак конч', 'распиздяй', 'разъебай', 'разъёбан',
    'ебанат', 'ёбнут', 'ебанут', 'ебанашк',
    'ёбарь', 'ебарь', 'ёбырь', 'выебок',
    'выёбок', 'заёбhouse', 'заебал ты', 'заебала ты',
    'заёбыва', 'заебыва', 'задолбал ты', 'достал ты',
    'говнюк', 'говноед', 'говнар', 'говнюш',
    'говнист', 'говённ', 'говнецо', 'засранец',
    'засрал', 'засранк', 'обосрал', 'обоссал',
    'обоссан', 'зассан', 'серун', 'серунья',
    'ссыкоман', 'пердун', 'пердёж', 'пердун старый',
    'вонючий пердун', 'пердабол', 'хмырь', 'хмыр',
    'хрен с горы', 'хер моржов', 'хер с бугра', 'хрен моржов',
    'членистоног', 'дрочер', 'дрочила', 'дрочун',
    'онанист тупой', 'рукоблуд', 'рукоблудник', 'импотент',
    'импотенц', 'кончен', 'конченый', 'конченая',
    'конченн', 'отморозок', 'отморозк', 'отмороженн',
    'беспредельщик', 'беспредел творит', 'быдлоту', 'скотомудак',
    'ебланище', 'хуило', 'хуйло', 'хуила',
    'хуеглот', 'елда', 'хер тебе', 'хуй тебе',
    'иди в жопу', 'иди в пизду', 'пшёл нахуй', 'пошёл нахуй',
    'пошел нахуй', 'да пошёл ты', 'да иди ты', 'в рот ебал',
    'в рот тебя', 'ебал я теб', 'ебал тебя', 'насрать на теб',
    'насрал на теб', 'похуй на теб', 'хуй на теб', 'хуй забил',
    'забей хуй', 'выкуси', 'соси хуй', 'сосни хуйц',
    'иди соси', 'хрен соси', 'жри говно', 'сожри говно',
    'ешь говно', 'говно жуй', 'говна поешь', 'утрись',
    'заглохни', 'завянь', 'сдулся ты', 'слился ты',
    'обтекай', 'обтекаешь', 'утекай', 'умойся',
    'умылся ты', 'проглоти язык', 'язык проглоти', 'прикуси язык',
    'ебало', 'ебло', 'заткни еб', 'ебал',
    'kill yourself', 'kill urself', 'kys', 'go die',
    'никому не нуж', 'ты ненуж', 'завали еб', 'тупорылост',
    'тупоумие', 'придурковат', 'недоразвитост', 'скудоумие',
    'безмозглост', 'тупорогост', 'деревянноголов', 'полудурье',
    'дурачество', 'балбесин', 'обалдуй', 'оболтусин',
    'тупайс', 'тупень', 'тупизн', 'дуралей',
    'дурында', 'остолопин', 'недоумочност', 'кретинищ',
    'дебилоидн', 'имбецильн', 'идиотиничн', 'клиническ идиот',
    'умственно убог', 'мозгов с гулькин', 'ни ума ни', 'дурак дураком',
    'тупее пробки', 'тупее валенка', 'мразотин', 'мерзотин',
    'паскудство', 'гнидск', 'подонковщин', 'сволочизм',
    'сволочьё', 'тварство', 'гадство', 'скотств',
    'быдлятин', 'быдлоид', 'хамство', 'хамьюга',
    'жлобство', 'нищебродск', 'голодранств', 'оборванств',
    'помойщик', 'клоунск', 'шутовск', 'паясничеств',
    'фиглярств', 'мразь конченн', 'выродочн', 'ублюдочност',
    'отбросн', 'отребьев', 'швалин', 'шушерн',
    'падальн', 'гнильц', 'гнилостн', 'мерзопакостн',
    'крысятничеств', 'шныровск', 'барыжничеств', 'спекулянтск',
    'живоглотств', 'кровопийств', 'паразитизм', 'нахлебник',
    'дармоед', 'тунеядец', 'прихлебатель', 'приживала',
    'присосалс', 'трусливост', 'трусопат', 'ссыкливост',
    'зассанец', 'очкун', 'очковтиратель', 'слабачеств',
    'тряпичност', 'мямленье', 'нытьишк', 'плаксивост',
    'сопляческ', 'хлюпиков', 'дрищеват', 'дохлятинк',
    'заморышн', 'задротств', 'ботанств', 'лошарск',
    'лоховщин', 'терпильств', 'петушинск', 'опущенств',
    'шестёрочн', 'холуйств', 'подхалимаж', 'лизоблюдств',
    'подстилочн', 'шакальн', 'слизнячеств', 'безвольност',
    'тюфячн', 'недомужичн', 'слюнтяйск', 'нюнеть',
    'раскисл', 'разнюнилс', 'сдрейфилс', 'струсилс',
    'пидорск', 'пидарск', 'пидрильск', 'педрильн',
    'педофильск', 'гомосячеств', 'шлюшеств', 'блядовск',
    'блядство', 'блядюшк', 'проститутск', 'потаскушеств',
    'шалавск', 'давалкин', 'мандавошечн', 'сучество',
    'сученышев', 'шкурничеств', 'целкин', 'пиздюшн',
    'пиздаболь', 'хуеплётск', 'членососн', 'спермоглотск',
    'блудняк', 'распутств', 'развратниче', 'гулящест',
    'панельщиц', 'путанск', 'шмарин', 'марушн',
    'сожительск', 'самкин', 'кобелин', 'озабоченн',
    'извращенц', 'извращенск', 'эксгибициониз', 'трансушн',
    'ротожопн', 'ротоблудн', 'залупоконск', 'сдохн бы',
    'подохн бы', 'здохн бы', 'удушу теб', 'удавлю теб',
    'утоплю теб', 'спалю теб', 'сожгу теб', 'взорву теб',
    'расчленю', 'расчленю теб', 'четвертую', 'выпотрошу',
    'кишки выпущу', 'кишки вырву', 'глаза выколю', 'зубы выбью',
    'челюсть сломаю', 'нос расквашу', 'башку проломлю', 'череп проломлю',
    'мозги вышибу', 'пристрелю', 'застрелю', 'зарою живьём',
    'в землю закатаю', 'в бетон закатаю', 'на ремни порежу', 'на куски порежу',
    'фарш сделаю', 'отбивную сделаю', 'калекой сделаю', 'инвалидом останешь',
    'ходить не будеш', 'жить не будеш', 'света не увид', 'дни сочтены',
    'часы сочтены', 'недолго осталось', 'скоро сдохнеш', 'готовь гроб',
    'рой могилу', 'молись пока', 'покойся с', 'уже мертвец',
    'ходячий труп', 'трупом будеш', 'на ноль помножу', 'из-под земли достан',
    'нигде не спрячеш', 'везде найду', 'достану везде', 'никто не любит теб',
    'всем на тебя плев', 'всем ты безразлич', 'никому не интерес', 'тебя все презир',
    'презираю теб', 'брезгую тобой', 'противен ты', 'омерзителен ты',
    'тошнит от теб', 'блевать от теб', 'воняет от теб', 'фу какой ты',
    'мерзкий ты', 'жалкий ты', 'ничтожный ты', 'пустой ты',
    'бесполезный ты', 'никчёмный ты', 'зря живёш', 'лучше сдохн',
    'лучше б сдох', 'зря родилс', 'не надо было рожать', 'выкидышем надо был',
    'семья стыдит', 'родители стыдят', 'позоришь всех', 'всех позориш',
    'на тебе пробы негде', 'клеймо ставить негде', 'на самом дне ты', 'ниже некуда ты',
    'хуже не быва', 'отбитый ты', 'больной на голову', 'псих ненормальн',
    'шизик', 'шизоид', 'даун конченн', 'уёбок конченн',
    'конч ты как личност', 'как личность ты ноль', 'ты пустое место', 'тебя забудут через',
    'о тебе не вспомн', 'следа не остан', 'мокрого места не', 'жиробасин',
    'жиртрестин', 'жирдяйск', 'свинорыл', 'свинопас',
    'кабанид', 'боровин', 'тушин', 'толстожоп',
    'жопаст', 'пузатост', 'брюхатост', 'обжорск',
    'рожемордый', 'мордоворот', 'кривожоп', 'косопуз',
    'косоморд', 'лысопер', 'плешивост', 'зубоскал',
    'гнилозубост', 'вонючест', 'потливост', 'грязнулин',
    'чумазост', 'сопленос', 'прыщеморд', 'угреморд',
    'рябоморд', 'конопатост', 'веснушчат', 'очкастост',
    'слепошарост', 'дохлятск', 'костлявост', 'скелетоподобн',
    'глистоподобн', 'дрищеватост', 'шпалоподобн', 'каланчист',
    'недомерочн', 'карликов', 'гномьес', 'лилипутск',
    'уродск', 'страховидл', 'страхолюдн', 'образин',
    'рожа кирпич', 'морда топор', 'лицо кирпич', 'рожа клином',
    'мордень', 'гандонск', 'гондонск', 'мудачест',
    'мудозвонск', 'распиздяйств', 'разъебайств', 'ебанатск',
    'ебанутост', 'ебланство', 'ёбнутост', 'выебонск',
    'заёбанн', 'задолбавш', 'говнюжеств', 'говноедск',
    'говнярск', 'засранств', 'обосранн', 'обоссанн',
    'серунск', 'пердунск', 'хмырьё', 'дрочерск',
    'дрочильн', 'онанистск', 'рукоблудств', 'импотентск',
    'конченост', 'отморозочн', 'беспредельн', 'свиномордост',
    'скотомудачь', 'хуилов', 'хуйлищ', 'хуеглотск',
    'елдачн', 'в рот ногами', 'в рот компот', 'в очко',
    'в анус', 'соси банан', 'соси залуп', 'на хую вертел',
    'хуй с тобой', 'хуй на рыло', 'насрать мне на', 'плевать мне на',
    'ноль для меня', 'пустое место для', 'suchara', 'blyadina',
    'pidorasina', 'mudilo', 'debilojd', 'urodina',
    'lohushka', 'chmoshnik', 'zaebal ty', 'poshel ty',
    'idi ty nahuy', 'huyeglot', 'hueplet', 'mrazota',
    'tvarina', 'scumbag', 'lowlife', 'degenerate',
    'subhuman', 'worthless piece', 'kill yrself', 'go die already',
    'nobody cares about', 'everyone hates you', 'ur a loser', 'youre pathetic',
    'безмозгло', 'тупоголо', 'тупоумо', 'скудоумо',
    'придурочеств', 'дебилизаци', 'идиотств', 'кретиниз',
    'имбецильност', 'олигофреник', 'дегенеративност', 'даунячест',
    'умалишённост', 'слабоумств', 'тупоумств', 'недоразвитк',
    'умом не блещ', 'мозги набекрен', 'без царя в голов', 'крыша поехал',
    'крыша съехал', 'ку-ку ты', 'ауешник тупой', 'тупое животн',
    'интеллекта ноль', 'ноль мозгов', 'мозг ампутир', 'извилин нет',
    'одна извилин', 'думать нечем', 'соображалк нет', 'котелок не вар',
    'голова два уха', 'дубовая башк', 'деревянная башк', 'мразотищ',
    'мерзавчик', 'подончик', 'гнидыш', 'сволочуг',
    'тварюшк', 'гадёнышев', 'скотиняк', 'быдляра',
    'хамьюжник', 'жлобяра', 'нищеброж', 'голодранчик',
    'босяцк', 'клоунессн', 'паяцств', 'фигляж',
    'крысёнышев', 'шныряг', 'барыжник', 'спекуляж',
    'живоглотн', 'кровопивец', 'паразитск', 'нахлебнич',
    'дармоедств', 'тунеядств', 'прихлебательств', 'приживальств',
    'мерзотник', 'выблядыш', 'падальщин', 'стервятничеств',
    'шавкодав', 'шакальств', 'трусишка позорн', 'ссыкунишк',
    'зассыха', 'очкарь трусл', 'слабачишк', 'тряпколог',
    'мямлик', 'нытикан', 'плаксун', 'соплежуй',
    'соплежевател', 'хлюпикан', 'дрищеван', 'дохлячок',
    'заморышек', 'задротяра', 'ботанюг', 'лошарик',
    'лошпедик', 'терпилушк', 'петушара', 'опущенчик',
    'зашкварник', 'шестёрочник', 'холуйчик', 'подхалимчик',
    'лизоблюдик', 'жополизик', 'подстилочк', 'шавочк',
    'пидорочк', 'пидарюг', 'педрильчик', 'гомосятник',
    'шлюхозавр', 'блядюга', 'проститутищ', 'потаскунья',
    'шалавушк', 'давалочк', 'мандавошечк', 'сученция',
    'курвишк', 'шкуреха', 'мандюк', 'пиздюшок',
    'пиздаболк', 'залупник', 'хуесосина', 'членососка',
    'блудняга', 'распутниц', 'развратниц', 'гулёна',
    'панельщин', 'путанк', 'шмарюг', 'самкан',
    'кобелина', 'озабоченк', 'извращуг', 'эксгибиц',
    'трансушк', 'ротожопик', 'залупоконин', 'удавлю гад',
    'утоплю гад', 'зарежу гад', 'прирежу гад', 'грохну гад',
    'замочу гад', 'урою гад', 'размажу гад', 'изобью гад',
    'придушу гад', 'задушу гад', 'закопаю гад', 'расчленю гад',
    'выпотрошу теб', 'кишки намотаю', 'голову снесу', 'башку снесу',
    'шею сверну теб', 'рёбра пересчита', 'зубы посчита', 'руки вырву',
    'язык вырву', 'глотку перегрызу', 'горло перережу', 'вены вскрою теб',
    'на органы пущу', 'на мясо пущу', 'в фарш', 'в труху',
    'сотру в порошок', 'живого места не остав', 'места живого не', 'мокрого места не остав',
    'из тебя отбивн', 'похороню теб', 'закатаю в асфальт', 'сгною теб',
    'уничтожу теб', 'раздавлю теб', 'сломаю жизнь теб', 'жизни не дам',
    'достану из-под', 'найду где угодно', 'всё равно найду', 'тебя все презирают',
    'все над тобой смеют', 'посмешище ты', 'объект насмешек', 'клоун для всех',
    'тебя все используют', 'ты для битья', 'мальчик для битья', 'груша для битья',
    'козёл отпущения', 'вечный неудачник', 'по жизни лох', 'по жизни терпил',
    'родился неудачник', 'проклятье семьи', 'обуза для всех', 'балласт для чата',
    'гиря на ногах', 'камень на ше', 'токсичное чмо', 'энергетический вампир',
    'нытик вечный', 'зануда конченн', 'душнила конченн', 'токсик ты',
    'никому ты не друг', 'друзей у тебя нет', 'один как перст', 'всеми брошен',
    'всеми покинут', 'никто не заплач', 'забудут на след', 'следа не остав',
    'пустое место занима', 'жирнота', 'жиробасищ', 'жиртрестищ',
    'свинорылост', 'кабанист', 'боровист', 'тушища',
    'толстожопост', 'пузатищ', 'брюхатищ', 'обжорищ',
    'мордоворотн', 'кривомордост', 'косопузост', 'лысопузост',
    'плешивец', 'зубоскалищ', 'гнилозубищ', 'потнота',
    'грязнулищ', 'сопленосн', 'прыщавост', 'угревост',
    'рябость', 'веснушчатост', 'слепошарость', 'костлявец',
    'глистин', 'дрищин', 'шпалин', 'каланчин',
    'коротышок', 'недомерочк', 'карличеств', 'гномищ',
    'лилипутищ', 'уродин конченн', 'страховидлищ', 'страхолюдищ',
    'образинищ', 'гандонищ', 'гондонищ', 'мудачищ',
    'мудозвонищ', 'распиздяйщин', 'ебанатищ', 'ебланищ',
    'выебонищ', 'говнюжищ', 'говноедищ', 'засранищ',
    'серунищ', 'пердунищ', 'дрочерищ', 'онанистищ',
    'импотентищ', 'отморозищ', 'беспределыщик', 'свиномордищ',
    'хуилищ', 'хуеглотищ', 'залупоконищ', 'елдачищ',
    'соси залупу', 'на хуй пош', 'в рот тебе', 'в очко тебе',
    'хуем по губам', 'обтекай говн', 'жри говнищ', 'утрись говн',
    'сожри дерьм', 'ешь дерьм', 'говна навали', 'blyadyuga',
    'pidorina', 'mudilishe', 'debiloid', 'urodische',
    'lohovshina', 'zaebalo ty', 'poshel von', 'idi v zhopu',
    'huesosina', 'mrazotina', 'tvaryuga', 'lowlife scum',
    'degenerate trash', 'subhuman filth', 'worthless trash', 'kys loser',
    'go rope', 'off yourself', 'neck yourself', 'end yourself',
    'nobody loves u', 'everyone hates u', 'побью теб', 'отмудохаю',
    'отметелю', 'отлуплю', 'отдубашу', 'отхуячу',
    'отпинаю', 'ногами запинаю', 'забью ногами', 'запинаю до',
    'отмолочу', 'ебало начищу', 'табло начищу', 'фейс набью',
    'лицо разобью', 'нос сломаю', 'челюсть сверну', 'зубы вынесу',
    'скулы сверну', 'бошку проломлю', 'голову проломлю', 'череп раскрою',
    'башню снесу', 'кость сломаю', 'рёбра поломаю', 'ребро сломаю',
    'руку сломаю', 'ногу сломаю', 'пальцы переломаю', 'позвоночник сломаю',
    'шею сломаю', 'свернуть шею', 'покрошу', 'отметелю до',
    'изобью до полус', 'убью теб', 'убъю', 'пришью',
    'завалю теб', 'прикончу', 'укокошу', 'замочу теб',
    'ножом пырну', 'пырну ножом', 'заколю', 'зарублю',
    'расстреляю', 'пулю в лоб', 'пулю получишь', 'свинца получишь',
    'удавлю', 'удушу', 'утоплю', 'повешу',
    'вздёрну', 'сожгу заживо', 'спалю заживо', 'подорву',
    'зарою', 'закатаю', 'похороню', 'в могилу свед',
    'в гроб загоню', 'отправлю на тот', 'жизни лишу', 'лишу жизни',
    'прикончу теб', 'кончу теб', 'конец тебе придёт', 'сдохнешь от моих',
    'от моей руки', 'мной будешь убит', 'выпущу кишки', 'глаз выколю',
    'язык вырежу', 'язык отрежу', 'глотку перережу', 'глотку порву',
    'вены вскрою', 'на куски', 'на части порежу', 'на ремни',
    'на фарш', 'в фарш пущу', 'мясо сделаю', 'шашлык сделаю',
    'на органы', 'скальп сниму', 'шкуру спущу', 'кожу сдеру',
    'живьём закопаю', 'живьём сожгу', 'живьём съем', 'по частям',
    'расчлененк', 'выслежу', 'вычислю', 'адрес найду',
    'из-под земли', 'где живёшь знаю', 'знаю где ты живёш', 'приду к тебе домой',
    'приеду к теб', 'встретимся в реал', 'выйди в реал', 'выйдем поговорим',
    'забьём стрелу', 'стрелку забьём', 'тебе пиздец', 'тебе амба',
    'ты мертвец', 'считай ты мёртв', 'домой не вернёш', 'не доживёш',
    'недолго протянеш', 'дни твои сочтен', 'час твой близ', 'молись пока живой',
    'чемодан вокзал', 'чемодан вокзал наху', 'чемодан вокзал росси', 'чемодан вокзал украин',
    'чемонад вокзал', 'вокзал чемодан', 'собирай чемодан', 'пакуй чемодан',
    'на вокзал иди', 'вали лесом', 'иди лесом', 'шёл бы ты лесом',
    'иди погуляй', 'иди проветрис', 'иди проспис', 'вали в закат',
    'катись колбаской', 'катись отседов', 'отвали уже', 'отвянь',
    'отцепис', 'отъебис', 'отъебись от', 'отстань уебок',
    'свали с глаз', 'сгинь с глаз', 'прочь с глаз', 'уйди в небыти',
    'самоустранис', 'депортирую', 'на мороз иди', 'иди на мороз',
    'в бан вали', 'кринж ходяч', 'кринжовый ты', 'ты кринж',
    'зашквар ходяч', 'ты зашквар', 'душнила', 'токсик конченн',
    'нонейм', 'ты нонейм', 'ноунейм', 'ливай отсюда',
    'ливни уже', 'го ливни', 'скуф', 'ты скуф',
    'скуфидон', 'скуфыч', 'альтушка дешёв', 'чел ты',
    'чел ты соснул', 'соснул', 'ты соснул', 'пасс ты',
    'рофлан ебало', 'ебало рофлан', 'имбецил ливни', 'краш кринжовый',
    'ты днина', 'днина', 'даунич', 'дегродан',
    'дегродом', 'ты овощ', 'биоматериал', 'расходник ты',
    'статист ты', 'массовка ты', 'обоссан и обос', 'обоссыш',
    'зашкварен по', 'шарящий за ноль', 'ноль по жизни', 'ливер ты',
    'нуб конченн', 'нубас', 'ты нубас', 'рак конченн',
    'ты рак', 'раковый', 'рачьё', 'рукожоп',
    'криворук', 'днарь', 'ты днарь', 'ливнул как',
    'тиммейт дно', 'фидер', 'ты фидер', '바보 нуб',
    'ливни из катки', 'удоли акк', 'удали тгк', 'удали себя',
    'делитни себя', 'делит акк', 'сноси прилож', 'рейджквитни',
    'slil katku', 'go next без теб', 'репортни себя', 'анимешник конченн',
    'задрот конченн', 'инцел', 'ты инцел', 'инцелина',
    'симп ты', 'симпан', 'вайпни себя', 'крашс об стену',
    'соевый ты', 'сойджак', 'нормис тупой', 'редпилл тупой',
    'чадоносец', 'virgin ты', 'вирджин', 'кучер дивана',
    'диванный воин', 'клавиатурный воин', 'анонимный смельчак', 'храбрец из-за экрана',
    'петух интернетн', 'клоун интернетн', 'ты ошибка', 'ты глюк',
    'ты баг', 'удалённый персонаж', 'забаненный по жизни', 'читер по жизни',
    'тебя забанит жизнь', 'жизнь тебя кикнет', 'реролл нужен', 'ребутни жизнь',
    'рестартни себя', 'ты лагаешь по жизни', 'пингуешь мозгом', 'соединение с мозгом потер',
    'мозг офлайн', 'мозг 404', 'айкью комнатной', 'айкью ниже плинтус',
    'два класса образован', 'три класса церковно', 'эй ты убогое', 'слышь ты дно',
    'слышь чмо', 'слышь додик', 'додик', 'ты додик',
    'додикан', 'лошпедрила', 'петушила', 'петуховод',
    'зашкварыш', 'чушпан', 'ты чушпан', 'чушпанск',
    'быдловатый', 'гопота тупая', 'гопник конченн', 'шпана тупая',
    'урла', 'ты урла', 'зэчуга', 'ливни отсюда',
    'балбес грёбаный', 'балбес драный', 'балбес ебаный', 'балбес конченый',
    'балбес позорный', 'балбес сраный', 'балбес тупой', 'балбес ёбаный',
    'барыга грёбаный', 'барыга драный', 'барыга ебаный', 'барыга конченый',
    'барыга позорный', 'барыга сраный', 'барыга тупой', 'барыга ёбаный',
    'беспредельщик грёбаный', 'беспредельщик драный', 'беспредельщик ебаный', 'беспредельщик конченый',
    'беспредельщик позорный', 'беспредельщик сраный', 'беспредельщик тупой', 'беспредельщик ёбаный',
    'биомусор грёбаный', 'биомусор драный', 'биомусор ебаный', 'биомусор конченый',
    'биомусор позорный', 'биомусор сраный', 'биомусор тупой', 'биомусор ёбаный',
    'блядина грёбаный', 'блядина драный', 'блядина ебаный', 'блядина конченый',
    'блядина позорный', 'блядина сраный', 'блядина тупой', 'блядина ёбаный',
    'болван грёбаный', 'болван драный', 'болван ебаный', 'болван конченый',
    'болван позорный', 'болван сраный', 'болван тупой', 'болван ёбаный',
    'боров грёбаный', 'боров драный', 'боров ебаный', 'боров конченый',
    'боров позорный', 'боров сраный', 'боров тупой', 'боров ёбаный',
    'быдлан грёбаный', 'быдлан драный', 'быдлан ебаный', 'быдлан конченый',
    'быдлан позорный', 'быдлан сраный', 'быдлан тупой', 'быдлан ёбаный',
    'быдло грёбаный', 'быдло драный', 'быдло ебаный', 'быдло конченый',
    'быдло позорный', 'быдло сраный', 'быдло тупой', 'быдло ёбаный',
    'вонючка грёбаный', 'вонючка драный', 'вонючка ебаный', 'вонючка конченый',
    'вонючка позорный', 'вонючка сраный', 'вонючка тупой', 'вонючка ёбаный',
    'выблядок грёбаный', 'выблядок драный', 'выблядок ебаный', 'выблядок конченый',
    'выблядок позорный', 'выблядок сраный', 'выблядок тупой', 'выблядок ёбаный',
    'выродок грёбаный', 'выродок драный', 'выродок ебаный', 'выродок конченый',
    'выродок позорный', 'выродок сраный', 'выродок тупой', 'выродок ёбаный',
    'гадина грёбаный', 'гадина драный', 'гадина ебаный', 'гадина конченый',
    'гадина позорный', 'гадина сраный', 'гадина тупой', 'гадина ёбаный',
    'гандон грёбаный', 'гандон драный', 'гандон ебаный', 'гандон конченый',
    'гандон позорный', 'гандон сраный', 'гандон тупой', 'гандон ёбаный',
    'глист грёбаный', 'глист драный', 'глист ебаный', 'глист конченый',
    'глист позорный', 'глист сраный', 'глист тупой', 'глист ёбаный',
    'гнида грёбаный', 'гнида драный', 'гнида ебаный', 'гнида конченый',
    'гнида позорный', 'гнида сраный', 'гнида тупой', 'гнида ёбаный',
    'гнусь грёбаный', 'гнусь драный', 'гнусь ебаный', 'гнусь конченый',
    'гнусь позорный', 'гнусь сраный', 'гнусь тупой', 'гнусь ёбаный',
    'говноед грёбаный', 'говноед драный', 'говноед ебаный', 'говноед конченый',
    'говноед позорный', 'говноед сраный', 'говноед тупой', 'говноед ёбаный',
    'говнюк грёбаный', 'говнюк драный', 'говнюк ебаный', 'говнюк конченый',
    'говнюк позорный', 'говнюк сраный', 'говнюк тупой', 'говнюк ёбаный',
    'гомик грёбаный', 'гомик драный', 'гомик ебаный', 'гомик конченый',
    'гомик позорный', 'гомик сраный', 'гомик тупой', 'гомик ёбаный',
    'гондон грёбаный', 'гондон драный', 'гондон ебаный', 'гондон конченый',
    'гондон позорный', 'гондон сраный', 'гондон тупой', 'гондон ёбаный',
    'гопник грёбаный', 'гопник драный', 'гопник ебаный', 'гопник конченый',
    'гопник позорный', 'гопник сраный', 'гопник тупой', 'гопник ёбаный',
    'грязнуля грёбаный', 'грязнуля драный', 'грязнуля ебаный', 'грязнуля конченый',
    'грязнуля позорный', 'грязнуля сраный', 'грязнуля тупой', 'грязнуля ёбаный',
    'гулящая грёбаный', 'гулящая драный', 'гулящая ебаный', 'гулящая конченый',
    'гулящая позорный', 'гулящая сраный', 'гулящая тупой', 'гулящая ёбаный',
    'давалка грёбаный', 'давалка драный', 'давалка ебаный', 'давалка конченый',
    'давалка позорный', 'давалка сраный', 'давалка тупой', 'давалка ёбаный',
    'дармоед грёбаный', 'дармоед драный', 'дармоед ебаный', 'дармоед конченый',
    'дармоед позорный', 'дармоед сраный', 'дармоед тупой', 'дармоед ёбаный',
    'даун грёбаный', 'даун драный', 'даун ебаный', 'даун конченый',
    'даун позорный', 'даун сраный', 'даун тупой', 'даун ёбаный',
    'дебил грёбаный', 'дебил драный', 'дебил ебаный', 'дебил конченый',
    'дебил позорный', 'дебил сраный', 'дебил тупой', 'дебил ёбаный',
    'дегенерат грёбаный', 'дегенерат драный', 'дегенерат ебаный', 'дегенерат конченый',
    'дегенерат позорный', 'дегенерат сраный', 'дегенерат тупой', 'дегенерат ёбаный',
    'днина грёбаный', 'днина драный', 'днина ебаный', 'днина конченый',
    'днина позорный', 'днина сраный', 'днина тупой', 'днина ёбаный',
    'днище грёбаный', 'днище драный', 'днище ебаный', 'днище конченый',
    'днище позорный', 'днище сраный', 'днище тупой', 'днище ёбаный',
    'додик грёбаный', 'додик драный', 'додик ебаный', 'додик конченый',
    'додик позорный', 'додик сраный', 'додик тупой', 'додик ёбаный',
    'долбоеб грёбаный', 'долбоеб драный', 'долбоеб ебаный', 'долбоеб конченый',
    'долбоеб позорный', 'долбоеб сраный', 'долбоеб тупой', 'долбоеб ёбаный',
    'долбоёб грёбаный', 'долбоёб драный', 'долбоёб ебаный', 'долбоёб конченый',
    'долбоёб позорный', 'долбоёб сраный', 'долбоёб тупой', 'долбоёб ёбаный',
    'дохляк грёбаный', 'дохляк драный', 'дохляк ебаный', 'дохляк конченый',
    'дохляк позорный', 'дохляк сраный', 'дохляк тупой', 'дохляк ёбаный',
    'драный балбес', 'драный барыга', 'драный беспредельщик', 'драный биомусор',
    'драный блядина', 'драный болван', 'драный боров', 'драный быдлан',
    'драный быдло', 'драный вонючка', 'драный выблядок', 'драный выродок',
    'драный гадина', 'драный гандон', 'драный глист', 'драный гнида',
    'драный гнусь', 'драный говноед', 'драный говнюк', 'драный гомик',
    'драный гондон', 'драный гопник', 'драный грязнуля', 'драный гулящая',
    'драный давалка', 'драный дармоед', 'драный даун', 'драный дебил',
    'драный дегенерат', 'драный днина', 'драный днище', 'драный додик',
    'драный долбоеб', 'драный долбоёб', 'драный дохляк', 'драный дрищ',
    'драный дрочер', 'драный дрочила', 'драный дурак', 'драный душнила',
    'драный ебанат', 'драный еблан', 'драный живоглот', 'драный жирдяй',
    'драный жиробас', 'драный жиртрест', 'драный жлоб', 'драный жополиз',
    'драный задрот', 'драный засранец', 'драный зашкварник', 'драный зэк',
    'драный идиот', 'драный извращенец', 'драный имбецил', 'драный импотент',
    'драный инцел', 'драный кабан', 'драный карлик', 'драный клоун',
    'драный кобель', 'драный конченый', 'драный коротышка', 'драный косоглазый',
    'драный косорылый', 'драный кретин', 'драный криворукий', 'драный кринж',
    'драный кровопийца', 'драный крыса', 'драный курва', 'драный лизоблюд',
    'драный лох', 'драный лошара', 'драный лысый', 'драный мерзавец',
    'драный мраз', 'драный мразота', 'драный мудак', 'драный мудила',
    'драный мудозвон', 'драный нахлебник', 'драный недомерок', 'драный недоумок',
    'драный недочеловек', 'драный никчёмность', 'драный ничтожество', 'драный нищеброд',
    'драный нормис', 'драный нубас', 'драный нытик', 'драный нюня',
    'драный онанист', 'драный опущенец', 'драный отброс', 'драный отморозок',
    'драный отребье', 'драный очкарик', 'драный падаль', 'драный паразит',
    'драный паскуда', 'драный педик', 'драный пердун', 'драный петух',
    'драный пигмей', 'драный пидор', 'драный пидорас', 'драный пиздабол',
    'драный плакса', 'драный плешивый', 'драный подонок', 'драный подстилка',
    'драный подхалим', 'драный посмешище', 'драный потаскуха', 'драный придурок',
    'драный проститутка', 'драный прыщавый', 'драный развратница', 'драный размазня',
    'драный распутница', 'драный рукожоп', 'драный самка', 'драный свиномордый',
    'драный свинья', 'драный сволочь', 'драный серун', 'драный симп',
    'драный скелет', 'драный скотина', 'драный скуф', 'драный слабак',
    'драный слизняк', 'драный слюнтяй', 'драный сопляк', 'драный ссыкло',
    'драный страшила', 'драный сучка', 'драный тварь', 'драный терпила',
    'драный токсик', 'драный толстуха', 'драный толстяк', 'драный трус',
    'драный тряпка', 'драный тунеядец', 'драный тупица', 'драный тупой',
    'драный туша', 'драный тюфяк', 'драный ублюд', 'драный ублюдок',
    'драный убогий', 'драный убожество', 'драный уебок', 'драный урла',
    'драный урод', 'драный уродец', 'драный уродина', 'драный уёбок',
    'драный хам', 'драный хлюпик', 'драный холуй', 'драный хуесос',
    'драный целка', 'драный членосос', 'драный чмо', 'драный чмошник',
    'драный чмырь', 'драный чушпан', 'драный шавка', 'драный шакал',
    'драный шалава', 'драный шваль', 'драный шестёрка', 'драный шкура',
    'драный шлюха', 'драный шныр', 'драный шпала', 'драный шпана',
    'драный шут', 'дрищ грёбаный', 'дрищ драный', 'дрищ ебаный',
    'дрищ конченый', 'дрищ позорный', 'дрищ сраный', 'дрищ тупой',
    'дрищ ёбаный', 'дрочер грёбаный', 'дрочер драный', 'дрочер ебаный',
    'дрочер конченый', 'дрочер позорный', 'дрочер сраный', 'дрочер тупой',
    'дрочер ёбаный', 'дрочила грёбаный', 'дрочила драный', 'дрочила ебаный',
    'дрочила конченый', 'дрочила позорный', 'дрочила сраный', 'дрочила тупой',
    'дрочила ёбаный', 'дурак грёбаный', 'дурак драный', 'дурак ебаный',
    'дурак конченый', 'дурак позорный', 'дурак сраный', 'дурак тупой',
    'дурак ёбаный', 'душнила грёбаный', 'душнила драный', 'душнила ебаный',
    'душнила конченый', 'душнила позорный', 'душнила сраный', 'душнила тупой',
    'душнила ёбаный', 'ебанат грёбаный', 'ебанат драный', 'ебанат ебаный',
    'ебанат конченый', 'ебанат позорный', 'ебанат сраный', 'ебанат тупой',
    'ебанат ёбаный', 'ебаный балбес', 'ебаный барыга', 'ебаный беспредельщик',
    'ебаный биомусор', 'ебаный блядина', 'ебаный болван', 'ебаный боров',
    'ебаный быдлан', 'ебаный быдло', 'ебаный вонючка', 'ебаный выблядок',
    'ебаный выродок', 'ебаный гадина', 'ебаный гандон', 'ебаный глист',
    'ебаный гнида', 'ебаный гнусь', 'ебаный говноед', 'ебаный говнюк',
    'ебаный гомик', 'ебаный гондон', 'ебаный гопник', 'ебаный грязнуля',
    'ебаный гулящая', 'ебаный давалка', 'ебаный дармоед', 'ебаный даун',
    'ебаный дебил', 'ебаный дегенерат', 'ебаный днина', 'ебаный днище',
    'ебаный додик', 'ебаный долбоеб', 'ебаный долбоёб', 'ебаный дохляк',
    'ебаный дрищ', 'ебаный дрочер', 'ебаный дрочила', 'ебаный дурак',
    'ебаный душнила', 'ебаный ебанат', 'ебаный еблан', 'ебаный живоглот',
    'ебаный жирдяй', 'ебаный жиробас', 'ебаный жиртрест', 'ебаный жлоб',
    'ебаный жополиз', 'ебаный задрот', 'ебаный засранец', 'ебаный зашкварник',
    'ебаный зэк', 'ебаный идиот', 'ебаный извращенец', 'ебаный имбецил',
    'ебаный импотент', 'ебаный инцел', 'ебаный кабан', 'ебаный карлик',
    'ебаный клоун', 'ебаный кобель', 'ебаный конченый', 'ебаный коротышка',
    'ебаный косоглазый', 'ебаный косорылый', 'ебаный кретин', 'ебаный криворукий',
    'ебаный кринж', 'ебаный кровопийца', 'ебаный крыса', 'ебаный курва',
    'ебаный лизоблюд', 'ебаный лох', 'ебаный лошара', 'ебаный лысый',
    'ебаный мерзавец', 'ебаный мраз', 'ебаный мразота', 'ебаный мудак',
    'ебаный мудила', 'ебаный мудозвон', 'ебаный нахлебник', 'ебаный недомерок',
    'ебаный недоумок', 'ебаный недочеловек', 'ебаный никчёмность', 'ебаный ничтожество',
    'ебаный нищеброд', 'ебаный нормис', 'ебаный нубас', 'ебаный нытик',
    'ебаный нюня', 'ебаный онанист', 'ебаный опущенец', 'ебаный отброс',
    'ебаный отморозок', 'ебаный отребье', 'ебаный очкарик', 'ебаный падаль',
    'ебаный паразит', 'ебаный паскуда', 'ебаный педик', 'ебаный пердун',
    'ебаный петух', 'ебаный пигмей', 'ебаный пидор', 'ебаный пидорас',
    'ебаный пиздабол', 'ебаный плакса', 'ебаный плешивый', 'ебаный подонок',
    'ебаный подстилка', 'ебаный подхалим', 'ебаный посмешище', 'ебаный потаскуха',
    'ебаный придурок', 'ебаный проститутка', 'ебаный прыщавый', 'ебаный развратница',
    'ебаный размазня', 'ебаный распутница', 'ебаный рукожоп', 'ебаный самка',
    'ебаный свиномордый', 'ебаный свинья', 'ебаный сволочь', 'ебаный серун',
    'ебаный симп', 'ебаный скелет', 'ебаный скотина', 'ебаный скуф',
    'ебаный слабак', 'ебаный слизняк', 'ебаный слюнтяй', 'ебаный сопляк',
    'ебаный ссыкло', 'ебаный страшила', 'ебаный сучка', 'ебаный тварь',
    'ебаный терпила', 'ебаный токсик', 'ебаный толстуха', 'ебаный толстяк',
    'ебаный трус', 'ебаный тряпка', 'ебаный тунеядец', 'ебаный тупица',
    'ебаный тупой', 'ебаный туша', 'ебаный тюфяк', 'ебаный ублюд',
    'ебаный ублюдок', 'ебаный убогий', 'ебаный убожество', 'ебаный уебок',
    'ебаный урла', 'ебаный урод', 'ебаный уродец', 'ебаный уродина',
    'ебаный уёбок', 'ебаный хам', 'ебаный хлюпик', 'ебаный холуй',
    'ебаный хуесос', 'ебаный целка', 'ебаный членосос', 'ебаный чмо',
    'ебаный чмошник', 'ебаный чмырь', 'ебаный чушпан', 'ебаный шавка',
    'ебаный шакал', 'ебаный шалава', 'ебаный шваль', 'ебаный шестёрка',
    'ебаный шкура', 'ебаный шлюха', 'ебаный шныр', 'ебаный шпала',
    'ебаный шпана', 'ебаный шут', 'еблан грёбаный', 'еблан драный',
    'еблан ебаный', 'еблан конченый', 'еблан позорный', 'еблан сраный',
    'еблан тупой', 'еблан ёбаный', 'живоглот грёбаный', 'живоглот драный',
    'живоглот ебаный', 'живоглот конченый', 'живоглот позорный', 'живоглот сраный',
    'живоглот тупой', 'живоглот ёбаный', 'жирдяй грёбаный', 'жирдяй драный',
    'жирдяй ебаный', 'жирдяй конченый', 'жирдяй позорный', 'жирдяй сраный',
    'жирдяй тупой', 'жирдяй ёбаный', 'жиробас грёбаный', 'жиробас драный',
    'жиробас ебаный', 'жиробас конченый', 'жиробас позорный', 'жиробас сраный',
    'жиробас тупой', 'жиробас ёбаный', 'жиртрест грёбаный', 'жиртрест драный',
    'жиртрест ебаный', 'жиртрест конченый', 'жиртрест позорный', 'жиртрест сраный',
    'жиртрест тупой', 'жиртрест ёбаный', 'жлоб грёбаный', 'жлоб драный',
    'жлоб ебаный', 'жлоб конченый', 'жлоб позорный', 'жлоб сраный',
    'жлоб тупой', 'жлоб ёбаный', 'жополиз грёбаный', 'жополиз драный',
    'жополиз ебаный', 'жополиз конченый', 'жополиз позорный', 'жополиз сраный',
    'жополиз тупой', 'жополиз ёбаный', 'задрот грёбаный', 'задрот драный',
    'задрот ебаный', 'задрот конченый', 'задрот позорный', 'задрот сраный',
    'задрот тупой', 'задрот ёбаный', 'засранец грёбаный', 'засранец драный',
    'засранец ебаный', 'засранец конченый', 'засранец позорный', 'засранец сраный',
    'засранец тупой', 'засранец ёбаный', 'зашкварник грёбаный', 'зашкварник драный',
    'зашкварник ебаный', 'зашкварник конченый', 'зашкварник позорный', 'зашкварник сраный',
    'зашкварник тупой', 'зашкварник ёбаный', 'зэк грёбаный', 'зэк драный',
    'зэк ебаный', 'зэк конченый', 'зэк позорный', 'зэк сраный',
    'зэк тупой', 'зэк ёбаный', 'идиот грёбаный', 'идиот драный',
    'идиот ебаный', 'идиот конченый', 'идиот позорный', 'идиот сраный',
    'идиот тупой', 'идиот ёбаный', 'извращенец грёбаный', 'извращенец драный',
    'извращенец ебаный', 'извращенец конченый', 'извращенец позорный', 'извращенец сраный',
    'извращенец тупой', 'извращенец ёбаный', 'имбецил грёбаный', 'имбецил драный',
    'имбецил ебаный', 'имбецил конченый', 'имбецил позорный', 'имбецил сраный',
    'имбецил тупой', 'имбецил ёбаный', 'импотент грёбаный', 'импотент драный',
    'импотент ебаный', 'импотент конченый', 'импотент позорный', 'импотент сраный',
    'импотент тупой', 'импотент ёбаный', 'инцел грёбаный', 'инцел драный',
    'инцел ебаный', 'инцел конченый', 'инцел позорный', 'инцел сраный',
    'инцел тупой', 'инцел ёбаный', 'кабан грёбаный', 'кабан драный',
    'кабан ебаный', 'кабан конченый', 'кабан позорный', 'кабан сраный',
    'кабан тупой', 'кабан ёбаный', 'карлик грёбаный', 'карлик драный',
    'карлик ебаный', 'карлик конченый', 'карлик позорный', 'карлик сраный',
    'карлик тупой', 'карлик ёбаный', 'клоун грёбаный', 'клоун драный',
    'клоун ебаный', 'клоун конченый', 'клоун позорный', 'клоун сраный',
    'клоун тупой', 'клоун ёбаный', 'кобель грёбаный', 'кобель драный',
    'кобель ебаный', 'кобель конченый', 'кобель позорный', 'кобель сраный',
    'кобель тупой', 'кобель ёбаный', 'конченый балбес', 'конченый барыга',
    'конченый беспредельщик', 'конченый биомусор', 'конченый блядина', 'конченый болван',
    'конченый боров', 'конченый быдлан', 'конченый быдло', 'конченый вонючка',
    'конченый выблядок', 'конченый выродок', 'конченый гадина', 'конченый гандон',
    'конченый глист', 'конченый гнида', 'конченый гнусь', 'конченый говноед',
    'конченый говнюк', 'конченый гомик', 'конченый гондон', 'конченый гопник',
    'конченый грязнуля', 'конченый грёбаный', 'конченый гулящая', 'конченый давалка',
    'конченый дармоед', 'конченый даун', 'конченый дебил', 'конченый дегенерат',
    'конченый днина', 'конченый днище', 'конченый додик', 'конченый долбоеб',
    'конченый долбоёб', 'конченый дохляк', 'конченый драный', 'конченый дрищ',
    'конченый дрочер', 'конченый дрочила', 'конченый дурак', 'конченый душнила',
    'конченый ебанат', 'конченый ебаный', 'конченый еблан', 'конченый живоглот',
    'конченый жирдяй', 'конченый жиробас', 'конченый жиртрест', 'конченый жлоб',
    'конченый жополиз', 'конченый задрот', 'конченый засранец', 'конченый зашкварник',
    'конченый зэк', 'конченый идиот', 'конченый извращенец', 'конченый имбецил',
    'конченый импотент', 'конченый инцел', 'конченый кабан', 'конченый карлик',
    'конченый клоун', 'конченый кобель', 'конченый конченый', 'конченый коротышка',
    'конченый косоглазый', 'конченый косорылый', 'конченый кретин', 'конченый криворукий',
    'конченый кринж', 'конченый кровопийца', 'конченый крыса', 'конченый курва',
    'конченый лизоблюд', 'конченый лох', 'конченый лошара', 'конченый лысый',
    'конченый мерзавец', 'конченый мраз', 'конченый мразота', 'конченый мудак',
    'конченый мудила', 'конченый мудозвон', 'конченый нахлебник', 'конченый недомерок',
    'конченый недоумок', 'конченый недочеловек', 'конченый никчёмность', 'конченый ничтожество',
    'конченый нищеброд', 'конченый нормис', 'конченый нубас', 'конченый нытик',
    'конченый нюня', 'конченый онанист', 'конченый опущенец', 'конченый отброс',
    'конченый отморозок', 'конченый отребье', 'конченый очкарик', 'конченый падаль',
    'конченый паразит', 'конченый паскуда', 'конченый педик', 'конченый пердун',
    'конченый петух', 'конченый пигмей', 'конченый пидор', 'конченый пидорас',
    'конченый пиздабол', 'конченый плакса', 'конченый плешивый', 'конченый подонок',
    'конченый подстилка', 'конченый подхалим', 'конченый позорный', 'конченый посмешище',
    'конченый потаскуха', 'конченый придурок', 'конченый проститутка', 'конченый прыщавый',
    'конченый развратница', 'конченый размазня', 'конченый распутница', 'конченый рукожоп',
    'конченый самка', 'конченый свиномордый', 'конченый свинья', 'конченый сволочь',
    'конченый серун', 'конченый симп', 'конченый скелет', 'конченый скотина',
    'конченый скуф', 'конченый слабак', 'конченый слизняк', 'конченый слюнтяй',
    'конченый сопляк', 'конченый сраный', 'конченый ссыкло', 'конченый страшила',
    'конченый сучка', 'конченый тварь', 'конченый терпила', 'конченый токсик',
    'конченый толстуха', 'конченый толстяк', 'конченый трус', 'конченый тряпка',
    'конченый тунеядец', 'конченый тупица', 'конченый тупой', 'конченый туша',
    'конченый тюфяк', 'конченый ублюд', 'конченый ублюдок', 'конченый убогий',
    'конченый убожество', 'конченый уебок', 'конченый урла', 'конченый урод',
    'конченый уродец', 'конченый уродина', 'конченый уёбок', 'конченый хам',
    'конченый хлюпик', 'конченый холуй', 'конченый хуесос', 'конченый целка',
    'конченый членосос', 'конченый чмо', 'конченый чмошник', 'конченый чмырь',
    'конченый чушпан', 'конченый шавка', 'конченый шакал', 'конченый шалава',
    'конченый шваль', 'конченый шестёрка', 'конченый шкура', 'конченый шлюха',
    'конченый шныр', 'конченый шпала', 'конченый шпана', 'конченый шут',
    'конченый ёбаный', 'коротышка грёбаный', 'коротышка драный', 'коротышка ебаный',
    'коротышка конченый', 'коротышка позорный', 'коротышка сраный', 'коротышка тупой',
    'коротышка ёбаный', 'косоглазый грёбаный', 'косоглазый драный', 'косоглазый ебаный',
    'косоглазый конченый', 'косоглазый позорный', 'косоглазый сраный', 'косоглазый тупой',
    'косоглазый ёбаный', 'косорылый грёбаный', 'косорылый драный', 'косорылый ебаный',
    'косорылый конченый', 'косорылый позорный', 'косорылый сраный', 'косорылый тупой',
    'косорылый ёбаный', 'кретин грёбаный', 'кретин драный', 'кретин ебаный',
    'кретин конченый', 'кретин позорный', 'кретин сраный', 'кретин тупой',
    'кретин ёбаный', 'криворукий грёбаный', 'криворукий драный', 'криворукий ебаный',
    'криворукий конченый', 'криворукий позорный', 'криворукий сраный', 'криворукий тупой',
    'криворукий ёбаный', 'кринж грёбаный', 'кринж драный', 'кринж ебаный',
    'кринж конченый', 'кринж позорный', 'кринж сраный', 'кринж тупой',
    'кринж ёбаный', 'кровопийца грёбаный', 'кровопийца драный', 'кровопийца ебаный',
    'кровопийца конченый', 'кровопийца позорный', 'кровопийца сраный', 'кровопийца тупой',
    'кровопийца ёбаный', 'крыса грёбаный', 'крыса драный', 'крыса ебаный',
    'крыса конченый', 'крыса позорный', 'крыса сраный', 'крыса тупой',
    'крыса ёбаный', 'курва грёбаный', 'курва драный', 'курва ебаный',
    'курва конченый', 'курва позорный', 'курва сраный', 'курва тупой',
    'курва ёбаный', 'лизоблюд грёбаный', 'лизоблюд драный', 'лизоблюд ебаный',
    'лизоблюд конченый', 'лизоблюд позорный', 'лизоблюд сраный', 'лизоблюд тупой',
    'лизоблюд ёбаный', 'лох грёбаный', 'лох драный', 'лох ебаный',
    'лох конченый', 'лох позорный', 'лох сраный', 'лох тупой',
    'лох ёбаный', 'лошара грёбаный', 'лошара драный', 'лошара ебаный',
    'лошара конченый', 'лошара позорный', 'лошара сраный', 'лошара тупой',
    'лошара ёбаный', 'лысый грёбаный', 'лысый драный', 'лысый ебаный',
    'лысый конченый', 'лысый позорный', 'лысый сраный', 'лысый тупой',
    'лысый ёбаный', 'мерзавец грёбаный', 'мерзавец драный', 'мерзавец ебаный',
    'мерзавец конченый', 'мерзавец позорный', 'мерзавец сраный', 'мерзавец тупой',
    'мерзавец ёбаный', 'мраз грёбаный', 'мраз драный', 'мраз ебаный',
    'мраз конченый', 'мраз позорный', 'мраз сраный', 'мраз тупой',
    'мраз ёбаный', 'мразота грёбаный', 'мразота драный', 'мразота ебаный',
    'мразота конченый', 'мразота позорный', 'мразота сраный', 'мразота тупой',
    'мразота ёбаный', 'мудак грёбаный', 'мудак драный', 'мудак ебаный',
    'мудак конченый', 'мудак позорный', 'мудак сраный', 'мудак тупой',
    'мудак ёбаный', 'мудила грёбаный', 'мудила драный', 'мудила ебаный',
    'мудила конченый', 'мудила позорный', 'мудила сраный', 'мудила тупой',
    'мудила ёбаный', 'мудозвон грёбаный', 'мудозвон драный', 'мудозвон ебаный',
    'мудозвон конченый', 'мудозвон позорный', 'мудозвон сраный', 'мудозвон тупой',
    'мудозвон ёбаный', 'нахлебник грёбаный', 'нахлебник драный', 'нахлебник ебаный',
    'нахлебник конченый', 'нахлебник позорный', 'нахлебник сраный', 'нахлебник тупой',
    'нахлебник ёбаный', 'недомерок грёбаный', 'недомерок драный', 'недомерок ебаный',
    'недомерок конченый', 'недомерок позорный', 'недомерок сраный', 'недомерок тупой',
    'недомерок ёбаный', 'недоумок грёбаный', 'недоумок драный', 'недоумок ебаный',
    'недоумок конченый', 'недоумок позорный', 'недоумок сраный', 'недоумок тупой',
    'недоумок ёбаный', 'недочеловек грёбаный', 'недочеловек драный', 'недочеловек ебаный',
    'недочеловек конченый', 'недочеловек позорный', 'недочеловек сраный', 'недочеловек тупой',
    'недочеловек ёбаный', 'никчёмность грёбаный', 'никчёмность драный', 'никчёмность ебаный',
    'никчёмность конченый', 'никчёмность позорный', 'никчёмность сраный', 'никчёмность тупой',
    'никчёмность ёбаный', 'ничтожество грёбаный', 'ничтожество драный', 'ничтожество ебаный',
    'ничтожество конченый', 'ничтожество позорный', 'ничтожество сраный', 'ничтожество тупой',
    'ничтожество ёбаный', 'нищеброд грёбаный', 'нищеброд драный', 'нищеброд ебаный',
    'нищеброд конченый', 'нищеброд позорный', 'нищеброд сраный', 'нищеброд тупой',
    'нищеброд ёбаный', 'нормис грёбаный', 'нормис драный', 'нормис ебаный',
    'нормис конченый', 'нормис позорный', 'нормис сраный', 'нормис ёбаный',
    'нубас грёбаный', 'нубас драный', 'нубас ебаный', 'нубас конченый',
    'нубас позорный', 'нубас сраный', 'нубас тупой', 'нубас ёбаный',
    'нытик грёбаный', 'нытик драный', 'нытик ебаный', 'нытик конченый',
    'нытик позорный', 'нытик сраный', 'нытик тупой', 'нытик ёбаный',
    'нюня грёбаный', 'нюня драный', 'нюня ебаный', 'нюня конченый',
    'нюня позорный', 'нюня сраный', 'нюня тупой', 'нюня ёбаный',
    'онанист грёбаный', 'онанист драный', 'онанист ебаный', 'онанист конченый',
    'онанист позорный', 'онанист сраный', 'онанист ёбаный', 'опущенец грёбаный',
    'опущенец драный', 'опущенец ебаный', 'опущенец конченый', 'опущенец позорный',
    'опущенец сраный', 'опущенец тупой', 'опущенец ёбаный', 'отброс грёбаный',
    'отброс драный', 'отброс ебаный', 'отброс конченый', 'отброс позорный',
    'отброс сраный', 'отброс тупой', 'отброс ёбаный', 'отморозок грёбаный',
    'отморозок драный', 'отморозок ебаный', 'отморозок конченый', 'отморозок позорный',
    'отморозок сраный', 'отморозок тупой', 'отморозок ёбаный', 'отребье грёбаный',
    'отребье драный', 'отребье ебаный', 'отребье конченый', 'отребье позорный',
    'отребье сраный', 'отребье тупой', 'отребье ёбаный', 'очкарик грёбаный',
    'очкарик драный', 'очкарик ебаный', 'очкарик конченый', 'очкарик позорный',
    'очкарик сраный', 'очкарик тупой', 'очкарик ёбаный', 'падаль грёбаный',
    'падаль драный', 'падаль ебаный', 'падаль конченый', 'падаль позорный',
    'падаль сраный', 'падаль тупой', 'падаль ёбаный', 'паразит грёбаный',
    'паразит драный', 'паразит ебаный', 'паразит конченый', 'паразит позорный',
    'паразит сраный', 'паразит тупой', 'паразит ёбаный', 'паскуда грёбаный',
    'паскуда драный', 'паскуда ебаный', 'паскуда конченый', 'паскуда позорный',
    'паскуда сраный', 'паскуда тупой', 'паскуда ёбаный', 'педик грёбаный',
    'педик драный', 'педик ебаный', 'педик конченый', 'педик позорный',
    'педик сраный', 'педик тупой', 'педик ёбаный', 'пердун грёбаный',
    'пердун драный', 'пердун ебаный', 'пердун конченый', 'пердун позорный',
    'пердун сраный', 'пердун тупой', 'пердун ёбаный', 'петух грёбаный',
    'петух драный', 'петух ебаный', 'петух конченый', 'петух позорный',
    'петух сраный', 'петух тупой', 'петух ёбаный', 'пигмей грёбаный',
    'пигмей драный', 'пигмей ебаный', 'пигмей конченый', 'пигмей позорный',
    'пигмей сраный', 'пигмей тупой', 'пигмей ёбаный', 'пидор грёбаный',
    'пидор драный', 'пидор ебаный', 'пидор конченый', 'пидор позорный',
    'пидор сраный', 'пидор тупой', 'пидор ёбаный', 'пидорас грёбаный',
    'пидорас драный', 'пидорас ебаный', 'пидорас конченый', 'пидорас позорный',
    'пидорас сраный', 'пидорас тупой', 'пидорас ёбаный', 'пиздабол грёбаный',
    'пиздабол драный', 'пиздабол ебаный', 'пиздабол конченый', 'пиздабол позорный',
    'пиздабол сраный', 'пиздабол тупой', 'пиздабол ёбаный', 'плакса грёбаный',
    'плакса драный', 'плакса ебаный', 'плакса конченый', 'плакса позорный',
    'плакса сраный', 'плакса тупой', 'плакса ёбаный', 'плешивый грёбаный',
    'плешивый драный', 'плешивый ебаный', 'плешивый конченый', 'плешивый позорный',
    'плешивый сраный', 'плешивый тупой', 'плешивый ёбаный', 'подонок грёбаный',
    'подонок драный', 'подонок ебаный', 'подонок конченый', 'подонок позорный',
    'подонок сраный', 'подонок тупой', 'подонок ёбаный', 'подстилка грёбаный',
    'подстилка драный', 'подстилка ебаный', 'подстилка конченый', 'подстилка позорный',
    'подстилка сраный', 'подстилка тупой', 'подстилка ёбаный', 'подхалим грёбаный',
    'подхалим драный', 'подхалим ебаный', 'подхалим конченый', 'подхалим позорный',
    'подхалим сраный', 'подхалим тупой', 'подхалим ёбаный', 'посмешище грёбаный',
    'посмешище драный', 'посмешище ебаный', 'посмешище конченый', 'посмешище позорный',
    'посмешище сраный', 'посмешище тупой', 'посмешище ёбаный', 'потаскуха грёбаный',
    'потаскуха драный', 'потаскуха ебаный', 'потаскуха конченый', 'потаскуха позорный',
    'потаскуха сраный', 'потаскуха тупой', 'потаскуха ёбаный', 'придурок грёбаный',
    'придурок драный', 'придурок ебаный', 'придурок конченый', 'придурок позорный',
    'придурок сраный', 'придурок тупой', 'придурок ёбаный', 'проститутка грёбаный',
    'проститутка драный', 'проститутка ебаный', 'проститутка конченый', 'проститутка позорный',
    'проститутка сраный', 'проститутка тупой', 'проститутка ёбаный', 'прыщавый грёбаный',
    'прыщавый драный', 'прыщавый ебаный', 'прыщавый конченый', 'прыщавый позорный',
    'прыщавый сраный', 'прыщавый тупой', 'прыщавый ёбаный', 'развратница грёбаный',
    'развратница драный', 'развратница ебаный', 'развратница конченый', 'развратница позорный',
    'развратница сраный', 'развратница тупой', 'развратница ёбаный', 'размазня грёбаный',
    'размазня драный', 'размазня ебаный', 'размазня конченый', 'размазня позорный',
    'размазня сраный', 'размазня тупой', 'размазня ёбаный', 'распутница грёбаный',
    'распутница драный', 'распутница ебаный', 'распутница конченый', 'распутница позорный',
    'распутница сраный', 'распутница тупой', 'распутница ёбаный', 'рукожоп грёбаный',
    'рукожоп драный', 'рукожоп ебаный', 'рукожоп конченый', 'рукожоп позорный',
    'рукожоп сраный', 'рукожоп тупой', 'рукожоп ёбаный', 'самка грёбаный',
    'самка драный', 'самка ебаный', 'самка конченый', 'самка позорный',
    'самка сраный', 'самка тупой', 'самка ёбаный', 'свиномордый грёбаный',
    'свиномордый драный', 'свиномордый ебаный', 'свиномордый конченый', 'свиномордый позорный',
    'свиномордый сраный', 'свиномордый тупой', 'свиномордый ёбаный', 'свинья грёбаный',
    'свинья драный', 'свинья ебаный', 'свинья конченый', 'свинья позорный',
    'свинья сраный', 'свинья тупой', 'свинья ёбаный', 'сволочь грёбаный',
    'сволочь драный', 'сволочь ебаный', 'сволочь конченый', 'сволочь позорный',
    'сволочь сраный', 'сволочь тупой', 'сволочь ёбаный', 'серун грёбаный',
    'серун драный', 'серун ебаный', 'серун конченый', 'серун позорный',
    'серун сраный', 'серун тупой', 'серун ёбаный', 'симп грёбаный',
    'симп драный', 'симп ебаный', 'симп конченый', 'симп позорный',
    'симп сраный', 'симп тупой', 'симп ёбаный', 'скелет грёбаный',
    'скелет драный', 'скелет ебаный', 'скелет конченый', 'скелет позорный',
    'скелет сраный', 'скелет тупой', 'скелет ёбаный', 'скотина грёбаный',
    'скотина драный', 'скотина ебаный', 'скотина конченый', 'скотина позорный',
    'скотина сраный', 'скотина тупой', 'скотина ёбаный', 'скуф грёбаный',
    'скуф драный', 'скуф ебаный', 'скуф конченый', 'скуф позорный',
    'скуф сраный', 'скуф тупой', 'скуф ёбаный', 'слабак грёбаный',
    'слабак драный', 'слабак ебаный', 'слабак конченый', 'слабак позорный',
    'слабак сраный', 'слабак тупой', 'слабак ёбаный', 'слизняк грёбаный',
    'слизняк драный', 'слизняк ебаный', 'слизняк конченый', 'слизняк позорный',
    'слизняк сраный', 'слизняк тупой', 'слизняк ёбаный', 'слюнтяй грёбаный',
    'слюнтяй драный', 'слюнтяй ебаный', 'слюнтяй конченый', 'слюнтяй позорный',
    'слюнтяй сраный', 'слюнтяй тупой', 'слюнтяй ёбаный', 'сопляк грёбаный',
    'сопляк драный', 'сопляк ебаный', 'сопляк конченый', 'сопляк позорный',
    'сопляк сраный', 'сопляк тупой', 'сопляк ёбаный', 'ссыкло грёбаный',
    'ссыкло драный', 'ссыкло ебаный', 'ссыкло конченый', 'ссыкло позорный',
    'ссыкло сраный', 'ссыкло тупой', 'ссыкло ёбаный', 'страшила грёбаный',
    'страшила драный', 'страшила ебаный', 'страшила конченый', 'страшила позорный',
    'страшила сраный', 'страшила тупой', 'страшила ёбаный', 'сучка грёбаный',
    'сучка драный', 'сучка ебаный', 'сучка конченый', 'сучка позорный',
    'сучка сраный', 'сучка тупой', 'сучка ёбаный', 'тварь грёбаный',
    'тварь драный', 'тварь ебаный', 'тварь конченый', 'тварь позорный',
    'тварь сраный', 'тварь тупой', 'тварь ёбаный', 'терпила грёбаный',
    'терпила драный', 'терпила ебаный', 'терпила конченый', 'терпила позорный',
    'терпила сраный', 'терпила тупой', 'терпила ёбаный', 'токсик грёбаный',
    'токсик драный', 'токсик ебаный', 'токсик конченый', 'токсик позорный',
    'токсик сраный', 'токсик тупой', 'токсик ёбаный', 'толстуха грёбаный',
    'толстуха драный', 'толстуха ебаный', 'толстуха конченый', 'толстуха позорный',
    'толстуха сраный', 'толстуха тупой', 'толстуха ёбаный', 'толстяк грёбаный',
    'толстяк драный', 'толстяк ебаный', 'толстяк конченый', 'толстяк позорный',
    'толстяк сраный', 'толстяк тупой', 'толстяк ёбаный', 'трус грёбаный',
    'трус драный', 'трус ебаный', 'трус конченый', 'трус позорный',
    'трус сраный', 'трус тупой', 'трус ёбаный', 'тряпка грёбаный',
    'тряпка драный', 'тряпка ебаный', 'тряпка конченый', 'тряпка позорный',
    'тряпка сраный', 'тряпка тупой', 'тряпка ёбаный', 'тунеядец грёбаный',
    'тунеядец драный', 'тунеядец ебаный', 'тунеядец конченый', 'тунеядец позорный',
    'тунеядец сраный', 'тунеядец тупой', 'тунеядец ёбаный', 'тупица грёбаный',
    'тупица драный', 'тупица ебаный', 'тупица конченый', 'тупица позорный',
    'тупица сраный', 'тупица тупой', 'тупица ёбаный', 'тупой балбес',
    'тупой барыга', 'тупой беспредельщик', 'тупой биомусор', 'тупой блядина',
    'тупой болван', 'тупой боров', 'тупой быдлан', 'тупой быдло',
    'тупой вонючка', 'тупой выблядок', 'тупой выродок', 'тупой гадина',
    'тупой гандон', 'тупой глист', 'тупой гнида', 'тупой гнусь',
    'тупой говноед', 'тупой говнюк', 'тупой гомик', 'тупой гондон',
    'тупой гопник', 'тупой грязнуля', 'тупой грёбаный', 'тупой гулящая',
    'тупой давалка', 'тупой дармоед', 'тупой даун', 'тупой дебил',
    'тупой дегенерат', 'тупой днина', 'тупой днище', 'тупой додик',
    'тупой долбоеб', 'тупой долбоёб', 'тупой дохляк', 'тупой драный',
    'тупой дрищ', 'тупой дрочер', 'тупой дрочила', 'тупой дурак',
    'тупой душнила', 'тупой ебанат', 'тупой ебаный', 'тупой еблан',
    'тупой живоглот', 'тупой жирдяй', 'тупой жиробас', 'тупой жиртрест',
    'тупой жлоб', 'тупой жополиз', 'тупой задрот', 'тупой засранец',
    'тупой зашкварник', 'тупой зэк', 'тупой идиот', 'тупой извращенец',
    'тупой имбецил', 'тупой импотент', 'тупой инцел', 'тупой кабан',
    'тупой карлик', 'тупой клоун', 'тупой кобель', 'тупой конченый',
    'тупой коротышка', 'тупой косоглазый', 'тупой косорылый', 'тупой кретин',
    'тупой криворукий', 'тупой кринж', 'тупой кровопийца', 'тупой крыса',
    'тупой курва', 'тупой лизоблюд', 'тупой лох', 'тупой лошара',
    'тупой лысый', 'тупой мерзавец', 'тупой мраз', 'тупой мразота',
    'тупой мудак', 'тупой мудила', 'тупой мудозвон', 'тупой нахлебник',
    'тупой недомерок', 'тупой недоумок', 'тупой недочеловек', 'тупой никчёмность',
    'тупой ничтожество', 'тупой нищеброд', 'тупой нормис', 'тупой нубас',
    'тупой нытик', 'тупой нюня', 'тупой онанист', 'тупой опущенец',
    'тупой отброс', 'тупой отморозок', 'тупой отребье', 'тупой очкарик',
    'тупой падаль', 'тупой паразит', 'тупой паскуда', 'тупой педик',
    'тупой пердун', 'тупой петух', 'тупой пигмей', 'тупой пидор',
    'тупой пидорас', 'тупой пиздабол', 'тупой плакса', 'тупой плешивый',
    'тупой подонок', 'тупой подстилка', 'тупой подхалим', 'тупой позорный',
    'тупой посмешище', 'тупой потаскуха', 'тупой придурок', 'тупой проститутка',
    'тупой прыщавый', 'тупой развратница', 'тупой размазня', 'тупой распутница',
    'тупой рукожоп', 'тупой самка', 'тупой свиномордый', 'тупой свинья',
    'тупой сволочь', 'тупой серун', 'тупой симп', 'тупой скелет',
    'тупой скотина', 'тупой скуф', 'тупой слабак', 'тупой слизняк',
    'тупой слюнтяй', 'тупой сопляк', 'тупой сраный', 'тупой ссыкло',
    'тупой страшила', 'тупой сучка', 'тупой тварь', 'тупой терпила',
    'тупой токсик', 'тупой толстуха', 'тупой толстяк', 'тупой трус',
    'тупой тряпка', 'тупой тунеядец', 'тупой тупица', 'тупой тупой',
    'тупой туша', 'тупой тюфяк', 'тупой ублюд', 'тупой ублюдок',
    'тупой убогий', 'тупой убожество', 'тупой уебок', 'тупой урла',
    'тупой урод', 'тупой уродец', 'тупой уродина', 'тупой уёбок',
    'тупой хам', 'тупой хлюпик', 'тупой холуй', 'тупой хуесос',
    'тупой целка', 'тупой членосос', 'тупой чмо', 'тупой чмошник',
    'тупой чмырь', 'тупой чушпан', 'тупой шавка', 'тупой шакал',
    'тупой шалава', 'тупой шваль', 'тупой шестёрка', 'тупой шкура',
    'тупой шлюха', 'тупой шныр', 'тупой шпала', 'тупой шпана',
    'тупой шут', 'тупой ёбаный', 'туша грёбаный', 'туша драный',
    'туша ебаный', 'туша конченый', 'туша позорный', 'туша сраный',
    'туша тупой', 'туша ёбаный', 'тюфяк грёбаный', 'тюфяк драный',
    'тюфяк ебаный', 'тюфяк конченый', 'тюфяк позорный', 'тюфяк сраный',
    'тюфяк тупой', 'тюфяк ёбаный', 'ублюд грёбаный', 'ублюд драный',
    'ублюд ебаный', 'ублюд конченый', 'ублюд позорный', 'ублюд сраный',
    'ублюд тупой', 'ублюд ёбаный', 'ублюдок грёбаный', 'ублюдок драный',
    'ублюдок ебаный', 'ублюдок конченый', 'ублюдок позорный', 'ублюдок сраный',
    'ублюдок тупой', 'ублюдок ёбаный', 'убогий грёбаный', 'убогий драный',
    'убогий ебаный', 'убогий конченый', 'убогий позорный', 'убогий сраный',
    'убогий тупой', 'убогий ёбаный', 'убожество грёбаный', 'убожество драный',
    'убожество ебаный', 'убожество конченый', 'убожество позорный', 'убожество сраный',
    'убожество тупой', 'убожество ёбаный', 'уебок грёбаный', 'уебок драный',
    'уебок ебаный', 'уебок конченый', 'уебок позорный', 'уебок сраный',
    'уебок тупой', 'уебок ёбаный', 'урла грёбаный', 'урла драный',
    'урла ебаный', 'урла конченый', 'урла позорный', 'урла сраный',
    'урла тупой', 'урла ёбаный', 'урод грёбаный', 'урод драный',
    'урод ебаный', 'урод конченый', 'урод позорный', 'урод сраный',
    'урод тупой', 'урод ёбаный', 'уродец грёбаный', 'уродец драный',
    'уродец ебаный', 'уродец конченый', 'уродец позорный', 'уродец сраный',
    'уродец тупой', 'уродец ёбаный', 'уродина грёбаный', 'уродина драный',
    'уродина ебаный', 'уродина конченый', 'уродина позорный', 'уродина сраный',
    'уродина тупой', 'уродина ёбаный', 'уёбок грёбаный', 'уёбок драный',
    'уёбок ебаный', 'уёбок конченый', 'уёбок позорный', 'уёбок сраный',
    'уёбок тупой', 'уёбок ёбаный', 'хам грёбаный', 'хам драный',
    'хам ебаный', 'хам конченый', 'хам позорный', 'хам сраный',
    'хам тупой', 'хам ёбаный', 'хлюпик грёбаный', 'хлюпик драный',
    'хлюпик ебаный', 'хлюпик конченый', 'хлюпик позорный', 'хлюпик сраный',
    'хлюпик тупой', 'хлюпик ёбаный', 'холуй грёбаный', 'холуй драный',
    'холуй ебаный', 'холуй конченый', 'холуй позорный', 'холуй сраный',
    'холуй тупой', 'холуй ёбаный', 'хуесос грёбаный', 'хуесос драный',
    'хуесос ебаный', 'хуесос конченый', 'хуесос позорный', 'хуесос сраный',
    'хуесос тупой', 'хуесос ёбаный', 'целка грёбаный', 'целка драный',
    'целка ебаный', 'целка конченый', 'целка позорный', 'целка сраный',
    'целка тупой', 'целка ёбаный', 'членосос грёбаный', 'членосос драный',
    'членосос ебаный', 'членосос конченый', 'членосос позорный', 'членосос сраный',
    'членосос тупой', 'членосос ёбаный', 'чмо грёбаный', 'чмо драный',
    'чмо ебаный', 'чмо конченый', 'чмо позорный', 'чмо сраный',
    'чмо тупой', 'чмо ёбаный', 'чмошник грёбаный', 'чмошник драный',
    'чмошник ебаный', 'чмошник конченый', 'чмошник позорный', 'чмошник сраный',
    'чмошник тупой', 'чмошник ёбаный', 'чмырь грёбаный', 'чмырь драный',
    'чмырь ебаный', 'чмырь конченый', 'чмырь позорный', 'чмырь сраный',
    'чмырь тупой', 'чмырь ёбаный', 'чушпан грёбаный', 'чушпан драный',
    'чушпан ебаный', 'чушпан конченый', 'чушпан позорный', 'чушпан сраный',
    'чушпан тупой', 'чушпан ёбаный', 'шавка грёбаный', 'шавка драный',
    'шавка ебаный', 'шавка конченый', 'шавка позорный', 'шавка сраный',
    'шавка тупой', 'шавка ёбаный', 'шакал грёбаный', 'шакал драный',
    'шакал ебаный', 'шакал конченый', 'шакал позорный', 'шакал сраный',
    'шакал тупой', 'шакал ёбаный', 'шалава грёбаный', 'шалава драный',
    'шалава ебаный', 'шалава конченый', 'шалава позорный', 'шалава сраный',
    'шалава тупой', 'шалава ёбаный', 'шваль грёбаный', 'шваль драный',
    'шваль ебаный', 'шваль конченый', 'шваль позорный', 'шваль сраный',
    'шваль тупой', 'шваль ёбаный', 'шестёрка грёбаный', 'шестёрка драный',
    'шестёрка ебаный', 'шестёрка конченый', 'шестёрка позорный', 'шестёрка сраный',
    'шестёрка тупой', 'шестёрка ёбаный', 'шкура грёбаный', 'шкура драный',
    'шкура ебаный', 'шкура конченый', 'шкура позорный', 'шкура сраный',
    'шкура тупой', 'шкура ёбаный', 'шлюха грёбаный', 'шлюха драный',
    'шлюха ебаный', 'шлюха конченый', 'шлюха позорный', 'шлюха сраный',
    'шлюха тупой', 'шлюха ёбаный', 'шныр грёбаный', 'шныр драный',
    'шныр ебаный', 'шныр конченый', 'шныр позорный', 'шныр сраный',
    'шныр тупой', 'шныр ёбаный', 'шпала грёбаный', 'шпала драный',
    'шпала ебаный', 'шпала конченый', 'шпала позорный', 'шпала сраный',
    'шпала тупой', 'шпала ёбаный', 'шпана грёбаный', 'шпана драный',
    'шпана ебаный', 'шпана конченый', 'шпана позорный', 'шпана сраный',
    'шпана тупой', 'шпана ёбаный', 'шут грёбаный', 'шут драный',
    'шут ебаный', 'шут конченый', 'шут позорный', 'шут сраный',
    'шут тупой', 'шут ёбаный', 'ёбаный балбес', 'ёбаный барыга',
    'ёбаный беспредельщик', 'ёбаный биомусор', 'ёбаный блядина', 'ёбаный болван',
    'ёбаный боров', 'ёбаный быдлан', 'ёбаный быдло', 'ёбаный вонючка',
    'ёбаный выблядок', 'ёбаный выродок', 'ёбаный гадина', 'ёбаный гандон',
    'ёбаный глист', 'ёбаный гнида', 'ёбаный гнусь', 'ёбаный говноед',
    'ёбаный говнюк', 'ёбаный гомик', 'ёбаный гондон', 'ёбаный гопник',
    'ёбаный грязнуля', 'ёбаный гулящая', 'ёбаный давалка', 'ёбаный дармоед',
    'ёбаный даун', 'ёбаный дебил', 'ёбаный дегенерат', 'ёбаный днина',
    'ёбаный днище', 'ёбаный додик', 'ёбаный долбоеб', 'ёбаный долбоёб',
    'ёбаный дохляк', 'ёбаный дрищ', 'ёбаный дрочер', 'ёбаный дрочила',
    'ёбаный дурак', 'ёбаный душнила', 'ёбаный ебанат', 'ёбаный еблан',
    'ёбаный живоглот', 'ёбаный жирдяй', 'ёбаный жиробас', 'ёбаный жиртрест',
    'ёбаный жлоб', 'ёбаный жополиз', 'ёбаный задрот', 'ёбаный засранец',
    'ёбаный зашкварник', 'ёбаный зэк', 'ёбаный идиот', 'ёбаный извращенец',
    'ёбаный имбецил', 'ёбаный импотент', 'ёбаный инцел', 'ёбаный кабан',
    'ёбаный карлик', 'ёбаный клоун', 'ёбаный кобель', 'ёбаный конченый',
    'ёбаный коротышка', 'ёбаный косоглазый', 'ёбаный косорылый', 'ёбаный кретин',
    'ёбаный криворукий', 'ёбаный кринж', 'ёбаный кровопийца', 'ёбаный крыса',
    'ёбаный курва', 'ёбаный лизоблюд', 'ёбаный лох', 'ёбаный лошара',
    'ёбаный лысый', 'ёбаный мерзавец', 'ёбаный мраз', 'ёбаный мразота',
    'ёбаный мудак', 'ёбаный мудила', 'ёбаный мудозвон', 'ёбаный нахлебник',
    'ёбаный недомерок', 'ёбаный недоумок', 'ёбаный недочеловек', 'ёбаный никчёмность',
    'ёбаный ничтожество', 'ёбаный нищеброд', 'ёбаный нормис', 'ёбаный нубас',
    'ёбаный нытик', 'ёбаный нюня', 'ёбаный онанист', 'ёбаный опущенец',
    'ёбаный отброс', 'ёбаный отморозок', 'ёбаный отребье', 'ёбаный очкарик',
    'ёбаный падаль', 'ёбаный паразит', 'ёбаный паскуда', 'ёбаный педик',
    'ёбаный пердун', 'ёбаный петух', 'ёбаный пигмей', 'ёбаный пидор',
    'ёбаный пидорас', 'ёбаный пиздабол', 'ёбаный плакса', 'ёбаный плешивый',
    'ёбаный подонок', 'ёбаный подстилка', 'ёбаный подхалим', 'ёбаный посмешище',
    'ёбаный потаскуха', 'ёбаный придурок', 'ёбаный проститутка', 'ёбаный прыщавый',
    'ёбаный развратница', 'ёбаный размазня', 'ёбаный распутница', 'ёбаный рукожоп',
    'ёбаный самка', 'ёбаный свиномордый', 'ёбаный свинья', 'ёбаный сволочь',
    'ёбаный серун', 'ёбаный симп', 'ёбаный скелет', 'ёбаный скотина',
    'ёбаный скуф', 'ёбаный слабак', 'ёбаный слизняк', 'ёбаный слюнтяй',
    'ёбаный сопляк', 'ёбаный ссыкло', 'ёбаный страшила', 'ёбаный сучка',
    'ёбаный тварь', 'ёбаный терпила', 'ёбаный токсик', 'ёбаный толстуха',
    'ёбаный толстяк', 'ёбаный трус', 'ёбаный тряпка', 'ёбаный тунеядец',
    'ёбаный тупица', 'ёбаный тупой', 'ёбаный туша', 'ёбаный тюфяк',
    'ёбаный ублюд', 'ёбаный ублюдок', 'ёбаный убогий', 'ёбаный убожество',
    'ёбаный уебок', 'ёбаный урла', 'ёбаный урод', 'ёбаный уродец',
    'ёбаный уродина', 'ёбаный уёбок', 'ёбаный хам', 'ёбаный хлюпик',
    'ёбаный холуй', 'ёбаный хуесос', 'ёбаный целка', 'ёбаный членосос',
    'ёбаный чмо', 'ёбаный чмошник', 'ёбаный чмырь', 'ёбаный чушпан',
    'ёбаный шавка', 'ёбаный шакал', 'ёбаный шалава', 'ёбаный шваль',
    'ёбаный шестёрка', 'ёбаный шкура', 'ёбаный шлюха', 'ёбаный шныр',
    'ёбаный шпала', 'ёбаный шпана', 'ёбаный шут', 'слышь ты инцел',
    'вот ты ссыкло', 'эй ты отморозок', 'эй ты жиртрест', 'ты конченый дрищ',
    'эй ты мразь', 'ты позорный уёбок', 'ты конченый мразь', 'вот ты тварь',
    'ты жалкий дрочила', 'ты ёбаный скуф', 'слышь ты жополиз', 'ты позорный мразь',
    'ну ты и еблан', 'ты сраный выродок', 'ты позорный барыга', 'ты жалкий мудак',
    'блядина грёбаное', 'хуесос ебаное', 'ты конченый импотент', 'ты ёбаный карлик',
    'какой же ты урла', 'вот ты свинья', 'ну ты и быдло', 'ты настоящий блядина',
    'ты ёбаный дрищ', 'ты ёбаный тупой', 'какой же ты урод', 'ты тупой коротышка',
    'ты додик по жизни', 'слышь ты лошара', 'ты настоящий ссыкло', 'ты ёбаный мудак',
    'ты сраный гандон', 'ты конченый жлоб', 'ты настоящий свинья', 'ну ты и нубас',
    'ты сраный быдло', 'ты тупой выродок', 'вот ты сучка', 'ты просто засранец',
    'идиот грёбаное', 'ты ёбаный рукожоп', 'быдло ебаное', 'ну ты и паразит',
    'тупой грёбаное', 'ты настоящий трус', 'ты сраный чушпан', 'какой же ты душнила',
    'ты ёбаный днище', 'ты настоящий дебил', 'ты тупой шлюха', 'ты шалава по жизни',
    'какой же ты импотент', 'слышь ты слизняк', 'ты ссыкло по жизни', 'ты ёбаный трус',
    'ты тупой сучка', 'ты ёбаный слюнтяй', 'какой же ты пердун', 'ты ёбаный нубас',
    'ты настоящий гандон', 'ты холуй по жизни', 'ты просто дебил', 'ты ёбаный жиртрест',
    'ты позорный лошара', 'ты тупой гадина', 'вот ты говнюк', 'ты ёбаный говнюк',
    'какой же ты слюнтяй', 'ты ёбаный коротышка', 'ты сраный дебил', 'слышь ты чушпан',
    'слышь ты барыга', 'эй ты паразит', 'ты настоящий задрот', 'ты настоящий клоун',
    'ты тупой клоун', 'ты реальный днище', 'ты просто чмырь', 'ты сраный холуй',
    'ну ты и гопник', 'какой же ты дрочила', 'ты говнюк по жизни', 'ты сраный клоун',
    'слышь ты рукожоп', 'ты реальный страшила', 'вот ты пидор', 'ты сраный падаль',
    'ты тупой дебил', 'какой же ты ссыкло', 'ты сраный нытик', 'ты сраный шкура',
    'ты реальный додик', 'ну ты и жиртрест', 'ты просто рукожоп', 'ты позорный дебил',
    'гадина грёбаное', 'ты ёбаный очкарик', 'ну ты и крыса', 'эй ты нубас',
    'ты конченый токсик', 'ты тупой страшила', 'трус грёбаное', 'ты позорный токсик',
    'слышь ты клоун', 'ты тупой блядина', 'слышь ты падаль', 'ты жалкий днище',
    'вот ты крыса', 'ты конченый сопляк', 'ты сраный хуесос', 'ты настоящий ублюдок',
    'вот ты нытик', 'эй ты скелет', 'ты реальный слабак', 'слышь ты днище',
    'ты жалкий говнюк', 'ты настоящий дрочила', 'ссыкло грёбаное', 'вот ты ничтожество',
    'ты настоящий инцел', 'ну ты и слабак', 'падаль ебаное', 'эй ты тварь',
    'ты жалкий лошара', 'ты реальный мудак', 'ты настоящий еблан', 'какой же ты инцел',
    'ну ты и карлик', 'ты ёбаный крыса', 'ты сраный слюнтяй', 'ты тупой трус',
    'ну ты и тряпка', 'ну ты и мразь', 'ты реальный туша', 'вот ты инцел',
    'ты ёбаный страшила', 'ты настоящий шалава', 'ты просто даун', 'ты конченый уёбок',
    'ты просто сопляк', 'ты конченый жиртрест', 'эй ты дебил', 'ты шкура по жизни',
    'ты конченый нубас', 'ты настоящий токсик', 'ты конченый пердун', 'ты реальный чмырь',
    'быдло грёбаное', 'ты реальный биомусор', 'вот ты гандон', 'ты конченый лошара',
    'ты позорный пиздабол', 'ты реальный коротышка', 'слышь ты опущенец', 'жиробас ебаное',
    'душнила ебаное', 'эй ты шавка', 'ты конченый выродок', 'ты просто пидор',
    'эй ты ничтожество', 'ты настоящий тварь', 'ты жалкий уёбок', 'какой же ты чмырь',
    'какой же ты скелет', 'ты падаль по жизни', 'ты настоящий биомусор', 'ты скелет по жизни',
    'нытик грёбаное', 'ну ты и лох', 'ты конченый урла', 'эй ты говнюк',
    'клоун грёбаное', 'эй ты пидор', 'ты ёбаный ссыкло', 'ты нубас по жизни',
    'ты реальный уёбок', 'ты реальный пидор', 'скотина грёбаное', 'слышь ты дебил',
    'ты сраный чмырь', 'ты долбоёб по жизни', 'ты реальный жиробас', 'ну ты и очкарик',
    'вот ты шестёрка', 'ты сраный блядина', 'ты настоящий отморозок', 'ну ты и дармоед',
    'ты тупой гандон', 'ну ты и шлюха', 'ты сраный мразь', 'эй ты барыга',
    'пердун ебаное', 'ты реальный засранец', 'ну ты и падаль', 'ты сраный гадина',
    'слышь ты хам', 'ты настоящий сучка', 'какой же ты ничтожество', 'ты просто днище',
    'ты настоящий рукожоп', 'какой же ты скотина', 'ты конченый опущенец', 'какой же ты токсик',
    'ты конченый хам', 'ты реальный карлик', 'ты просто холуй', 'выродок ебаное',
    'какой же ты сучка', 'слышь ты выродок', 'ты тупой ничтожество', 'туша ебаное',
    'ты жалкий хуесос', 'скотина ебаное', 'ты жалкий долбоёб', 'ты жалкий терпила',
    'эй ты тупой', 'слышь ты туша', 'ты сраный шестёрка', 'ты позорный засранец',
    'ты реальный дрочила', 'ты днище по жизни', 'биомусор ебаное', 'ты просто лошара',
    'дебил ебаное', 'ты позорный быдло', 'ну ты и сопляк', 'ты клоун по жизни',
    'мудак грёбаное', 'ты ёбаный ничтожество', 'лох ебаное', 'крыса ебаное',
    'какой же ты трус', 'ты настоящий крыса', 'ты тупой импотент', 'ну ты и шавка',
    'ты конченый пидор', 'ну ты и пидор', 'ты позорный еблан', 'ты тупой по жизни',
    'вот ты выродок', 'задрот ебаное', 'ты тупой туша', 'ты реальный чушпан',
    'ты реальный нубас', 'какой же ты биомусор', 'даун ебаное', 'ты ёбаный шалава',
    'ты реальный мразь', 'свинья ебаное', 'ты реальный крыса', 'ты позорный чушпан',
    'урод грёбаное', 'ты конченый засранец', 'ты жалкий скотина', 'ты мудак по жизни',
    'ты реальный очкарик', 'ты конченый шестёрка', 'какой же ты нытик', 'ты сраный мудак',
    'шлюха ебаное', 'вот ты чмырь', 'ничтожество грёбаное', 'ты тупой отморозок',
    'даун грёбаное', 'урод ебаное', 'ты просто урод', 'ты сраный кретин',
    'ты просто нытик', 'ничтожество ебаное', 'вот ты мудак', 'ты жалкий задрот',
    'ты реальный инцел', 'ты реальный пиздабол', 'ты реальный долбоёб', 'ты ублюдок по жизни',
    'эй ты долбоёб', 'какой же ты жиробас', 'вот ты слюнтяй', 'какой же ты отморозок',
    'импотент ебаное', 'слышь ты дрищ', 'ты ёбаный слабак', 'ты тупой шалава',
    'эй ты страшила', 'ты жалкий урод', 'какой же ты шлюха', 'ты реальный скуф',
    'ты ёбаный гадина', 'эй ты дрищ', 'ты настоящий кретин', 'лошара ебаное',
    'какой же ты сопляк', 'вот ты туша', 'ты настоящий хуесос', 'ты просто гандон',
    'ты позорный гандон', 'ты жалкий чушпан', 'ты просто идиот', 'гандон грёбаное',
    'какой же ты дрищ', 'ты реальный ссыкло', 'ты конченый холуй', 'ты ёбаный тварь',
    'ты просто ссыкло', 'ты очкарик по жизни', 'вот ты жиртрест', 'ты выродок по жизни',
    'эй ты чмырь', 'ты свинья по жизни', 'долбоёб ебаное', 'ты конченый сучка',
    'ты просто очкарик', 'ты реальный сопляк', 'ты настоящий тупой', 'ну ты и тупой',
    'ты настоящий чмырь', 'ты конченый идиот', 'ты настоящий додик', 'ну ты и мудак',
    'ты реальный импотент', 'ты жалкий опущенец', 'петух грёбаное', 'какой же ты падаль',
    'ты тупой жиробас', 'ты настоящий скотина', 'вот ты чмо', 'ты еблан по жизни',
    'эй ты терпила', 'ты тупой ссыкло', 'ты настоящий жополиз', 'ну ты и ничтожество',
    'какой же ты петух', 'ты настоящий даун', 'ты жалкий пидор', 'слышь ты шлюха',
    'ты просто тупой', 'ты ёбаный петух', 'какой же ты карлик', 'ты реальный задрот',
    'ты настоящий чушпан', 'ты тупой долбоёб', 'гадина ебаное', 'эй ты душнила',
    'ты настоящий нытик', 'слышь ты очкарик', 'ты тупой лох', 'ты мразь по жизни',
    'ты сраный ничтожество', 'вот ты коротышка', 'ты петух по жизни', 'ты конченый паразит',
    'слышь ты сучка', 'ты настоящий опущенец', 'тварь ебаное', 'ты сраный днище',
    'скуф грёбаное', 'слышь ты скотина', 'слышь ты тупой', 'ты жалкий еблан',
    'слышь ты трус', 'эй ты шкура', 'вот ты ублюдок', 'ты позорный падаль',
    'эй ты быдло', 'ты позорный кретин', 'какой же ты страшила', 'ты сраный терпила',
    'ты конченый шкура', 'очкарик грёбаное', 'эй ты хам', 'ну ты и слизняк',
    'жополиз ебаное', 'рукожоп ебаное', 'эй ты опущенец', 'ты тупой засранец',
    'ты реальный терпила', 'ты реальный тупой', 'ты паразит по жизни', 'эй ты чушпан',
    'ты реальный жополиз', 'ты сопляк по жизни', 'ты позорный гопник', 'слышь ты жиробас',
    'ты ёбаный быдло', 'ты задрот по жизни', 'ты позорный чмырь', 'слышь ты сопляк',
    'ты тупой лошара', 'ты ёбаный хам', 'днище ебаное', 'вот ты шавка',
    'ты настоящий идиот', 'ты сраный импотент', 'душнила грёбаное', 'ты тупой барыга',
    'эй ты клоун', 'вот ты холуй', 'ты тупой быдло', 'ты позорный туша',
    'ты конченый очкарик', 'эй ты мудак', 'ты реальный шалава', 'ты тупой пидор',
    'ты ёбаный импотент', 'ну ты и шалава', 'ты настоящий карлик', 'ну ты и рукожоп',
    'блядина ебаное', 'вот ты биомусор', 'ты сраный додик', 'ты жалкий чмо',
    'вот ты рукожоп', 'ты засранец по жизни', 'ну ты и урла', 'ты ёбаный вонючка',
    'ты жалкий урла', 'ты жалкий шлюха', 'ты настоящий импотент', 'ты ёбаный мразь',
    'ты сраный пидор', 'ты уёбок по жизни', 'ты жиртрест по жизни', 'очкарик ебаное',
    'ну ты и биомусор', 'ты просто страшила', 'ну ты и ублюдок', 'ты реальный гадина',
    'ты просто душнила', 'ты сраный лох', 'ты жалкий отморозок', 'ну ты и туша',
    'ты ёбаный скелет', 'нубас грёбаное', 'ты жалкий ссыкло', 'ты настоящий падаль',
    'слышь ты слабак', 'ты настоящий сопляк', 'ты тупой еблан', 'ты сраный идиот',
    'вот ты скотина', 'ты сучка по жизни', 'ты ёбаный дебил', 'мразь грёбаное',
    'ты ёбаный еблан', 'засранец ебаное', 'эй ты жиробас', 'вот ты терпила',
    'ты просто терпила', 'ты жалкий дрищ', 'ты чмо по жизни', 'ты токсик по жизни',
    'ты конченый пиздабол', 'ну ты и уёбок', 'ты реальный скелет', 'вот ты слизняк',
    'ты реальный чмо', 'ты тупой слюнтяй', 'ты ёбаный шлюха', 'ты реальный кретин',
    'ты реальный еблан', 'слюнтяй грёбаное', 'ты просто ублюдок', 'эй ты биомусор',
    'ты настоящий скелет', 'слышь ты шкура', 'ты реальный петух', 'ты сраный инцел',
    'ну ты и чмырь', 'ты сраный шалава', 'вот ты урод', 'ты сраный даун',
    'слышь ты импотент', 'вот ты додик', 'ты просто трус', 'ты тупой рукожоп',
    'вот ты трус', 'ты тупой инцел', 'ну ты и нытик', 'ты жалкий тряпка',
    'ты пидор по жизни', 'какой же ты блядина', 'ты тупой додик', 'ты сраный опущенец',
    'какой же ты рукожоп', 'ты настоящий вонючка', 'ты настоящий терпила', 'какой же ты тварь',
    'ты сраный карлик', 'ты позорный блядина', 'ты жалкий страшила', 'ну ты и гадина',
    'эй ты очкарик', 'ты ёбаный урод', 'ты тупой шестёрка', 'вот ты жополиз',
    'дрищ ебаное', 'ты реальный лошара', 'ты конченый хуесос', 'ты шестёрка по жизни',
    'ты тупой жлоб', 'ты реальный выродок', 'ты реальный сучка', 'вот ты дебил',
    'жиробас грёбаное', 'жлоб грёбаное', 'вот ты долбоёб', 'ты реальный вонючка',
    'ты тряпка по жизни', 'эй ты туша', 'ты идиот по жизни', 'ты дармоед по жизни',
    'вот ты душнила', 'карлик ебаное', 'слышь ты ссыкло', 'ты просто говнюк',
    'ты конченый дрочила', 'ты конченый тупой', 'ты конченый жополиз', 'ты жалкий коротышка',
    'терпила грёбаное', 'ты жалкий дебил', 'ты ёбаный блядина', 'ты позорный шлюха',
    'ты конченый слабак', 'слышь ты карлик', 'ты сраный долбоёб', 'ты слизняк по жизни',
    'ты тупой задрот', 'ты настоящий шлюха', 'ты жалкий скуф', 'ты просто быдло',
    'эй ты жлоб', 'ты сраный петух', 'эй ты лох', 'ты настоящий мудак',
    'ты позорный скелет', 'ты ёбаный барыга', 'терпила ебаное', 'слышь ты чмырь',
    'ты тупой скотина', 'ты настоящий скуф', 'ты жалкий шавка', 'ты позорный вонючка',
    'ты просто тряпка', 'ты просто чушпан', 'барыга грёбаное', 'ты дрочила по жизни',
    'ты тупой терпила', 'ты просто жиробас', 'ты пиздабол по жизни', 'какой же ты мудак',
    'ты тупой мудак', 'какой же ты терпила', 'эй ты скуф', 'ты просто петух',
    'дармоед грёбаное', 'ты просто кретин', 'ты просто додик', 'ну ты и кретин',
    'ты сраный тряпка', 'эй ты шлюха', 'ты сраный сучка', 'ты тупой пердун',
    'скелет грёбаное', 'ты опущенец по жизни', 'ты просто импотент', 'чмырь ебаное',
    'ты сраный нубас', 'ты жлоб по жизни', 'ты настоящий засранец', 'жлоб ебаное',
    'ты позорный долбоёб', 'ну ты и жиробас', 'ну ты и вонючка', 'додик грёбаное',
    'ты жалкий пиздабол', 'ты кретин по жизни', 'ты позорный тупой', 'ты тупой опущенец',
    'ты терпила по жизни', 'какой же ты быдло', 'ты настоящий быдло', 'ты настоящий ничтожество',
    'ты жалкий рукожоп', 'эй ты карлик', 'чушпан грёбаное', 'ты жалкий гопник',
    'ты хам по жизни', 'ты ёбаный даун', 'слышь ты паразит', 'ты реальный душнила',
    'ну ты и коротышка', 'ну ты и терпила', 'ты позорный пердун', 'ты сраный трус',
    'ты позорный хуесос', 'вот ты задрот', 'ты сраный сопляк', 'вот ты вонючка',
    'ты конченый скелет', 'ты конченый тварь', 'страшила грёбаное', 'шавка ебаное',
    'токсик грёбаное', 'слышь ты гопник', 'ну ты и долбоёб', 'эй ты слизняк',
    'ты сраный жиртрест', 'дебил грёбаное', 'ну ты и жлоб', 'ты конченый барыга',
    'ты позорный лох', 'страшила ебаное', 'нубас ебаное', 'дрочила ебаное',
    'ты позорный рукожоп', 'ты ёбаный выродок', 'ты просто крыса', 'ты жалкий мразь',
    'ты настоящий холуй', 'ты позорный душнила', 'ну ты и пиздабол', 'ты реальный паразит',
    'ты тупой паразит', 'ну ты и урод', 'ты сраный жиробас', 'засранец грёбаное',
    'слышь ты нубас', 'холуй грёбаное', 'вот ты токсик', 'ты настоящий коротышка',
    'ты позорный мудак', 'вот ты падаль', 'ты конченый биомусор', 'слышь ты шестёрка',
    'слышь ты урла', 'трус ебаное', 'эй ты инцел', 'слышь ты тряпка',
    'ты просто скуф', 'дрочила грёбаное', 'какой же ты хам', 'ты реальный жиртрест',
    'ты просто задрот', 'ты просто пердун', 'опущенец ебаное', 'ну ты и свинья',
    'ты позорный холуй', 'ну ты и дебил', 'ты сраный токсик', 'шестёрка ебаное',
    'ты жалкий слюнтяй', 'ты лошара по жизни', 'ты сраный ублюдок', 'ты конченый додик',
    'ты дрищ по жизни', 'ты позорный урла', 'ты позорный скотина', 'ты жалкий сопляк',
    'ну ты и скелет', 'инцел грёбаное', 'ты жалкий барыга', 'ты реальный жлоб',
    'опущенец грёбаное', 'ты сраный барыга', 'вот ты сопляк', 'эй ты ссыкло',
    'ты скотина по жизни', 'вот ты хуесос', 'ты настоящий петух', 'ты просто скелет',
    'эй ты пердун', 'ты просто блядина', 'вот ты шалава', 'ты просто мудак',
    'какой же ты чмо', 'ты просто биомусор', 'ты шавка по жизни', 'мудак ебаное',
    'ты сраный пиздабол', 'ты жалкий скелет', 'ты настоящий очкарик', 'ты ёбаный дармоед',
    'ты позорный отморозок', 'ты настоящий гадина', 'эй ты пиздабол', 'ты ёбаный сопляк',
    'ты ёбаный нытик', 'какой же ты коротышка', 'ты сраный жлоб', 'ты настоящий пердун',
    'ты позорный тварь', 'ты просто опущенец', 'какой же ты крыса', 'слышь ты лох',
    'ну ты и опущенец', 'ты позорный ссыкло', 'ты позорный нытик', 'пидор ебаное',
    'ну ты и хуесос', 'эй ты урод', 'ты ёбаный засранец', 'эй ты лошара',
    'карлик грёбаное', 'ну ты и дрочила', 'лошара грёбаное', 'ты просто дармоед',
    'ты настоящий пиздабол', 'ты жалкий дармоед', 'ты реальный пердун', 'ну ты и импотент',
    'слышь ты говнюк', 'ты ёбаный душнила', 'ну ты и чушпан', 'ты настоящий слюнтяй',
    'ты пердун по жизни', 'вот ты урла', 'холуй ебаное', 'вот ты тупой',
    'какой же ты уёбок', 'вот ты быдло', 'ты тупой скелет', 'падаль грёбаное',
    'слышь ты жиртрест', 'слышь ты отморозок', 'ты настоящий душнила', 'ты позорный задрот',
    'ты сраный паразит', 'ты просто жополиз', 'ты конченый шлюха', 'ты просто скотина',
    'ты жалкий карлик', 'урла грёбаное', 'слышь ты додик', 'ты тупой падаль',
    'какой же ты жлоб', 'ты конченый чмо', 'ты просто гопник', 'какой же ты пидор',
    'вот ты тряпка', 'ты позорный тряпка', 'ты просто жиртрест', 'какой же ты кретин',
    'ты сраный крыса', 'какой же ты клоун', 'ты ёбаный идиот', 'ты тупой тряпка',
    'вот ты жлоб', 'ты настоящий жиробас', 'ты коротышка по жизни', 'слизняк грёбаное',
    'эй ты холуй', 'какой же ты опущенец', 'ты сраный туша', 'ты ничтожество по жизни',
    'ты жалкий слабак', 'ты реальный дебил', 'какой же ты жиртрест', 'ты просто слабак',
    'пиздабол ебаное', 'ты просто тварь', 'ты реальный хуесос', 'ты импотент по жизни',
    'ты ёбаный задрот', 'биомусор грёбаное', 'ты реальный урла', 'сучка грёбаное',
    'ты гопник по жизни', 'ты настоящий тряпка', 'эй ты коротышка', 'вот ты кретин',
    'ты позорный клоун', 'ты просто свинья', 'ну ты и идиот', 'слышь ты холуй',
    'ты тупой шкура', 'ты просто хам', 'вот ты пиздабол', 'ты конченый душнила',
    'ты конченый трус', 'ты инцел по жизни', 'ты тупой душнила', 'ты быдло по жизни',
    'какой же ты слизняк', 'ты тупой днище', 'еблан ебаное', 'ты конченый карлик',
    'ты реальный шкура', 'ты настоящий слизняк', 'ты ёбаный жиробас', 'слышь ты задрот',
    'ты дебил по жизни', 'ну ты и скотина', 'ты позорный гадина', 'туша грёбаное',
    'ты тупой идиот', 'ну ты и страшила', 'ты жалкий нытик', 'ты даун по жизни',
    'ты настоящий дармоед', 'ты конченый скотина', 'ты тупой вонючка', 'ты просто уёбок',
    'слышь ты нытик', 'ты позорный инцел', 'ты просто коротышка', 'ты позорный выродок',
    'эй ты блядина', 'эй ты дрочила', 'какой же ты хуесос', 'ты настоящий дрищ',
    'ты ёбаный шавка', 'ты ёбаный сучка', 'ты просто дрищ', 'ты ёбаный отморозок',
    'ты конченый лох', 'ты ёбаный чушпан', 'идиот ебаное', 'какой же ты тупой',
    'ты конченый отморозок', 'уёбок грёбаное', 'ты просто сучка', 'какой же ты долбоёб',
    'ты конченый еблан', 'ты конченый кретин', 'эй ты шестёрка', 'ты настоящий урла',
    'ты жалкий душнила', 'ты просто карлик', 'шалава ебаное', 'вот ты паразит',
    'какой же ты днище', 'ну ты и отморозок', 'вот ты шкура', 'какой же ты паразит',
    'ты ёбаный кретин', 'ты реальный нытик', 'ты сраный скуф', 'уёбок ебаное',
    'слышь ты гадина', 'ты тупой скуф', 'сучка ебаное', 'ты сраный тупой',
    'ты конченый шалава', 'слышь ты урод', 'ты конченый ссыкло', 'какой же ты тряпка',
    'ты ёбаный урла', 'эй ты свинья', 'эй ты еблан', 'ты конченый гадина',
    'слышь ты дрочила', 'жиртрест грёбаное', 'ну ты и блядина', 'вот ты даун',
    'ты тупой очкарик', 'ты конченый даун', 'слышь ты блядина', 'ты просто шестёрка',
    'эй ты сучка', 'ну ты и инцел', 'ты конченый дебил', 'ну ты и шкура',
    'ты ёбаный хуесос', 'эй ты гандон', 'тряпка ебаное', 'ты жалкий свинья',
    'шалава грёбаное', 'ты конченый инцел', 'ты жалкий кретин', 'мразь ебаное',
    'ты позорный даун', 'ты настоящий шавка', 'ты блядина по жизни', 'эй ты даун',
    'ты жиробас по жизни', 'ты настоящий барыга', 'ты реальный лох', 'слышь ты уёбок',
    'ты реальный отморозок', 'вот ты петух', 'ты тупой жиртрест', 'ты настоящий лох',
    'ты позорный петух', 'ты сраный страшила', 'вот ты дрочила', 'ты реальный опущенец',
    'ты сраный скелет', 'вот ты очкарик', 'ты жалкий блядина', 'ты конченый страшила',
    'импотент грёбаное', 'ну ты и дрищ', 'ты жалкий гандон', 'какой же ты задрот',
    'ты тупой свинья', 'чмо ебаное', 'ты ёбаный долбоёб', 'ты позорный свинья',
    'ты настоящий долбоёб', 'свинья грёбаное', 'паразит ебаное', 'ты позорный дармоед',
    'ты реальный скотина', 'ты реальный трус', 'ты жалкий додик', 'ты позорный жиртрест',
    'вот ты лох', 'слышь ты шавка', 'ты просто шалава', 'ты тупой холуй',
    'ты конченый задрот', 'ты конченый мудак', 'выродок грёбаное', 'ты конченый шавка',
    'ты жалкий туша', 'слышь ты петух', 'ты просто пиздабол', 'ты ёбаный пиздабол',
    'ты просто выродок', 'ты настоящий страшила', 'скуф ебаное', 'ты просто чмо',
    'ты сраный еблан', 'эй ты засранец', 'ты настоящий шестёрка', 'ты реальный шавка',
    'ты позорный ничтожество', 'хуесос грёбаное', 'эй ты рукожоп', 'слышь ты даун',
    'ты ёбаный дрочила', 'эй ты нытик', 'ты конченый падаль', 'ты тупой говнюк',
    'слышь ты пердун', 'ну ты и выродок', 'ты сраный вонючка', 'какой же ты дармоед',
    'эй ты слюнтяй', 'вот ты страшила', 'ты жалкий вонючка', 'ты просто нубас',
    'ты слабак по жизни', 'ты ёбаный токсик', 'слабак ебаное', 'додик ебаное',
    'вот ты чушпан', 'слышь ты мудак', 'ты просто инцел', 'ты тупой гопник',
    'ты ёбаный додик', 'слышь ты чмо', 'токсик ебаное', 'ты жалкий шкура',
    'ты просто жлоб', 'ты скуф по жизни', 'ты жалкий нубас', 'ты жалкий трус',
    'ты ёбаный лошара', 'ты позорный шавка', 'ты конченый рукожоп', 'слышь ты биомусор',
    'дармоед ебаное', 'ты настоящий нубас', 'ты сраный задрот', 'ты жалкий шалава',
    'ты просто мразь', 'ты конченый ублюдок', 'ну ты и задрот', 'эй ты падаль',
    'какой же ты гопник', 'ты гадина по жизни', 'пердун грёбаное', 'слышь ты кретин',
    'ты слюнтяй по жизни', 'ты жалкий чмырь', 'слышь ты коротышка', 'ты позорный трус',
    'какой же ты еблан', 'какой же ты выродок', 'ты тупой кретин', 'ты настоящий гопник',
    'ублюдок ебаное', 'ты конченый туша', 'ну ты и даун', 'ты сраный хам',
    'ты сраный коротышка', 'эй ты тряпка', 'говнюк грёбаное', 'ты трус по жизни',
    'чушпан ебаное', 'ты позорный шалава', 'сопляк ебаное', 'вонючка грёбаное',
    'инцел ебаное', 'какой же ты барыга', 'ты шлюха по жизни', 'какой же ты нубас',
    'ты ёбаный чмо', 'ты ёбаный инцел', 'ты конченый гандон', 'ты тупой тварь',
    'какой же ты шкура', 'жополиз грёбаное', 'ты настоящий хам', 'ты тупой урла',
    'ты позорный сопляк', 'ты ёбаный шестёрка', 'ты тупой чушпан', 'ты конченый терпила',
    'ты тупой нытик', 'какой же ты лошара', 'ты позорный коротышка', 'ты позорный говнюк',
    'ты позорный идиот', 'ты ёбаный лох', 'ты жалкий инцел', 'ты тупой ублюдок',
    'ты позорный карлик', 'ты барыга по жизни', 'ты сраный рукожоп', 'ты конченый скуф',
    'ты сраный урла', 'ты крыса по жизни', 'ты вонючка по жизни', 'вот ты опущенец',
    'ты настоящий мразь', 'вот ты скелет', 'какой же ты шавка', 'вот ты гопник',
    'эй ты выродок', 'какой же ты очкарик', 'эй ты уёбок', 'ты сраный отморозок',
    'ну ты и токсик', 'ты жополиз по жизни', 'ты просто слюнтяй', 'пидор грёбаное',
    'ты тупой дрищ', 'гопник ебаное', 'ты реальный токсик', 'клоун ебаное',
    'ты жалкий очкарик', 'ты позорный дрочила', 'ты сраный очкарик', 'вот ты мразь',
    'ты сраный слизняк', 'слышь ты ублюдок', 'ты настоящий лошара', 'ты жалкий пердун',
    'слышь ты тварь', 'ты тупой хуесос', 'какой же ты пиздабол', 'вот ты засранец',
    'ты жалкий жлоб', 'эй ты слабак', 'эй ты гопник', 'ты позорный ублюдок',
    'слышь ты мразь', 'задрот грёбаное', 'ты настоящий жлоб', 'ты жалкий клоун',
    'чмо грёбаное', 'ты ёбаный уёбок', 'ты просто падаль', 'ты ёбаный паразит',
    'ну ты и засранец', 'ты реальный гопник', 'ты тупой шавка', 'вот ты нубас',
    'ты тупой нубас', 'ты сраный говнюк', 'ты настоящий урод', 'ты тупой тупой',
    'слышь ты долбоёб', 'ну ты и скуф', 'ты реальный слизняк', 'какой же ты ублюдок',
    'ты ёбаный ублюдок', 'ты конченый гопник', 'какой же ты свинья', 'ты позорный пидор',
    'ты ёбаный шкура', 'ну ты и хам', 'ты позорный слюнтяй', 'ты сраный шлюха',
    'ты просто шлюха', 'ты жалкий слизняк', 'ты настоящий чмо', 'ты жалкий крыса',
    'ты сраный ссыкло', 'вот ты жиробас', 'вот ты слабак', 'ты жалкий холуй',
    'какой же ты скуф', 'ты позорный шестёрка', 'ты конченый чушпан', 'ты настоящий шкура',
    'слышь ты душнила', 'ты тупой уёбок', 'ты реальный урод', 'ты позорный слабак',
    'ну ты и сучка', 'ты позорный жополиз', 'какой же ты холуй', 'ты жалкий жиртрест',
    'ну ты и трус', 'нытик ебаное', 'ты ёбаный слизняк', 'ты конченый дармоед',
    'вот ты барыга', 'какой же ты даун', 'ты тупой карлик', 'ты ёбаный свинья',
    'ты просто хуесос', 'вот ты пердун', 'ублюдок грёбаное', 'ты жалкий биомусор',
    'ты позорный жиробас', 'шавка грёбаное', 'ты сраный скотина', 'эй ты урла',
    'ты лох по жизни', 'ты туша по жизни', 'вот ты шлюха', 'ты реальный рукожоп',
    'ты конченый свинья', 'ты тупой пиздабол', 'какой же ты говнюк', 'эй ты трус',
    'эй ты импотент', 'ты жалкий импотент', 'ты позорный страшила', 'ты позорный додик',
    'какой же ты засранец', 'слышь ты еблан', 'коротышка ебаное', 'ты сраный жополиз',
    'какой же ты жополиз', 'ты позорный крыса', 'ты тупой слабак', 'шкура ебаное',
    'вонючка ебаное', 'паразит грёбаное', 'ты ёбаный холуй', 'ты реальный холуй',
    'ты сраный чмо', 'ну ты и додик', 'слышь ты быдло', 'барыга ебаное',
    'ну ты и лошара', 'кретин грёбаное', 'ты позорный опущенец', 'эй ты вонючка',
    'ты позорный нубас', 'ты жалкий жополиз', 'ты страшила по жизни', 'слизняк ебаное',
    'вот ты карлик', 'ну ты и барыга', 'ты конченый урод', 'ты реальный даун',
    'какой же ты додик', 'ты позорный шкура', 'ты отморозок по жизни', 'ну ты и душнила',
    'ты нытик по жизни', 'ты реальный шлюха', 'ты конченый коротышка', 'ты просто слизняк',
    'ты реальный идиот', 'слышь ты дармоед', 'ты ёбаный терпила', 'слабак грёбаное',
    'ты позорный терпила', 'вот ты еблан', 'ты жалкий хам', 'ну ты и ссыкло',
    'ты реальный говнюк', 'ты сраный слабак', 'ну ты и жополиз', 'ты просто отморозок',
    'рукожоп грёбаное', 'ты жалкий тупой', 'ну ты и гандон', 'скелет ебаное',
    'днище грёбаное', 'ты позорный хам', 'ты конченый днище', 'ты жалкий шестёрка',
    'ты ёбаный туша', 'ты жалкий паразит', 'вот ты дрищ', 'ну ты и клоун',
    'какой же ты дебил', 'эй ты хуесос', 'ты конченый слюнтяй', 'ты тупой жополиз',
    'какой же ты чушпан', 'ты сраный пердун', 'ты ёбаный чмырь', 'ты тупой крыса',
    'чмырь грёбаное', 'ты ёбаный пердун', 'ты позорный урод', 'ты конченый говнюк',
    'долбоёб грёбаное', 'тварь грёбаное', 'кретин ебаное', 'ты реальный барыга',
    'ты просто барыга', 'ты реальный ублюдок', 'шлюха грёбаное', 'слюнтяй ебаное',
    'ты реальный быдло', 'ты сраный шавка', 'слышь ты слюнтяй', 'ты ёбаный жлоб',
    'эй ты ублюдок', 'ты тупой дармоед', 'ты жалкий сучка', 'ты сраный засранец',
    'ты реальный блядина', 'слышь ты токсик', 'ну ты и пердун', 'ты настоящий днище',
    'ты сраный биомусор', 'ты сраный дрищ', 'слышь ты пидор', 'ты сраный свинья',
    'ты конченый крыса', 'ты карлик по жизни', 'ссыкло ебаное', 'хам грёбаное',
    'какой же ты шалава', 'вот ты дармоед', 'ты тупой петух', 'ты конченый быдло',
    'ты сраный дармоед', 'ты реальный тряпка', 'ты сраный урод', 'слышь ты жлоб',
    'ты жалкий выродок', 'ну ты и говнюк', 'ты жалкий ничтожество', 'ты конченый петух',
    'ты позорный чмо', 'ты тупой дрочила', 'эй ты кретин', 'ты конченый ничтожество',
    'ты сраный тварь', 'ты тупой токсик', 'ты просто туша', 'ты конченый долбоёб',
    'ты жалкий жиробас', 'эй ты идиот', 'ты сраный уёбок', 'ты настоящий уёбок',
    'ты сраный лошара', 'ты жалкий идиот', 'какой же ты вонючка', 'вот ты идиот',
    'ты реальный падаль', 'ты просто шавка', 'слышь ты засранец', 'хам ебаное',
    'ты душнила по жизни', 'ты тупой даун', 'ты конченый клоун', 'ты конченый слизняк',
    'ты позорный сучка', 'вот ты днище', 'ты настоящий жиртрест', 'слышь ты гандон',
    'ты реальный тварь', 'ты просто долбоёб', 'вот ты гадина', 'ты чушпан по жизни',
    'ты настоящий туша', 'ты конченый чмырь', 'пиздабол грёбаное', 'ты рукожоп по жизни',
    'слышь ты хуесос', 'крыса грёбаное', 'ты ёбаный гандон', 'коротышка грёбаное',
    'эй ты петух', 'дрищ грёбаное', 'вот ты уёбок', 'ты тупой мразь',
    'ты тварь по жизни', 'ну ты и холуй', 'какой же ты слабак', 'эй ты сопляк',
    'эй ты токсик', 'эй ты гадина', 'вот ты импотент', 'ты просто паразит',
    'слышь ты пиздабол', 'вот ты клоун', 'гопник грёбаное', 'ты просто еблан',
    'какой же ты мразь', 'ты ёбаный тряпка', 'ты просто шкура', 'ты жалкий ублюдок',
    'петух ебаное', 'ты конченый жиробас', 'эй ты днище', 'вот ты блядина',
    'ты просто клоун', 'ты реальный свинья', 'эй ты скотина', 'слышь ты терпила',
    'ты позорный днище', 'ты реальный хам', 'слышь ты ничтожество', 'эй ты чмо',
    'ты позорный скуф', 'ты позорный слизняк', 'слышь ты свинья', 'эй ты додик',
    'сопляк грёбаное', 'слышь ты скелет', 'ты ёбаный гопник', 'ну ты и слюнтяй',
    'ты биомусор по жизни', 'ты тупой хам', 'ты реальный дармоед', 'ты реальный дрищ',
    'ты тупой чмо', 'ты тупой слизняк', 'ты конченый блядина', 'вот ты отморозок',
    'вот ты лошара', 'еблан грёбаное', 'слышь ты скуф', 'эй ты крыса',
    'ты конченый нытик', 'ты позорный жлоб', 'ты сраный дрочила', 'ну ты и шестёрка',
    'ты жалкий даун', 'ты тупой чмырь', 'ты тупой сопляк', 'ты жалкий токсик',
    'ты жалкий гадина', 'ты настоящий говнюк', 'ты просто дрочила', 'отморозок ебаное',
    'гандон ебаное', 'ну ты и тварь', 'ты позорный паразит', 'шестёрка грёбаное',
    'какой же ты гандон', 'ты тупой биомусор', 'тупой ебаное', 'тряпка грёбаное',
    'слышь ты страшила', 'ну ты и петух', 'эй ты шалава', 'ты реальный гандон',
    'какой же ты туша', 'ты жалкий быдло', 'ты конченый вонючка', 'какой же ты гадина',
    'ты жалкий падаль', 'какой же ты шестёрка', 'какой же ты идиот', 'ты настоящий паразит',
    'ты реальный клоун', 'ты ёбаный пидор', 'вот ты скуф', 'лох грёбаное',
    'слышь ты вонючка', 'ты хуесос по жизни', 'ты реальный слюнтяй', 'ты сраный гопник',
    'ты просто гадина', 'ты тупой урод', 'ты ёбаный опущенец', 'вот ты хам',
    'ты просто вонючка', 'жиртрест ебаное', 'ты жалкий тварь', 'ты ёбаный скотина',
    'ты позорный импотент', 'ты сраный душнила', 'ты жалкий петух', 'слышь ты крыса',
    'слышь ты шалава', 'ну ты и чмо', 'ты ёбаный клоун', 'шкура грёбаное',
    'ну ты и днище', 'ты ёбаный падаль', 'ты просто токсик', 'отморозок грёбаное',
    'ты просто урла', 'ты реальный шестёрка', 'ты настоящий выродок', 'говнюк ебаное',
    'ты гандон по жизни', 'ты просто лох', 'ты ёбаный биомусор', 'ты урла по жизни',
    'ты настоящий пидор', 'ты позорный дрищ', 'ты жалкий лох', 'урла ебаное',
    'ты урод по жизни', 'ты настоящий слабак', 'ты реальный ничтожество', 'ты жалкий засранец',
    'эй ты дармоед', 'слышь ты идиот', 'ты конченый тряпка', 'ты чмырь по жизни',
    'какой же ты лох', 'ты ёбаный жополиз', 'ты позорный очкарик', 'эй ты задрот',
    'эй ты жополиз', 'ты просто ничтожество', 'ты позорный биомусор', 'овощ ебан',
    'овощ конченый', 'овощ безмозгл',
]

_shield_users: dict = defaultdict(dict)
# Кулдаун на SOS чтоб не спамили: {(cid, uid): last_ts}
_sos_cooldown: dict = {}
SOS_COOLDOWN_SEC = 120


def _antibully_init_db():
    conn = sqlite3.connect("captcha_state.db")
    conn.execute("CREATE TABLE IF NOT EXISTS shields (cid INTEGER NOT NULL, uid INTEGER NOT NULL, expires_at REAL NOT NULL, PRIMARY KEY (cid, uid))")
    conn.commit()
    conn.close()
...
def _antibully_load():
    try:
        ...
        logging.warning(f"antibully load: {e}")
...
async def _notify_admins(cid: int, text: str, exclude_uid: int = 0):
    """Шлёт текст всем админам чата в ЛС. Возвращает сколько получили."""
    notified = 0
    try:
        ...
        for adm in admins:
            if adm.user.is_bot:
                continue
            if adm.user.id == exclude_uid:
                continue
            try:
                await bot.send_message(adm.user.id, text, parse_mode="HTML")
                notified += 1
                await asyncio.sleep(0.2)
            except:
                pass
    except Exception as e:
        logging.warning(f"notify admins: {e}")
    return notified
...
try:
    ...
    logging.warning(f"antibully init: {_e}")


# ─── 🆘 ТРЕВОЖНАЯ КНОПКА ───
@dp.message(F.text.regexp(r"^\s*[\.\/](sos|помогите|спасите|sos911|тревога)(\s|$)"))
async def cmd_sos(message: Message):
    if message.chat.type not in ("group", "supergroup"):
        return
    cid = message.from_user.id and message.chat.id
    uid = message.from_user.id
    ...
    if message.reply_to_message:
        ...
        offender = rt.from_user.full_name if rt.from_user else "неизвестно"
        context = f"\n🌵 возможный обидчик — {rt.from_user.mention_html() if rt.from_user else offender}"
        # Ссылка на сообщение
        if message.chat.username:
            link = f"\nhttps://t.me/{message.chat.username}/{rt.message_id}"

    # Уведомление в чат (мягкое)
    await message.answer(
        f"🆘 <b>{message.from_user.mention_html()} зовёт на помощь</b>\n"
        f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        f"💛 админы уже получили сигнал\n"
        f"🌴 держись, помощь рядом",
        parse_mode="HTML"
    )

    # Алерт всем админам в ЛС
    alert = (
        f"🆘 <b>SOS в чате «{message.chat.title}»</b>\n"
        f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        f"🌺 позвал — {message.from_user.mention_html()} (<code>{uid}</code>)"
        f"{context}"
        f"{link}\n"
        f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        f"<i>🌴 загляни в чат — человеку нужна помощь</i>"
    )
    n = await _notify_admins(cid, alert)
    add_mod_history(cid, uid, "🆘 SOS вызов", "Тревожная кнопка", message.from_user.full_name)


# ─── 🤫 АНОНИМНАЯ ЖАЛОБА ───
# Состояние: ждём текст жалобы от юзера в ЛС {uid: cid}
_anon_complaint_pending: dict = {}

@dp.message(F.text.regexp(r"^\s*[\.\/](анонжалоба|анонимка|жалоба|complaint)(\s|$)"))
async def cmd_anon_complaint_start(message: Message):
    if message.chat.type in ("group", "supergroup"):
        # В группе — просим перейти в ЛС
        cid = message.chat.id
        _anon_complaint_pending[message.from_user.id] = cid
        try:
            await bot.send_message(
                message.from_user.id,
                f"🤫 <b>Анонимная жалоба</b>\n"
                f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
                f"🌺 опиши что происходит в чате «{message.chat.title}»\n"
                f"💛 твоё имя админы НЕ увидят\n"
                f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
                f"🌴 просто напиши следующим сообщением что случилось",
                parse_mode="HTML"
            )
            await reply_auto_delete(message,
                "🤫 написал тебе в ЛС — опиши жалобу там анонимно 💛")
        except:
            await reply_auto_delete(message,
                "🌵 напиши мне сначала в ЛС /start, потом сможешь подать анонимную жалобу")
...
async def handle_anon_complaint_text(message: Message):
    """Ловит текст анонимной жалобы в ЛС."""
    uid = message.from_user.id
    if uid not in _anon_complaint_pending:
        return  # не наш случай — пропускаем дальше (другие хендлеры)
    cid = _anon_complaint_pending.pop(uid)
    complaint = message.text[:1000]

    # Шлём админам БЕЗ имени жалобщика
    alert = (
        f"🤫 <b>Анонимная жалоба</b>\n"
        f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        f"☀️ чат — <b>«{(await bot.get_chat(cid)).title}»</b>\n"
        f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        f"🌴 <b>суть:</b>\n{complaint}\n"
        f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        f"<i>🤫 имя отправителя скрыто</i>"
    )
    n = await _notify_admins(cid, alert)
    await message.answer(
        f"💛 <b>Жалоба отправлена</b>\n"
        f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        f"🌺 получили — <b>{n}</b> админов\n"
        f"🌴 твоё имя они не видят ‧ спасибо что сказал",
        parse_mode="HTML"
    )
        ...
        if chat_cfg.get("welcome_enabled", True):
            ...
            if custom_text and custom_text != "👋 Добро пожаловать, {name}!":
                ...
                templates = [
                    ...
                    # 2. Тропическое
                    (
                        f"🌴 новый турист в нашем оазисе\n"
                        f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
                        f"🥥 <b>{member.full_name}</b> — располагайся под пальмой\n"
                        f"🍹 у нас всегда лето\n"
                        f"💛 /rules — путеводитель"
                    ),
                    # 3. Закатное
                    (
                        f"🌅 пока солнце садится — в чат пришёл кто-то новый\n"
                        f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
                        f"🦩 это <b>{member.full_name}</b>\n"
                        f"✨ оставайся — вечера у нас тёплые\n"
                        f"💛 правила — /rules"
                    ),
                    # 4. Музыкальное
                    (
                        f"🎵 кто-то добавил свою ноту в наш плейлист\n"
                        f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
                        f"🦩 <b>{member.full_name}</b>\n"
                        f"🍹 поймай ритм лета вместе с нами\n"
                        f"💛 партитура — /rules"
                    ),
                    # 5. Утреннее / морское
                    (
                        f"🌞 утро на берегу ‧ кто-то пришёл с нами завтракать\n"
                        f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
                        f"🍦 это <b>{member.full_name}</b>\n"
                        f"🏖 устраивайся — у нас всегда тепло\n"
                        f"💛 правила — /rules"
                    ),
                    # 6. Каникулы / отпуск
                    (
                        f"🕶 кто-то приехал на каникулы\n"
                        f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
                        f"🏝 встречайте — <b>{member.full_name}</b>\n"
                        f"🌺 расслабься, тут все свои\n"
                        f"💛 /rules — пока загораешь"
                    ),
                ]
                import random as _r
...
async def cb_panel(call: CallbackQuery):
    if not await is_admin_by_id(call.message.chat.id, call.from_user.id):
        await call.answer("🍃 Эта команда — для администраторов чата", show_alert=True); return
...
async def cb_mute(call: CallbackQuery):
    ...
    await log_action(f"🌅 <b>Море уснуло</b>\n\n👤 <b>Кто:</b> {call.from_user.mention_html()}\n🎯 <b>Кого:</b> <b>{tname}</b>\n⏱ <b>Время:</b> {label}\n💬 <b>Чат:</b> {call.message.chat.title}\n🕐 <b>Время:</b> {__import__('datetime').datetime.now().strftime('%d.%m.%Y %H:%M')}")
...
async def cb_warn(call: CallbackQuery):
    ...
    if count >= MAX_WARNINGS:
        ...
        await log_action(f"🌴 <b>Предупреждение</b>\n\n👤 <b>Кто:</b> {call.from_user.mention_html()}\n🎯 <b>Кого:</b> <b>{tname}</b>\n📝 <b>Причина:</b> {reason}\n💬 <b>Чат:</b> {call.message.chat.title}\n🕐 <b>Время:</b> {__import__('datetime').datetime.now().strftime('%d.%m.%Y %H:%M')}")
...
async def cb_ban(call: CallbackQuery):
    ...
    await log_action(f"🌻 <b>Бан вынесен</b>\n\n👤 <b>Кто:</b> {call.from_user.mention_html()}\n🎯 <b>Кого:</b> <b>{tname}</b>\n📝 <b>Причина:</b> {reason}\n💬 <b>Чат:</b> {call.message.chat.title}\n🕐 <b>Время:</b> {__import__('datetime').datetime.now().strftime('%d.%m.%Y %H:%M')}")
...
async def cmd_help(message: Message):
    ...
    text_user = (
        ...
        "🥥 <b>Фановые команды</b>\n"
        "├ .dox / /dox — фейк-досье <i>(реплай или себе)</i>\n"
        "├ .gp — приговор от вселенной\n"
        "├ .kill / .ubili — фейк-убийство <i>(реплай)</i>\n"
        "├ .pidor / .gay — % пидарастии\n"
        "├ .ebanat / .dolboeb — % долбоёбства\n"
        "├ .pizdezh — измеритель пиздежа\n"
        "├ .huesos — хуесос дня (рандом из чата)\n"
        "├ .naebka — фейк-новость про юзера\n"
        "├ .zaeb — индекс кринжа и токса\n"
        "├ .huinya — хуёвый совет дня\n"
        "└ .battle — фейк-баттл двух юзеров\n\n"
        ...
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌺 <i>пиши /start чтобы открыть главное меню</i>"
    )
    ...
    text_admin = (
        ...
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "<i>команды для администраторов 🌺</i>\n\n"
        ...
        "🌴 <b>Команды «Аутист» · текстовые</b>\n"
        "<i>любой регистр, реплай на сообщение</i>\n"
        ...
        "├ Аутист фриз — холодный душ на 24ч\n"
        "└ Аутист канал — бан канала <i>(реплай на сообщ. от канала)</i>\n\n"

        "🏝 <b>Защитные команды «Аутист»</b>\n"
        "├ Аутист тишина 10м — штиль в чате\n"
        "├ Аутист тень — невидимый бан 1ч <i>(реплай)</i>\n"
        "├ Аутист амнистия — разбан жертв автоматики за 24ч\n"
        "├ Аутист фильтр СЛОВО — добавить стоп-слово\n"
        "├ Аутист анфильтр СЛОВО — убрать стоп-слово\n"
        "├ Аутист проверка — повторная капча <i>(реплай)</i>\n"
        "├ Аутист волна N — удалить N последних сообщений\n"
        "├ Аутист белый — в whitelist <i>(реплай)</i>\n"
        "└ Аутист небелый — убрать из whitelist <i>(реплай)</i>\n\n"
        ...
        "├ /cascade — текущая ступень флуда <i>(реплай)</i>\n"
        "└ /cascadereset — сбросить лестницу <i>(реплай)</i>\n\n"
    ...
    text_owner = (
        "\n\n"
        "👑 <b>Раздел владельца</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "<i>команды владельца 👑</i>\n\n"

        "💛 <b>Защита от взлома админов</b>\n"
        ...
        "└ /sendreminder — принудительная рассылка про админов\n\n"
    ...
    if is_owner:
        ...
        await reply_auto_delete(message,
            "💌 <b>Справка отправлена</b>\n"
            "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
            "<i>🏝 загляни в личные сообщения</i>",
            parse_mode="HTML")
    elif is_adm:
...
async def cmd_panel(message: Message):
    ...
    if message.reply_to_message:
        ...
        open_reps   = sum(1 for r in report_queue.get(cid, []) if r.get("status") == "new")
        await reply_auto_delete(message,
            f"🛡 <b>Панель · {message.chat.title}</b>\n"
            f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n\n"
            f"👥 Участников: <b>{len(chat_stats[cid])}</b>\n"
            f"💬 Сообщений: <b>{total_msgs}</b>\n"
            f"⚡ Варнов: <b>{total_warns}</b>\n"
            f"🚨 Репортов: <b>{open_reps}</b>\n"
...
async def cmd_unmute(message: Message):
    ...
    await reply_auto_delete(message, f"👤 Цель: {target.mention_html()}\n<i>Шум прибоя вернулся.</i>", parse_mode="HTML")
    await log_action(f"☀️ <b>Голос вернулся</b>\n\n👤 <b>Кто:</b> {message.from_user.mention_html()}\n🎯 <b>Кого:</b> {target.mention_html()}\n💬 <b>Чат:</b> {message.chat.title}\n🕐 <b>Время:</b> {__import__('datetime').datetime.now().strftime('%d.%m.%Y %H:%M')}")

# ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧
#  🎙️ ТЕКСТОВЫЕ КОМАНДЫ «Аутист ...»
#     Реплай на сообщение + "Аутист <действие>" любым регистром.
#     Доступно всем админам чата.
#
#     Поддерживаемые действия:
#       • Аутист бан         → бан
#       • Аутист разбан      → разбан
#       • Аутист мут [время] → мут (по умолч. 1ч)
#       • Аутист размут      → размут
#       • Аутист варн        → варн
#       • Аутист снять варн  → снять варн
#       • Аутист удали       → удалить сообщение
#       • Аутист фриз        → мут на 24ч
# ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧
import re as _autist_re

# Регулярка ловит "Аутист", "АУТИСТ", "аутист" (а также с латинскими буквами)
_AUTIST_PREFIX = r"^\s*[АаAa][уyУY][тtТT][иiИI][сcСsCс][тtТT]\s+"

# Маппинг синонимов команд → каноническое действие
_AUTIST_ACTIONS = {
...
def _autist_match_action(text: str):
    """Возвращает (action, remainder) если текст начинается с «Аутист <действие>», иначе None."""
    m = _autist_re.match(_AUTIST_PREFIX, text, _autist_re.IGNORECASE)
    if not m:
        return None
    ...
    # Ищем самое длинное совпадение (чтобы "снять варн" поймать раньше "варн")
    best = None
    best_len = 0
    for action, words in _AUTIST_ACTIONS.items():
        for w in words:
            if rest_lower.startswith(w):
                # Должен быть пробел или конец строки после слова
                if len(rest_lower) == len(w) or rest_lower[len(w)] in " \t":
                    if len(w) > best_len:
                        best = action
                        best_len = len(w)
    if not best:
        return None
    remainder = rest[best_len:].strip()
    return best, remainder


@dp.message(F.text.regexp(_AUTIST_PREFIX))
async def cmd_autist_router(message: Message):
    """Универсальный роутер для команд «Аутист ...»."""
    if message.chat.type not in ("group", "supergroup"):
        return
    if not message.text:
        return
    parsed = _autist_match_action(message.text)
    if not parsed:
        return
    ...
    if action == "del":
        if not message.reply_to_message:
            await reply_auto_delete(message, "🌴 Реплайни на сообщение которое нужно удалить.")
            return
        try:
            await message.reply_to_message.delete()
        except Exception as e:
            await reply_auto_delete(message, f"🌵 Не удалось удалить: <code>{e}</code>", parse_mode="HTML")
            return
        try:
    ...
    if action == "ban":
        try:
            ...
            await reply_auto_delete(message, f"🌵 Не удалось забанить: <code>{e}</code>", parse_mode="HTML")
            return
        if message.reply_to_message.text:
            await log_violation_screenshot(
                cid, target.id, target.full_name, message.reply_to_message.text,
                "🔨 Аутист бан", reason, message.from_user.full_name, message.chat.title)
        dm_ok = await dm_warn_user(target.id, target.full_name, reason,
                                    message.chat.title, "🔨 Аутист бан", message.from_user.full_name)
        reply = (
            f"🌻 <b>Аутист · Бан</b>\n"
            f"‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧\n"
        ...
        add_mod_history(cid, target.id, "🔨 Аутист бан", reason, message.from_user.full_name)
        await ag.record_action(message.from_user.id, "ban",
            admin_name=message.from_user.full_name,
            target_id=target.id, target_name=target.full_name,
            cid=cid, reason=reason,
    ...
    if action == "unban":
        try:
            ...
            await reply_auto_delete(message, f"🌵 Не удалось разбанить: <code>{e}</code>", parse_mode="HTML")
            return
        await reply_auto_delete(message,
            f"✨ <b>Аутист · Разбан</b>\n‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧\n"
            f"└ 👤 {target.mention_html()} снова с нами", parse_mode="HTML")
        await log_action(
            f"✨ <b>Аутист · Разбан</b>\n\n👤 <b>Кто:</b> {message.from_user.mention_html()}\n"
            f"🎯 <b>Кого:</b> {target.mention_html()}\n💬 <b>Чат:</b> {message.chat.title}")
        add_mod_history(cid, target.id, "🕊 Аутист разбан", "—", message.from_user.full_name)
        await ag.record_action(message.from_user.id, "unban",
            admin_name=message.from_user.full_name,
            target_id=target.id, target_name=target.full_name,
            cid=cid, reason="—",
            extra={"chat_title": message.chat.title, "trigger": "autist"})
        ban_list[cid].pop(target.id, None)
        return

    # ───── МУТ / ФРИЗ ─────
    if action in ("mute", "freeze"):
        if action == "freeze":
            mins, label = 24*60, "24 ч."
        else:
            # parse_duration умеет "30m", "2h", "1d"
            mins, label = parse_duration(remainder or "60m")
            if not mins:
                mins, label = 60, "1 ч."
        try:
            await bot.restrict_chat_member(
            ...
            await reply_auto_delete(message, f"🌵 Не удалось замутить: <code>{e}</code>", parse_mode="HTML")
            return
        title = "АУТИСТ ФРИЗ" if action == "freeze" else "АУТИСТ МУТ"
        icon = "🧃" if action == "freeze" else "🔇"
        if message.reply_to_message.text:
            await log_violation_screenshot(
                cid, target.id, target.full_name, message.reply_to_message.text,
                f"{icon} {title} {label}", reason, message.from_user.full_name, message.chat.title)
        dm_ok = await dm_warn_user(target.id, target.full_name, reason,
            message.chat.title, f"{icon} {title} на {label}", message.from_user.full_name)
        reply = (
            f"{icon}  <b>{title}</b>\n"
            f"‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧\n"
            f"├ 👤 Цель: {target.mention_html()}\n"
        ...
        await log_action(
            ...
            f"📝 <b>Причина:</b> {reason}\n💬 <b>Чат:</b> {message.chat.title}\n"
            f"🕐 {datetime.now().strftime('%d.%m.%Y %H:%M')}")
        add_mod_history(cid, target.id, f"{icon} {title} {label}", reason, message.from_user.full_name)
        await ag.record_action(message.from_user.id, "mute",
            admin_name=message.from_user.full_name,
            target_id=target.id, target_name=target.full_name,
            cid=cid, reason=reason,
            extra={"minutes": mins, "chat_title": message.chat.title,
                   "trigger": "autist_freeze" if action == "freeze" else "autist_mute"})
    ...
    if action == "unmute":
        try:
            ...
            await reply_auto_delete(message, f"🌵 Не удалось размутить: <code>{e}</code>", parse_mode="HTML")
            return
        await reply_auto_delete(message,
            f"☀️ <b>Аутист · Размут</b>\n‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧\n"
            f"└ 👤 {target.mention_html()} снова может писать", parse_mode="HTML")
        await log_action(
            f"☀️ <b>Аутист · Размут</b>\n\n👤 <b>Кто:</b> {message.from_user.mention_html()}\n"
            f"🎯 <b>Кого:</b> {target.mention_html()}\n💬 <b>Чат:</b> {message.chat.title}")
        add_mod_history(cid, target.id, "🔊 Аутист размут", "—", message.from_user.full_name)
        await ag.record_action(message.from_user.id, "unmute",
            admin_name=message.from_user.full_name,
            target_id=target.id, target_name=target.full_name,
            cid=cid, reason="—",
            extra={"chat_title": message.chat.title, "trigger": "autist"})
    ...
    # ───── ВАРН ─────
    if action == "warn":
        warnings[cid][target.id] = warnings[cid].get(target.id, 0) + 1
        count = warnings[cid][target.id]

        # Автобан после MAX_WARNINGS
        if count >= MAX_WARNINGS:
            try: await bot.ban_chat_member(cid, target.id)
            except Exception: pass
            await reply_auto_delete(message,
                f"🌻 <b>Автобан</b>\n‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧\n"
                f"├ 👤 {target.mention_html()}\n"
                f"├ ⚡ Варнов: <b>{count}/{MAX_WARNINGS}</b>\n"
                f"└ 📝 {reason}", parse_mode="HTML")
            await log_action(
                f"🌻 <b>Автобан</b>\n\n👤 <b>Кто:</b> {message.from_user.mention_html()}\n"
                f"🎯 <b>Кого:</b> {target.mention_html()}\n⚡ <b>Варнов:</b> {count}/{MAX_WARNINGS}\n"
                f"📝 <b>Причина:</b> {reason}\n💬 <b>Чат:</b> {message.chat.title}")
            warnings[cid][target.id] = 0
            add_mod_history(cid, target.id, "🔨 Автобан (Аутист)", reason, message.from_user.full_name)
            await ag.record_action(message.from_user.id, "ban",
                admin_name=message.from_user.full_name,
                target_id=target.id, target_name=target.full_name,
        ...
        if message.reply_to_message.text:
            await log_violation_screenshot(
                cid, target.id, target.full_name, message.reply_to_message.text,
                f"⚡ Аутист варн {count}/{MAX_WARNINGS}", reason,
                message.from_user.full_name, message.chat.title)
        dm_ok = await dm_warn_user(target.id, target.full_name, reason,
            message.chat.title, f"⚡ Аутист варн {count}/{MAX_WARNINGS}", message.from_user.full_name)
        reply = (
            f"🍃 <b>Аутист · Варн</b>\n"
            f"‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧\n"
            f"├ 👤 Цель: {target.mention_html()}\n"
        ...
        await log_action(
            ...
            f"🎯 <b>Кого:</b> {target.mention_html()}\n📝 <b>Причина:</b> {reason}\n"
            f"💬 <b>Чат:</b> {message.chat.title}\n🕐 {datetime.now().strftime('%d.%m.%Y %H:%M')}")
        add_mod_history(cid, target.id, f"⚡ Аутист варн {count}/{MAX_WARNINGS}", reason, message.from_user.full_name)
        await ag.record_action(message.from_user.id, "warn",
            admin_name=message.from_user.full_name,
            target_id=target.id, target_name=target.full_name,
            cid=cid, reason=reason,
            extra={"count": count, "max": MAX_WARNINGS,
                   "chat_title": message.chat.title, "trigger": "autist"})
        return

    # ───── СНЯТЬ ВАРН ─────
    if action == "unwarn":
        if warnings[cid].get(target.id, 0) <= 0:
            await reply_auto_delete(message, "ℹ️ У этого пользователя нет варнов.")
            return
        warnings[cid][target.id] -= 1
        count = warnings[cid][target.id]
        await reply_auto_delete(message,
            f"💛 <b>Аутист · Варн снят</b>\n‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧\n"
            f"├ 👤 {target.mention_html()}\n"
            f"└ ⚡ Осталось: <b>{count}/{MAX_WARNINGS}</b>", parse_mode="HTML")
        await log_action(
            f"💛 <b>Аутист · Варн снят</b>\n\n👤 <b>Кто:</b> {message.from_user.mention_html()}\n"
    ...
    if action == "shield":
        ...
        try:
            conn = sqlite3.connect("captcha_state.db")
            conn.execute("INSERT OR REPLACE INTO shields (cid, uid, expires_at) VALUES (?, ?, ?)",
                         (cid, target.id, expires))
            conn.commit(); conn.close()
        except: pass
        await message.answer(
    ...
    if action == "unshield":
        if not message.reply_to_message:
            await reply_auto_delete(message, "🌴 Реплайни на участника, с кого снять щит."); return
        _shield_users[cid].pop(target.id, None)
        try:
            conn = sqlite3.connect("captcha_state.db")
            conn.execute("DELETE FROM shields WHERE cid=? AND uid=?", (cid, target.id))
            conn.commit(); conn.close()
        except: pass
        await message.answer(
            f"🌻 <b>Щит снят</b>\n"
            f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
            f"💛 с кого — {target.mention_html()}",
            parse_mode="HTML")
...
async def cmd_clear(message: Message, command: CommandObject):
    ...
    except ValueError: await reply_auto_delete(message, "🌴 Использование: /clear [кол-во]"); return
    deleted = 0
...
async def cmd_lock(message: Message):
    ...
    await reply_auto_delete(message, "<i>Отправка сообщений запрещена.</i>", parse_mode="HTML")
...
async def cmd_unlock(message: Message):
    ...
    await bot.set_chat_permissions(message.chat.id, ChatPermissions(
        can_send_messages=True, can_send_media_messages=True, can_send_polls=True,
        can_send_other_messages=True, can_add_web_page_previews=True, can_invite_users=True))
    await reply_auto_delete(message, "<i>Отправка сообщений разрешена.</i>", parse_mode="HTML")
    await log_action(f"👤 <b>Кто:</b> {message.from_user.mention_html()}\n💬 <b>Чат:</b> {message.chat.title}\n🕐 <b>Время:</b> {__import__('datetime').datetime.now().strftime('%d.%m.%Y %H:%M')}")
...
async def cmd_slowmode(message: Message, command: CommandObject):
    ...
    except ValueError: await reply_auto_delete(message, "🌴 /slowmode 30"); return
    if delay < 0 or delay > 900:
        await reply_auto_delete(message, "🌴 Значение от 0 до 900 секунд."); return
    try:
        await bot.set_chat_slow_mode_delay(message.chat.id, delay)
        if delay == 0:
            await reply_auto_delete(message, "<i>Режим замедления отключён.</i>", parse_mode="HTML")
        else:
            label = f"{delay} сек." if delay < 60 else f"{delay//60} мин. {delay%60} сек." if delay%60 else f"{delay//60} мин."
            await reply_auto_delete(message, f"⏱ Задержка: <b>{label}</b>\n<i>Режим замедления активирован.</i>", parse_mode="HTML")
    except Exception as e:
        await reply_auto_delete(message, f"🌴 Не удалось установить slowmode: <code>{e}</code>", parse_mode="HTML")

@dp.message(Command("promote"))
async def cmd_promote(message: Message, command: CommandObject):
    if not await require_admin(message): return
    if not message.reply_to_message:
        await reply_auto_delete(message, "🌴 Реплайни на сообщение пользователя."); return
    if not command.args:
        await reply_auto_delete(message, "🌴 Пример: /promote Модератор"); return
    title = command.args.strip()
    if len(title) > 32:
        await reply_auto_delete(message, "🌴 Тег максимум 32 символа."); return
    target = message.reply_to_message.from_user
    cid = message.chat.id
    try:
...
async def cb_panel_promote(call: CallbackQuery):
    ...
    try:
        await call.message.edit_text(
            ...
            f"🌴 Бот должен иметь право <b>can_manage_tags</b>",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
...
async def cmd_removetag(message: Message):
    ...
    try:
        ...
        if result.get("ok"):
            ...
            await reply_auto_delete(message,
                f"🌴 {result.get('description', 'Ошибка')}", parse_mode="HTML")
    except Exception as e:
        await reply_auto_delete(message,
            f"🌴 Ошибка: <code>{e}</code>", parse_mode="HTML")
...
async def cmd_poll(message: Message, command: CommandObject):
    ...
    if not command.args or "|" not in command.args:
        await reply_auto_delete(message, "🌴 /poll Вопрос|Вар1|Вар2"); return
    parts = [p.strip() for p in command.args.split("|")]
    if len(parts) < 3: await reply_auto_delete(message, "🌴 Нужно минимум 2 варианта."); return
    try: await message.delete()
    except: pass
...
async def cmd_note(message: Message, command: CommandObject):
    ...
    if action == "set":
        if not await require_admin(message): return
        if len(parts) < 3: await reply_auto_delete(message, "🌴 /note set [имя] [текст]"); return
        notes[cid][parts[1]] = parts[2]; save_data()
        await reply_auto_delete(message, f"📝 Заметка <b>{parts[1]}</b> сохранена!", parse_mode="HTML")
    elif action == "get":
        if len(parts) < 2: await reply_auto_delete(message, "🌴 /note get [имя]"); return
...
async def cmd_birthday(message: Message, command: CommandObject):
    ...
    try:
        ...
        await reply_auto_delete(message, "🌴 Неверный формат. Пример: /birthday 25.03"); return
    uid = message.from_user.id
...
async def cmd_remind(message: Message, command: CommandObject):
    ...
    if not mins: await reply_auto_delete(message, "🌴 Неверный формат времени. Примеры: 10m, 2h, 1d"); return
    text = parts[1].strip()
    cid = message.chat.id
...
async def cmd_tempban(message: Message, command: CommandObject):
    ...
    try:
        ...
        await reply_auto_delete(message, "🌴 Пример: /tempban 3 спам"); return
    if days < 1 or days > 365:
        await reply_auto_delete(message, "🌴 Срок от 1 до 365 дней."); return
    cid = message.chat.id
    # ЛС до бана
...
async def cmd_banlist(message: Message):
    ...
    lines = [f"<b>Всего забанено: {len(bans)}</b>\n"]
...
async def cmd_modtop(message: Message):
    # 🌊 Команда /modtop отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
        "💛 чат стал чище без рейтинга",
        parse_mode="HTML"
    )
...
async def cmd_top(message: Message):
    # 🌊 Команда /top отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_roll(message: Message, command: CommandObject):
    ...
    await reply_auto_delete(message, f"d{sides} → <b>{result}</b>\n", parse_mode="HTML")

async def cmd_flip(message: Message):
    result = random.choice(["🦅 Орёл", "🪙 Решка"])
    await reply_auto_delete(message, f"Результат: <b>{result}</b>\n", parse_mode="HTML")
...
async def cmd_rate(message: Message, command: CommandObject):
    if not command.args: await reply_auto_delete(message, "🌴 /rate [что]"); return
    score = random.randint(0, 10)
    await reply_auto_delete(message, 
        f"📌 {command.args}\n{'🌟'*score+'☆'*(10-score)}\n<b>{score}/10</b>\n", parse_mode="HTML")
...
async def cmd_iq(message: Message):
    ...
    iq   = random.randint(1, 200)
    c    = "🥔 Картошка умнее." if iq<70 else ("🐒 Обезьяна лучше." if iq<100 else ("😐 Сойдёт." if iq<130 else ("🧠 Умный!" if iq<160 else "🤖 Настоящий гений!")))
    await reply_auto_delete(message, f"👤 {user.mention_html()}\n📊 IQ: <b>{iq}</b>\n{c}", parse_mode="HTML")
...
async def cmd_wyr(message: Message):
    await reply_auto_delete(message, f"{random.choice(WOULD_YOU_RATHER)}\n", parse_mode="HTML")
...
async def cmd_choose(message: Message, command: CommandObject):
    if not command.args or "|" not in command.args:
        await reply_auto_delete(message, "🌴 /choose вар1|вар2|вар3"); return
    options = [o.strip() for o in command.args.split("|") if o.strip()]
    if len(options) < 2: await reply_auto_delete(message, "🌴 Минимум 2 варианта."); return
    await reply_auto_delete(message, f"✨ <b>{random.choice(options)}</b>\n", parse_mode="HTML")

@dp.message(Command("horoscope"))
async def cmd_horoscope(message: Message):
    user = message.reply_to_message.from_user if message.reply_to_message else message.from_user
    sign, text = random.choice(list(HOROSCOPES.items()))
    await reply_auto_delete(message, f"{sign} {user.mention_html()}\n{text}", parse_mode="HTML")
    ...
    try:
        if action == "бан":
            ...
            if not tag_text or tag_text == "Нарушение правил":
                await reply_auto_delete(message, "🌴 Укажи название тега. Пример: <code>аутист тег @user Сигма</code>", parse_mode="HTML"); return
            import aiohttp
            async with aiohttp.ClientSession() as session:
                url = f"https://api.telegram.org/bot{BOT_TOKEN}/setChatMemberTag"
            ...
            if result.get("ok"):
                ...
                await reply_auto_delete(message, f"🌴 {result.get('description', 'Ошибка')}", parse_mode="HTML")
        elif action == "удалить":
            try: await message.reply_to_message.delete(); await reply_auto_delete(message, "🗑 Сообщение удалено!")
            except: await reply_auto_delete(message, "🌴 Не удалось удалить сообщение.")
            ...
            except: await reply_auto_delete(message, "🌴 Не удалось закрепить сообщение.")
        elif action == "предупредить":
            text_warn = rest.strip() or "Нарушение правил"
            await reply_auto_delete(message, f"🌴 Внимание {tname}!\n📝 {text_warn}", parse_mode="HTML")
        ...
        # ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧
            ...
            if arg == "вкл":
                ...
                await reply_auto_delete(message, "🌴 Укажи: <b>аутист антиспам вкл</b> или <b>выкл</b>", parse_mode="HTML")
        ...
        # ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧
            ...
            text = rest.strip()
            if not text:
                await reply_auto_delete(message, "🌴 Укажи текст: <b>аутист анонс текст</b>", parse_mode="HTML"); return
            try: await message.delete()
            except: pass
            await bot.send_message(cid,
                f"📢 <b>Объявление</b>\n\n{text}\n",
            ...
            text = rest.strip()
            if not text:
                await reply_auto_delete(message, "🌴 Укажи текст после юзернейма", parse_mode="HTML"); return
            try: await message.delete()
            except: pass
            await bot.send_message(cid,
        ...
        # ── Предыдущий набор (слежка, репа, хаос, сброс, лотерея, смерть, зеркало) ──
            ...
            members = list(chat_stats[cid].keys())
            if not members:
                await reply_auto_delete(message, "🌴 Нет участников!"); return
        ...
        # ── Новые owner команды ──
            ...
            if not members:
                await reply_auto_delete(message, "🌴 Нет активных участников!"); return
        ...
        # ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧
            ...
            await bot.send_message(cid,
                ...
                f"🌴 Просим соблюдать правила чата → /rules\n"
                f"", parse_mode="HTML")
            ...
            if not m4:
                await reply_auto_delete(message, "🌴 Формат: <b>аутист уровень @user +5</b> или <b>-3</b> или <b>10</b>", parse_mode="HTML"); return
            val = int(m4.group(1))
            ...
            if not m5:
                await reply_auto_delete(message, "🌴 Формат: <b>аутист напомни мод 1ч текст</b>", parse_mode="HTML"); return
        ...
        # ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧
            ...
            if not m3:
                await reply_auto_delete(message, "🌴 Формат: <b>аутист напомни 30м текст</b>", parse_mode="HTML"); return
            ...
            if not text_pin and not message.reply_to_message:
                await reply_auto_delete(message, "🌴 Укажи текст или реплайни на сообщение", parse_mode="HTML"); return
            try: await message.delete()
            except: pass
            ...
            if len(parts_poll) < 3:
                await reply_auto_delete(message,
                    "🌴 Формат: <b>аутист голос Вопрос? | Вариант1 | Вариант2</b>", parse_mode="HTML"); return
        ...
        # ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧
            ...
            new_name = rest.strip()
            if not new_name:
                await reply_auto_delete(message, "🌴 Укажи имя: <b>аутист режим Имя</b>", parse_mode="HTML"); return
            try:
                await bot.set_my_name(new_name)
                await reply_auto_delete(message, f"🤖 Имя бота изменено на: <b>{new_name}</b>", parse_mode="HTML")
            except Exception as ex:
                await reply_auto_delete(message, f"🌴 Не удалось: {ex}", parse_mode="HTML")
...
async def cmd_send_ask(message: Message, command: CommandObject):
    ...
    parts = command.args.split(maxsplit=1)
    if len(parts) < 2:
        await reply_auto_delete(message, "🌴 Укажи юзернейм и текст вопроса!"); return
    ...
    try:
        ...
        await reply_auto_delete(message, "🌴 Не удалось отправить вопрос.")
...
async def cmd_confession(message: Message, command: CommandObject):
    ...
    if len(text) < 5:
        await reply_auto_delete(message, "🌴 Слишком короткое сообщение!"); return
    try: await message.delete()
    except: pass
...
async def cmd_secret(message: Message, command: CommandObject):
    ...
    try:
        ...
        await reply_auto_delete(message, "🌴 Не удалось отправить — участник должен написать боту в лс хотя бы раз!")
# ===== ДУЭЛИ =====
duel_requests = {}
...
async def cmd_profile(message: Message):
    # 🌊 Команда /profile отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_addrep(message: Message, command: CommandObject):
    # 🌊 Команда /addrep отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_tournament(message: Message, command: CommandObject):
    ...
    if sub == "start":
        if cid in tournament_data and tournament_data[cid].get("active"):
            await reply_auto_delete(message, "🌴 Турнир уже идёт!"); return
        ...
        if len(parts) < 2:
            await reply_auto_delete(message, "🌴 Нужно минимум 2 участника!"); return
...
WARN_TEMPLATES = {
    ...
    "10": {"label": "🌴 Правила",          "text": "Нарушение правил чата"},
}
...
SHOP_ITEMS = {
    ...
    "33":  {"name": "🍉 Снежинка",         "price": 72,    "type": "title"},
    ...
    "104": {"name": "🐻‍🍉 Полярный",       "price": 760,   "type": "title"},
    ...
    "186": {"name": "🍉 Ледяной трон",     "price": 35000, "type": "title"},
    ...
    "r2":  {"name": "🍉 [ЛЁД]",           "price": 500,    "type": "role", "desc": "Тег в профиле"},
...
async def cb_report_action_v2(call: CallbackQuery):
    ...
    if report.get("status") != "new":
        await call.answer(f"🌴 Уже обработан: {report.get('assigned_mod','?')}", show_alert=True); return
    try:
        result = ""
        if action == "warn":
...
async def cb_report_vote(call: CallbackQuery):
    ...
    if yes_v >= 2:
        await call.message.edit_text("✨ <b>Репорт подтверждён</b>\n2 модератора проголосовали. Решение — за владельцем.")
        await bot.send_message(OWNER_ID, f"🌴 <b>2 голоса за репорт на админа!</b>\nКлюч: {vote_key}", parse_mode="HTML")
...
async def cmd_giverep(message: Message, command: CommandObject):
    if not message.reply_to_message:
        await reply_auto_delete(message,
            ...
            parse_mode="HTML"); return
    if not command.args:
        await reply_auto_delete(message, "🌴 Укажи сумму: <code>/giverep 50</code>", parse_mode="HTML"); return
    try:
...
async def cmd_meme(message: Message, command: CommandObject):
    ...
    try:
        ...
        await reply_auto_delete(message, f"🌴 Ошибка генерации: {e}")
...
async def cmd_broadcast(message: Message, command: CommandObject):
    ...
    text = command.args.strip()
    if len(text) < 3:
        await reply_auto_delete(message, "🌴 Слишком короткий текст!"); return
    broadcast_text = (
...
async def cb_start_menu(call: CallbackQuery):
    ...
    if action == "help":
        ...
        # 🌊 Профиль выпилен (вместе с системой рейтинга)
        await call.message.edit_text(
            f"🌊 <b>Профиль больше не доступен</b>\n"
            f"<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
            f"🌻 уровни / опыт / репутация / магазин выпилены\n"
            f"💛 чат стал чище без рейтинга",
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="◀️ Назад", callback_data="start:back")
            ]])
        ...
        await call.message.edit_text(
            ...
            f"\n"
            f"🌸  <b>Твой статус</b>\n"
            f"🌴  предупреждений — <b>{user_warns2}</b>\n"
            f"\n"
            f"🌺  <b>Возможности</b>\n"
            f"🛡  модерация чата\n"
            f"💌  тикеты поддержки\n"
...
async def cmd_journal(message: Message, command: CommandObject):
    ...
    if command.args:
        text = command.args.strip()
        if len(text) < 3:
            await reply_auto_delete(message, "🌴 Слишком короткая запись!"); return
...
async def cmd_clan(message: Message, command: CommandObject):
    # 🌊 Команда /clan отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_clan_create(message: Message, command: CommandObject):
    # 🌊 Команда /clan_create отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_clan_join(message: Message, command: CommandObject):
    # 🌊 Команда /clan_join отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_clan_leave(message: Message):
    # 🌊 Команда /clan_leave отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def handle_private_message(message: Message):
    ...
    responses = [
        "🛡 <b>CHAT GUARD</b>\nЯ работаю в групповых чатах.\nДобавь меня в группу → /help",
        "👋 Привет! Мои функции доступны в группах.\n/help — список команд",
        "🤖 Я бот для управления чатами.\nДобавь меня в группу и используй /help",
        "⚡ Все функции — в групповых чатах!\nДобавь меня туда.",
...
async def cmd_gift(message: Message, command: CommandObject):
    # 🌊 Команда /gift отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_announce_slash(message: Message, command: CommandObject):
    ...
    if not text:
        await reply_auto_delete(message, "🌴 Укажи текст: /announce текст"); return
    try: await message.delete()
    except: pass
    await bot.send_message(message.chat.id,
        f"📢 <b>Объявление</b>\n\n{text}\n",
...
async def cmd_give_role(message: Message):
    ...
    if message.from_user.id not in ADMIN_IDS:
        await reply_auto_delete(message, "🚫 Выдавать роли может только владелец."); return
    ...
    if role_arg not in MOD_ROLE_PERMISSIONS:
        await reply_auto_delete(message, "🌴 Роль: junior / senior / head"); return
    target = None
    if message.reply_to_message:
        target = message.reply_to_message.from_user
    else:
    ...
    if not target:
        await reply_auto_delete(message, "🌴 Юзер не найден"); return
    cid = message.chat.id
...
async def cmd_take_role(message: Message):
    ...
    if target.id in mod_roles[cid]:
        ...
        await reply_auto_delete(message, "🌴 У юзера нет роли")
...
async def cmd_blacklist(message: Message):
    ...
    if uid in global_blacklist:
        await reply_auto_delete(message, f"🌴 {target.full_name} уже в чёрном списке"); return
...
async def cmd_clone_chat(message: Message):
    ...
    try:
        ...
        await reply_auto_delete(message, "🌴 Укажи числовые ID чатов"); return
...
LANGS = {
    "ru": {
        "warn_issued": "🌴 {name} получает варн! ({count}/{max})",
    ...
    "en": {
        "warn_issued": "🌴 {name} gets a warning! ({count}/{max})",
    ...
    "uk": {
        "warn_issued": "🌴 {name} отримує попередження! ({count}/{max})",
...
async def cmd_set_shift(message: Message):
    ...
    except: await reply_auto_delete(message, "🌴 Часы числами"); return
    cid = message.chat.id
    conn = db_connect()
...
async def cmd_mod_rating(message: Message):
    # 🌊 Команда /modrating отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_done_task(message: Message):
    args = message.text.split()[1:] if message.text else []
    if not args:
        await reply_auto_delete(message, "🌴 Укажи ID задачи: /donetask 5"); return
    try: task_id = int(args[0])
    except: await reply_auto_delete(message, "🌴 ID числом"); return
    uid = message.from_user.id
    conn = db_connect()
...
async def cmd_tasks(message: Message):
    ...
    for r in rows:
        ...
        overdue = "🌴 ПРОСРОЧЕНО" if r["deadline"] < _tsk.time() else ""
...
async def cmd_vip(message: Message):
    # 🌊 Команда /vip отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_restore_db(message: Message):
    ...
    if not doc.file_name.endswith(".db"):
        await reply_auto_delete(message, "🌴 Нужен файл с расширением .db"); return
            ...
            for cid in list(known_chats.keys()):
                ...
                if open_reps >= 5:
                    await safe_send(bot.send_message(OWNER_ID,
                        ...
                        f"🌴 В <b>{known_chats.get(cid,'?')}</b> накопилось <b>{open_reps}</b> необработанных репортов!\n"
...
async def cmd_set_bio(message: Message):
    # 🌊 Команда /setbio отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_set_mood(message: Message):
    # 🌊 Команда /setmood отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_my_profile(message: Message):
    # 🌊 Команда /myprofile отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_propose(message: Message):
    # 🌊 Команда /propose отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_breakup(message: Message):
    # 🌊 Команда /breakup отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_anon_msg(message: Message):
    ...
    if not text:
        await reply_auto_delete(message, "🌴 Напиши текст сообщения"); return
    target = message.reply_to_message.from_user
    uid = message.from_user.id
...
async def cmd_scan_qr(message: Message):
    ...
    if qr["used"]:
        conn.close(); await reply_auto_delete(message, "🌴 Код уже использован"); return
    if qr["uid"] == uid:
...
async def cmd_translate_msg(message: Message):
    ...
    if not text:
        await reply_auto_delete(message, "🌴 Нет текста для перевода"); return
    try:
        async with aiohttp.ClientSession() as session:
...
async def cmd_set_perm(message: Message):
    ...
    if role not in ("junior", "senior", "head", "admin", "owner"):
        await reply_auto_delete(message, "🌴 Роли: junior / senior / head / admin / owner"); return
    cid = message.chat.id
    conn = db_connect()
...
async def cmd_note_mod(message: Message, command: CommandObject):
    ...
    text   = command.args or ""
    if not text:
        await reply_auto_delete(message, "🌴 Напиши текст заметки")
        return
...
async def cmd_notes_mod(message: Message):
    ...
    if not await check_admin(message):
        return
    if not message.reply_to_message:
        await reply_auto_delete(message, "🌴 Реплай на сообщение юзера")
        return
...
async def cmd_delnote_mod(message: Message, command: CommandObject):
    ...
    if not await check_admin(message):
        return
    if not message.reply_to_message:
        await reply_auto_delete(message, "🌴 Реплай на сообщение юзера + номер заметки")
        return
    ...
    try:
        ...
        await reply_auto_delete(message, "🌴 Произошла ошибка.")
    conn.close()
...
async def cmd_setq(message: Message):
    ...
    try:
        ...
        if slot not in (1, 2, 3):
            await reply_auto_delete(message, "🌴 Слот должен быть 1, 2 или 3"); return
    except:
        await reply_auto_delete(message, "🌴 Укажи номер слота: /setq 1 текст"); return
    text = args[1]
...
async def cmd_stats(message: Message):
    # 🌊 Команда /stats отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_daily(message: Message):
    # 🌊 Команда /daily отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
async def cmd_ship(message: Message):
    # 🌊 Команда /ship отключена (система рейтинга выпилена)
    await message.answer(
        "🌊 <b>Система отключена</b>\n"
        "<i>‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧</i>\n"
        "🌻 уровни / опыт / репутация / магазин больше не работают\n"
...
FORTUNES = [
    ...
    # ⚠️ Тревожные
    "🌴 Осторожно — кто-то завидует твоему успеху",
...
async def cmd_calc(message: Message):
    ...
    try:
        ...
        if not safe:
            await reply_auto_delete(message, "🌴 Недопустимые символы"); return
...
async def cmd_banid(message: Message, command: CommandObject):
    ...
    try: uid = int(parts[0])
        ...
        await reply_auto_delete(message, "🌴 Укажи корректный Telegram ID"); return
...
async def cmd_modlog(message: Message, command: CommandObject):
    ...
    lines = [f"<b>Последние {limit} действий:</b>\n"]
    for r in rows:
...
async def cmd_alerts(message: Message, command: CommandObject):
    ...
    limit = 10
    lines = [f"<b>Последние {min(limit, len(alerts_list))}:</b>\n"]
...
async def cmd_appeals_list(message: Message):
    ...
    lines = [f"<b>Ожидают решения: {len(pending)}</b>\n"]
    for d in pending[:10]:
        lines.append(
            f""
...
async def cmd_appeal_approve(message: Message, command: CommandObject):
    if not await require_admin(message): return
    if not command.args:
        await reply_auto_delete(message, "🌴 /appealapprove [user_id]"); return
    try: uid = int(command.args.split()[0])
    except: await reply_auto_delete(message, "🌴 Укажи корректный ID"); return
    conn = db_connect()
    import json as _json
...
async def cmd_appeal_deny(message: Message, command: CommandObject):
    if not await require_admin(message): return
    if not command.args:
        await reply_auto_delete(message, "🌴 /appealdeny [user_id] [причина]"); return
    parts = command.args.split(None, 1)
    try: uid = int(parts[0])
    except: await reply_auto_delete(message, "🌴 Укажи корректный ID"); return
...
def kb_cs_mod(cid: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=f"💬 Флуд порог: {s.get('flood_msgs',15)} за {s.get('flood_window',20)}с", callback_data=f"cs_set:{cid}:flood_msgs")],
...
async def cmd_antivpn(message: Message, command: CommandObject):
    ...
    if args[0] == "on":
        ...
        await reply_auto_delete(message, "🌴 /antivpn on/off/action warn|kick|ban")
...
async def cb_cs_schedule(call: CallbackQuery):
    ...
    cid = int(call.data.split(":")[1])
    await call.message.edit_text(
        f"Время открытия/закрытия чата:",
...
async def handle_cs_input(message: Message):
    ...
    if key in time_keys:
        ...
        if not _re.match(r"^\d{2}:\d{2}$", val):
            await reply_auto_delete(message, "🌴 Формат: HH:MM (например 08:00)"); return
        cs.update_setting(cid, key, val)
    else:
        try:
            cs.update_setting(cid, key, int(val))
        except ValueError:
            await reply_auto_delete(message, "🌴 Введи числовое значение"); return
    await reply_auto_delete(message,
        f""
...
async def cmd_incidents(message: Message):
    ...
    lines = [f"<b>Критических: {len(alerts_list)}</b>\n"]
...
async def cmd_trigger(message: Message, command: CommandObject):
    ...
    if sub == "list":
        ...
        for t in triggers:
            lines.append(f"🔤 <code>{t['word']}</code> → <b>{t['action']}</b>\n📋 {t['reason']}\n👮 {t['added_by']}")
        await reply_auto_delete(message, "\n".join(lines), parse_mode="HTML")
        ...
        await reply_auto_delete(message, "🌴 /trigger add/del/list")
...
async def cmd_autopardon_status(message: Message):
    ...
    lines = ["<b>Скоро будут помилованы:</b>\n"]
...
def _owner_settings_get() -> dict:
    ...
    defaults = {
        ...
        "xp_weekend_double":  "1",
        "xp_cooldown_secs":   "60",
        "level_up_announce":  "0",   # 🌊 выпилено (рейтинг отключён)
        # Модерация
        "max_warns":          "3",
        "warn_expiry_days":   "30",
        "auto_ban_on_warns":  "1",
...
async def cb_owner_settings(call: CallbackQuery):
    ...
    # ── Действия ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧ ‧
        ...
        if param == "reset_xp":
            ...
            await call.message.edit_text(
                "🌴 <b>Сброс репутации ВСЕХ участников</b>\n\nВся репутация будет обнулена!\nУверен?",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
    ...
    # ── Сброс настроек ─
        ...
        if param == "confirm":
            await call.message.edit_text(
                "🌴 <b>Сброс ВСЕХ настроек к дефолту</b>\n\nУверен?",
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
