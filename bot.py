...
def migrate_json_to_sqlite():
    ...
    try:
        ...
        print("[OK] Миграция data.json → SQLite завершена!")
...
def load_data():
    ...
    try:
        ...
        conn.close()
        print("[OK] Данные загружены из SQLite")
    except Exception as e:
...
BALL_ANSWERS = [
    ...
    "[OK] Да, и ещё раз да!", "💯 Абсолютно точно!", "🎯 Попал в точку — да!",
...
async def require_admin(message: Message) -> bool:
    ...
    if not await check_admin(message):
        await reply_auto_delete(message, "[!] access denied :: admin only", parse_mode="HTML")
        return False
    return True
...
def kb_user_panel(tid: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="[!]  Варн",              callback_data=f"panel:warn:{tid}"),
         InlineKeyboardButton(text="[OK]  Снять варн",         callback_data=f"panel:unwarn:{tid}")],
        ...
        if uid and uid in pending and not (event.text and event.text.startswith("/")):
            ...
            try:
                if action == "mute_custom":
                    ...
                    await bot.send_message(chat_id,
                        f"📢 <b>Объявление</b>\n\n{text}\n\n— <b>Администрация</b>", parse_mode="HTML")
                    await event.answer("[OK] Объявление отправлено!")
            except Exception as e:
...
async def send_captcha(message: Message, member):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text=opt, callback_data=f"captcha:{uid}:{opt}:{answer}")
         for opt in options],
        [InlineKeyboardButton(text="[OK] Пропустить (только для админов)", callback_data=f"captcha_skip:{uid}")]
    ])
...
async def cb_captcha_skip(call: CallbackQuery):
    ...
    note = await call.message.answer(
        f"[OK] <b>{name}</b> пропущен администратором {call.from_user.mention_html()}",
        parse_mode="HTML"
    )
    await asyncio.sleep(5)
    try: await note.delete()
    except: pass
    await call.answer("[OK] Участник пропущен")
...
async def cb_captcha(call: CallbackQuery):
    ...
    if chosen == answer:
        ...
        note = await call.message.answer(
            f"[OK] <b>{name}</b> прошёл проверку и вошёл в чат!",
            parse_mode="HTML"
        )
        await asyncio.sleep(5)
        try: await note.delete()
        ...
        note = await call.message.answer(
            f"[ERR] <b>{name}</b> ввёл неверный ответ и был удалён.",
            parse_mode="HTML"
        )
...
async def cb_panel(call: CallbackQuery):
    if not await is_admin_by_id(call.message.chat.id, call.from_user.id):
        await call.answer("[!] access denied :: admin only", show_alert=True); return
    ...
    try:
        if action == "close":
            ...
            await call.message.edit_text(
                ...
                f"🧼 Антимат: <b>{'[OK] вкл' if ANTI_MAT_ENABLED else '[ERR] выкл'}</b>\n"
                f"🤖 Автокик: <b>{'[OK] вкл' if AUTO_KICK_BOTS else '[ERR] выкл'}</b>\n\nВыбери раздел:",
            ...
            await call.message.edit_text(
                f"📊 <b>[~] stats.fetch :: ANALYTICS бота</b>\n\n💬 Сообщений: <b>{total_msgs}</b>\n"
            ...
            if not queue:
                await call.answer("[OK] Жалоб нет!", show_alert=True); return
            ...
            kb_rep = InlineKeyboardMarkup(inline_keyboard=[
                [InlineKeyboardButton(text="[OK] Варн",    callback_data=f"rpt:warn:{queue[0]['target']}:0"),
                ...
                [InlineKeyboardButton(text="🔨 Бан",     callback_data=f"rpt:ban:{queue[0]['target']}:0"),
                 InlineKeyboardButton(text="[ERR] Отклонить", callback_data=f"rpt:reject:{queue[0]['target']}:0")],
                [InlineKeyboardButton(text="🔙 Назад",   callback_data="panel:mainmenu:0")]
            ])
        ...
        # ── Новые разделы панели ────────────────────
            ...
            await call.message.edit_text(
                f"⚙️ <b>[~] config.load :: SETTINGS чата</b>\n"
            ...
            s = welcome_get(cid)
            status = "[OK] включён" if s["enabled"] else "[ERR] выключен"
            await call.message.edit_text(
                f"🔔 <b>Welcome экран</b>\n"
                f"Статус: {status}\n"
                ...
                parse_mode="HTML",
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[
                    [InlineKeyboardButton(text="[OK] Вкл",    callback_data=f"panelset:welcomeon:{cid}"),
                     InlineKeyboardButton(text="[ERR] Выкл",   callback_data=f"panelset:welcomeoff:{cid}")],
...
async def cb_panelset(call: CallbackQuery):
    ...
    if action == "welcome" or action == "welcomeon":
        ...
        if cid in auto_rules_chats: auto_rules_chats.discard(cid); await call.answer("[ERR] Авто-правила выкл")
        else: auto_rules_chats.add(cid); await call.answer("[OK] Авто-правила вкл")
    elif action == "quarantine":
        if cid in quarantine_chats: quarantine_chats.discard(cid); await call.answer("[ERR] Карантин выкл")
        else: quarantine_chats.add(cid); await call.answer("[OK] Карантин вкл")
    elif action == "lockdown":
        try:
            await bot.set_chat_permissions(cid, ChatPermissions(can_send_messages=False))
            await call.answer("🔒 Чат закрыт")
        except Exception as e: await call.answer(f"[ERR] {e}", show_alert=True)
...
async def cb_owner_panel(call: CallbackQuery):
    ...
    if action == "chats":
        ...
        await call.answer(f"[OK] Разлокдаун {unlocked} чатов!", show_alert=True)
        ...
        await call.message.edit_text(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="[ERR] Отмена", callback_data="owner:back:0")
            ]]))
        ...
        await call.answer("[OK] Бэкап отправлен в ЛС!")
        ...
        conn.close()
        if not rows2: await call.answer("[OK] Задач нет", show_alert=True); return
        text = "🎯 Задачи:\n" + "\n".join(
...
async def cb_mute(call: CallbackQuery):
    ...
    await log_action(f"🔇 <b>[~] action.execute :: MUTE</b>\n\n👤 <b>Кто:</b> {call.from_user.mention_html()}\n🎯 <b>Кого:</b> <b>{tname}</b>\n⏱ <b>Время:</b> {label}\n💬 <b>Чат:</b> {call.message.chat.title}\n🕐 <b>Время:</b> {__import__('datetime').datetime.now().strftime('%d.%m.%Y %H:%M')}")
...
async def cb_ban(call: CallbackQuery):
    ...
    await log_action(f"🔨 <b>[~] action.execute :: BAN</b>\n\n👤 <b>Кто:</b> {call.from_user.mention_html()}\n🎯 <b>Кого:</b> <b>{tname}</b>\n📝 <b>Причина:</b> {reason}\n💬 <b>Чат:</b> {call.message.chat.title}\n🕐 <b>Время:</b> {__import__('datetime').datetime.now().strftime('%d.%m.%Y %H:%M')}")
...
async def cb_members(call: CallbackQuery):
    ...
    if action == "adminlist":
        ...
        if not stats: await call.answer("📊 Статистика пуста!", show_alert=True)
            ...
            lines    = ["🏆 <b>[~] top.fetch :: LEADERBOARD активных:</b>\n"]
...
async def cb_chat(call: CallbackQuery):
    ...
    if action == "lock":
        ...
        await call.answer(f"🧼 Антимат {'[OK] включён' if ANTI_MAT_ENABLED else '[ERR] выключен'}!", show_alert=True)
    elif action == "autokick":
        AUTO_KICK_BOTS = not AUTO_KICK_BOTS
        await call.answer(f"🤖 Автокик {'[OK] включён' if AUTO_KICK_BOTS else '[ERR] выключен'}!", show_alert=True)
    elif action == "rules":
...
async def cmd_ban(message: Message, command: CommandObject):
    ...
    if await is_admin_by_id(message.chat.id, target.id):
        await reply_auto_delete(message, "[!] denied :: target is admin"); return
...
async def cmd_autist_router(message: Message):
    ...
    if action == "del":
        ...
        try:
            ...
            await reply_auto_delete(message, f"[ERR] Не удалось удалить: <code>{e}</code>", parse_mode="HTML")
            return
        try:
    ...
    if action in ("ban", "mute", "warn", "freeze") and await is_admin_by_id(cid, target.id):
        await reply_auto_delete(message, "[!] denied :: target is admin")
        return
    ...
    if action == "ban":
        try:
            ...
            await reply_auto_delete(message, f"[ERR] Не удалось забанить: <code>{e}</code>", parse_mode="HTML")
            return
        if message.reply_to_message.text:
            await log_violation_screenshot(
    ...
    if action == "unban":
        try:
            ...
            await reply_auto_delete(message, f"[ERR] Не удалось разбанить: <code>{e}</code>", parse_mode="HTML")
            return
        await reply_auto_delete(message,
            f"🕊 <b>[~] autist.execute :: UNBAN</b>\n────────────────────\n"
    ...
    if action in ("mute", "freeze"):
        ...
        try:
            ...
            await reply_auto_delete(message, f"[ERR] Не удалось замутить: <code>{e}</code>", parse_mode="HTML")
            return
    ...
    if action == "unmute":
        try:
            ...
            await reply_auto_delete(message, f"[ERR] Не удалось размутить: <code>{e}</code>", parse_mode="HTML")
            return
        await reply_auto_delete(message,
            f"🔊 <b>[~] autist.execute :: UNMUTE</b>\n────────────────────\n"
    ...
    if action == "warn":
        ...
        if count >= MAX_WARNINGS:
            ...
            await reply_auto_delete(message,
                f"🔨 <b>[~] auto.execute :: AUTOBAN</b>\n────────────────────\n"
                f"├ 👤 {target.mention_html()}\n"
                f"├ ⚡ Варнов: <b>{count}/{MAX_WARNINGS}</b>\n"
                f"└ 📝 {reason}", parse_mode="HTML")
            await log_action(
                f"🔨 <b>[~] auto.execute :: AUTOBAN_WARN</b>\n\n👤 <b>Кто:</b> {message.from_user.mention_html()}\n"
        ...
        await reply_auto_delete(message, reply, parse_mode="HTML")
        await log_action(
            f"⚡ <b>[~] autist.execute :: WARN {count}/{MAX_WARNINGS}</b>\n\n"
    ...
    if action == "unwarn":
        ...
        count = warnings[cid][target.id]
        await reply_auto_delete(message,
            f"[OK] <b>[~] autist.execute :: UNWARN</b>\n────────────────────\n"
            f"├ 👤 {target.mention_html()}\n"
        ...
        await log_action(
            ...
            f"💬 <b>Чат:</b> {message.chat.title}")
        add_mod_history(cid, target.id, "[OK] Аутист снят варн", "—", message.from_user.full_name)
        return
...
async def cmd_antimat(message: Message, command: CommandObject):
    ...
    if a == "on": ANTI_MAT_ENABLED = True; await reply_auto_delete(message, "[OK] Фильтр нецензурной лексики <b>включён</b>.", parse_mode="HTML")
...
async def cmd_note(message: Message, command: CommandObject):
    ...
    if action == "set":
        ...
        await reply_auto_delete(message, f"📄 <b>{parts[1]}:</b>\n{t}" if t else "[ERR] Заметка не найдена.", parse_mode="HTML")
    elif action == "del":
        if not await require_admin(message): return
        ...
        else: await reply_auto_delete(message, "[ERR] Не найдена.")
    elif action == "list":
...
def kb_warn_templates(target_id: int) -> InlineKeyboardMarkup:
    ...
    for i in range(0, len(items), 2):
        ...
        rows.append(row)
    rows.append([InlineKeyboardButton(text="[ERR] Отмена", callback_data=f"wt:cancel:{target_id}")])
...
async def cb_warn_template(call: CallbackQuery):
    ...
    if not tmpl:
        await call.answer("[ERR] Шаблон не найден!", show_alert=True); return
    cid = call.message.chat.id
    reason = tmpl["text"]
    ...
    await call.answer(f"[OK] {tmpl['label']}")
...
async def handle_verify_code_input(message: Message):
    ...
    try:
        await bot.edit_message_text(
            f"[OK] <b>Верификация пройдена</b>\n<code>────────────────────</code>\n\n"
            f"👤 {message.from_user.mention_html()}\n"
            f"[OK] Код принят — добро пожаловать!",
...
async def cmd_top(message: Message):
    ...
    sorted_u = sorted(stats.items(), key=lambda x: x[1], reverse=True)[:10]
    medals   = ["🥇","🥈","🥉","4️⃣","5️⃣","6️⃣","7️⃣","8️⃣","9️⃣","🔟"]
    lines    = ["🏆 <b>[~] top.fetch :: LEADERBOARD чата</b>\n<code>────────────────────</code>\n"]
...
async def cmd_choose(message: Message, command: CommandObject):
    ...
    await reply_auto_delete(message, f"[OK] <b>{random.choice(options)}</b>\n", parse_mode="HTML")
...
async def autist_commands(message: Message):
    ...
    try:
        ...
        # ────────────────────
            ...
            if arg == "вкл":
                ...
                await bot.set_chat_permissions(cid, ChatPermissions(
                    ...
                    can_send_polls=True, can_send_other_messages=True,
                    can_add_web_page_previews=True))
                await reply_auto_delete(message, "[OK] <b>Антиспам режим выключен!</b>", parse_mode="HTML")
            else:
        ...
        # ────────────────────
            ...
            lines.append("🏆 <b>[~] top.fetch :: LEADERBOARD активных:</b>")
...
async def guess_handler(message: Message):
    ...
    if num == game["number"]:
        ...
        await reply_auto_delete(message, 
            ...
            f"[OK] Число было <b>{num}</b>!", parse_mode="HTML")
...
async def cmd_askoff(message: Message):
    if message.from_user.id in ask_targets:
        ...
        await reply_auto_delete(message, "[ERR] У тебя не открыт АСК.")
...
async def cmd_send_ask(message: Message, command: CommandObject):
    ...
    if not target_id:
        await reply_auto_delete(message, "[ERR] Этот пользователь не открыл АСК или не найден."); return
    data = ask_targets[target_id]
    try:
        await bot.send_message(data["chat_id"],
            f"📬 <b>Анонимный вопрос для {data['name']}:</b>\n\n💬 {text}\n\n<i>Ответь командой /reply</i>",
            parse_mode="HTML")
        await reply_auto_delete(message, "[OK] Вопрос отправлен анонимно!")
    except:
...
async def cmd_mvpstats(message: Message):
    ...
    medals = ["🥇","🥈","🥉","4️⃣","5️⃣"]
    lines = ["🏆 <b>[~] top.fetch :: LEADERBOARD МВП:</b>\n"]
...
async def cmd_secret(message: Message, command: CommandObject):
    if message.chat.type == "private":
        await reply_auto_delete(message, "[ERR] Эту команду используй в чате!"); return
    if not command.args or len(command.args.split(maxsplit=1)) < 2:
        await reply_auto_delete(message, 
    ...
    if not target:
        await reply_auto_delete(message, "[ERR] Участник не найден в чате!"); return
    try:
        try: await message.delete()
        except: pass
...
async def cmd_duel(message: Message, command: CommandObject):
    ...
    await reply_auto_delete(message, 
        ...
        f"[OK] {target.mention_html()}, напиши <b>/accept</b> чтобы принять!\n"
        f"[ERR] Или <b>/decline</b> чтобы отказаться!",
        parse_mode="HTML")
...
async def cmd_accept(message: Message):
    ...
    if cid not in duel_requests:
        await reply_auto_delete(message, "[ERR] Нет активных дуэлей!"); return
    duel = duel_requests[cid]
    if uid != duel["target_id"]:
        await reply_auto_delete(message, "[ERR] Это не твоя дуэль!"); return
...
async def cmd_addrep(message: Message, command: CommandObject):
    ...
    save_data()
    await reply_auto_delete(message, 
        f"[OK] {target.mention_html()} добавлено <b>{amount}</b> репутации!\n"
...
async def cmd_tournament(message: Message, command: CommandObject):
    ...
    if sub == "start":
        ...
        if cid not in tournament_data:
            await reply_auto_delete(message, "[ERR] Сначала открой регистрацию: /tournament start"); return
        ...
        if cid not in tournament_data or not tournament_data[cid].get("active"):
            await reply_auto_delete(message, "[ERR] Нет активного турнира!"); return
        parts = tournament_data[cid]["participants"]
        if len(parts) == 1:
            winner = parts[0]
...
async def cmd_join(message: Message):
    ...
    if cid not in tournament_data or not tournament_data[cid].get("registration"):
        await reply_auto_delete(message, "[ERR] Регистрация на турнир не открыта!"); return
    parts = tournament_data[cid]["participants"]
    if any(p["id"] == uid for p in parts):
        await reply_auto_delete(message, "[OK] Ты уже записан!"); return
    parts.append({"id": uid, "name": message.from_user.full_name})
    await reply_auto_delete(message, 
        f"[OK] {message.from_user.mention_html()} записан в турнир!\n"
...
async def cb_shop(call: CallbackQuery):
    ...
    if action == "noop":
        ...
        if call.from_user.id != uid:
            await call.answer("[ERR] Это не твой магазин!", show_alert=True); return
        rep = reputation[cid].get(uid, 0)
        await call.message.edit_text(
            f"🏪 <b>Магазин титулов</b>\n\n"
            f"💰 Твоя репутация: <b>{rep:+d}</b>\n\n"
        ...
        if not item:
            await call.answer("[ERR] Товар не найден!", show_alert=True); return
...
def kb_report_category(target_id: int, msg_id: int) -> InlineKeyboardMarkup:
    ...
    for i in range(0, len(items), 2):
        ...
        rows.append(row)
    rows.append([InlineKeyboardButton(text="[ERR] Отмена", callback_data="report_cat:cancel:0:0")])
    return InlineKeyboardMarkup(inline_keyboard=rows)

def kb_report_action_v2(cid: int, idx: int, target_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="[OK] Варн",      callback_data=f"rpt2:warn:{cid}:{idx}:{target_id}"),
        ...
        [InlineKeyboardButton(text="🔨 Бан",       callback_data=f"rpt2:ban:{cid}:{idx}:{target_id}"),
         InlineKeyboardButton(text="[ERR] Отклонить", callback_data=f"rpt2:reject:{cid}:{idx}:{target_id}")],
...
async def cb_report_category(call: CallbackQuery):
    ...
    if is_admin_target:
        ...
        vote_kb = InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="[OK] Подтвердить", callback_data=f"rpt_vote:yes:{vote_key}"),
            InlineKeyboardButton(text="[ERR] Отклонить",   callback_data=f"rpt_vote:no:{vote_key}"),
        ]])
        await bot.send_message(OWNER_ID,
            f"[!] <b>Репорт на администратора</b>\n🎯 {target_name}\n{cat_emoji} {cat_label}\n📝 {reason}\nТребует 2 голоса:",
            parse_mode="HTML", reply_markup=vote_kb)
        await call.message.edit_text("[OK] <b>Жалоба отправлена</b>\nОжидает подтверждения 2 модераторов.")
    else:
        await call.message.edit_text(
            f"[OK] <b>Жалоба принята!</b>\n{cat_emoji} {cat_label} | {priority}\n🕐 Модераторы рассмотрят в течение часа.",
            parse_mode="HTML")
        try:
            admins = await bot.get_chat_administrators(cid)
            for adm in admins:
...
async def cb_report_action_v2(call: CallbackQuery):
    ...
    queue = report_queue.get(cid, [])
    if idx >= len(queue):
        await call.answer("[ERR] Уже обработан", show_alert=True); return
    report = queue[idx]
    ...
    if action == "reject":
        ...
        await call.answer("[OK] Доступен снова"); return
    elif action == "note":
        ...
        await call.message.edit_text("📝 Напиши заметку к репорту:",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="[ERR] Отмена", callback_data=f"rpt2:cancel_note:{cid}:{idx}:{target_id}")
            ]]))
        await call.answer(); return
    ...
    try:
        ...
        report_mod_stats[cid][call.from_user.id] += 1
        save_data()
        try: await bot.send_message(report["reporter"], f"[OK] Жалоба на <b>{report['target_name']}</b> принята! {result}", parse_mode="HTML")
        except: pass
        await log_action(f"[OK] <b>Репорт обработан</b>\n🎯 {report['target_name']}\n👮 {mod_name}\n⚙️ {result}")
        await call.message.edit_text(f"[OK] <b>Обработано</b>\n{result}\n👮 {mod_name}", parse_mode="HTML")
        await call.answer("[OK] Выполнено.")
    except Exception as e:
...
async def cb_report_vote(call: CallbackQuery):
    ...
    if yes_v >= 2:
        await call.message.edit_text("[OK] <b>Репорт подтверждён</b>\n2 модератора проголосовали. Решение — за владельцем.")
        ...
        await call.message.edit_text("[ERR] Репорт на администратора отклонён (2 против)")
        del report_mod_votes[vote_key]
    else:
...
async def cmd_report_stats(message: Message):
    ...
    lines = [f"📊 <b>[~] stats.fetch :: ANALYTICS репортов</b>\n",
             f"📋 Всего: <b>{total}</b> | 🆕 Новых: <b>{new_r}</b>",
             f"[OK] Принято: <b>{done_r}</b> | ❌ Отклонено: <b>{rej_r}</b>\n",
...
async def cmd_chatstats(message: Message):
    ...
    lines = [
                "📊 <b>[~] stats.fetch :: ANALYTICS чата</b>",
                "",
...
async def cmd_giverep(message: Message, command: CommandObject):
    ...
    try:
        ...
        await reply_auto_delete(message, "[ERR] Сумма должна быть числом больше 0"); return
    uid = message.from_user.id
    cid = message.chat.id
    target = message.reply_to_message.from_user
    if target.id == uid:
        await reply_auto_delete(message, "[ERR] Нельзя переводить самому себе!"); return
    if target.is_bot:
        await reply_auto_delete(message, "[ERR] Нельзя переводить боту!"); return
    from time import time
    now = time()
    cd_key = f"{uid}_{cid}"
...
async def cmd_broadcast(message: Message, command: CommandObject):
    ...
    await status_msg.edit_text(
        ...
        f"[OK] Доставлено: <b>{sent_ok}</b> чатов\n"
        f"[ERR] Ошибок: <b>{sent_fail}</b>\n"
...
async def cmd_like(message: Message):
    ...
    if target_msg.from_user and target_msg.from_user.id == uid:
        await reply_auto_delete(message, "[ERR] Нельзя лайкать себя!"); return
...
async def cmd_dislike(message: Message):
    ...
    uid = message.from_user.id
    if target_msg.from_user and target_msg.from_user.id == uid:
        await reply_auto_delete(message, "[ERR] Нельзя дизлайкать себя!"); return
    if uid in reactions_data[mid]["👎"]:
        reactions_data[mid]["👎"].discard(uid)
        await reply_auto_delete(message, "[OK] Дизлайк убран")
    else:
        reactions_data[mid]["👎"].add(uid)
        reactions_data[mid]["👍"].discard(uid)
        if target_msg.from_user:
        ...
        if uid_str != inviter_id and uid_str not in referral_used:
            ...
            try:
                await bot.send_message(int(inviter_id),
                    f"[OK] <b>{message.from_user.full_name}</b> перешёл по вашей реферальной ссылке.\n"
...
async def cmd_avatar(message: Message, command: CommandObject):
    ...
    if emoji in AVATAR_EMOJIS:
        avatars[uid] = emoji
        await reply_auto_delete(message, f"[OK] Аватар установлен: {emoji}")
    else:
        await reply_auto_delete(message, f"[ERR] Выбери из списка: {' '.join(AVATAR_EMOJIS)}")
...
async def cb_setavatar(call: CallbackQuery):
    ...
    avatars[uid] = emoji
    await call.message.edit_text(f"[OK] Аватар установлен: {emoji}")
    asyncio.create_task(auto_delete(call.message))
    await call.answer()
...
async def cmd_boost(message: Message, command: CommandObject):
    ...
    if not command.args:
        ...
        for bid, b in BOOSTERS_SHOP.items():
            ...
            status = f"[OK] активен ещё {int((active-now)//60)} мин" if active > now else "⬜ неактивен"
    ...
    if bid not in BOOSTERS_SHOP:
        await reply_auto_delete(message, "[ERR] Такого бустера нет. /boost — список"); return
    b = BOOSTERS_SHOP[bid]
    rep = reputation[cid].get(message.from_user.id, 0)
    if rep < b["price"]:
        await reply_auto_delete(message, f"[ERR] Нужно {b['price']} репы, у тебя {rep}"); return
    from time import time
    now = time()
...
async def cmd_roulette(message: Message, command: CommandObject):
    ...
    try:
        ...
        await reply_auto_delete(message, "[ERR] Ставка должна быть числом больше 0"); return
    rep = reputation[cid].get(uid, 0)
    if rep < bet:
        await reply_auto_delete(message, f"[ERR] Недостаточно репы! У тебя {rep}"); return
    roulette_cd[uid] = now
    # Спин
...
async def cmd_artifact_roll(message: Message):
    ...
    rep = reputation[cid].get(message.from_user.id, 0)
    if rep < 100:
        await reply_auto_delete(message, "[ERR] Нужно 100 репы для ролла артефакта!"); return
...
async def cmd_lottery(message: Message):
    ...
    await reply_auto_delete(message,
        ...
        f"{'[OK] Ты уже участвуешь!' if already_in else 'Купить билет: /lottery_buy (цена: 20 репы)'}",
        parse_mode="HTML")
...
async def cmd_lottery_buy(message: Message):
    ...
    if uid in lottery_tickets[cid]:
        await reply_auto_delete(message, "[OK] Ты уже купил билет на сегодня!"); return
    rep = reputation[cid].get(uid, 0)
    if rep < 20:
        await reply_auto_delete(message, "[ERR] Нужно 20 репы для билета!"); return
...
async def cmd_stock_invest(message: Message, command: CommandObject):
    ...
    try:
        ...
        await reply_auto_delete(message, "[ERR] Введи число больше 0"); return
    rep = reputation[cid].get(uid, 0)
    if rep < amount:
        await reply_auto_delete(message, f"[ERR] Недостаточно репы! У тебя {rep}"); return
    reputation[cid][uid] -= amount
    ...
    save_data()
    await reply_auto_delete(message,
        f"[OK] Вложено <b>{amount}</b> репы на биржу!\n"
...
async def cmd_stock_withdraw(message: Message):
    ...
    invested = stock_invested[cid].get(uid, 0)
    if invested == 0:
        await reply_auto_delete(message, "[ERR] У тебя нет вложений!"); return
    # Вывод с штрафом 10%
...
async def cmd_quote_save(message: Message):
    ...
    if len(text) > 300:
        await reply_auto_delete(message, "[ERR] Цитата слишком длинная (макс 300 символов)"); return
...
async def cmd_clan_create(message: Message, command: CommandObject):
    ...
    if uid in clan_members:
        await reply_auto_delete(message, "[ERR] Ты уже в клане! /clan_leave чтобы выйти"); return
    if not command.args or len(command.args.split()) < 2:
        await reply_auto_delete(message, "[!] Формат: <code>/clan_create ТЕГ Название клана</code>", parse_mode="HTML"); return
    parts = command.args.split(maxsplit=1)
    tag = parts[0].upper()[:5]
    name = parts[1].strip()[:30]
    if any(c["tag"] == tag for c in clans.values()):
        await reply_auto_delete(message, f"[ERR] Тег [{tag}] уже занят!"); return
    rep = reputation[cid].get(uid, 0)
    if rep < 200:
        await reply_auto_delete(message, "[ERR] Нужно 200 репы для создания клана!"); return
...
async def cmd_clan_join(message: Message, command: CommandObject):
    uid = message.from_user.id
    if uid in clan_members:
        await reply_auto_delete(message, "[ERR] Ты уже в клане!"); return
    if not command.args:
        await reply_auto_delete(message, "[!] Формат: <code>/clan_join ТЕГ</code>", parse_mode="HTML"); return
    tag = command.args.strip().upper()
    target_clan = next(((cid, c) for cid, c in clans.items() if c["tag"] == tag), None)
    if not target_clan:
        await reply_auto_delete(message, f"[ERR] Клан [{tag}] не найден!"); return
    clan_id, c = target_clan
    c["members"].append(uid)
    clan_members[uid] = clan_id
    save_data()
    await reply_auto_delete(message, f"[OK] Ты вступил в клан <b>[{tag}] {c['name']}</b>!", parse_mode="HTML")
...
async def cmd_clan_leave(message: Message):
    ...
    if not clan_id or clan_id not in clans:
        await reply_auto_delete(message, "[ERR] Ты не в клане!"); return
    c = clans[clan_id]
    if c["leader"] == uid:
        await reply_auto_delete(message, "[ERR] Лидер не может покинуть клан! Используй /clan_disband"); return
    c["members"].remove(uid)
    del clan_members[uid]
    save_data()
    await reply_auto_delete(message, f"[OK] Ты покинул клан [{c['tag']}]")
...
async def handle_private_message(message: Message):
    ...
    if text.startswith("/ticket"):
        ...
        if not chat_list:
            await message.answer(
                "[ERR] Бот ещё не добавлен ни в один чат.\n"
        ...
        for cid in list(known_chats.keys()):
            try:
                ...
                await bot.send_message(cid,
                    ...
                    f"🏆 <b>[~] top.fetch :: LEADERBOARD вчера:</b>\n"
...
async def cb_silentban(call: CallbackQuery):
    ...
    try:
        ...
        await call.answer("[OK] Тихий бан применён", show_alert=False)
        await call.message.edit_text("🔕 Тихий бан применён.\n<i>Сообщение в чат не отправлено.</i>", parse_mode="HTML")
        asyncio.create_task(auto_delete(call.message))
    except Exception as e:
        await call.answer(f"[ERR] Ошибка: {e}", show_alert=True)
...
async def cmd_usernote(message: Message, command: CommandObject):
    ...
    save_data()
    await reply_auto_delete(message, f"[OK] Заметка добавлена к профилю {target.mention_html()}!", parse_mode="HTML")
...
def kb_report_action(reporter_id: int, target_id: int, idx: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text="[OK] Принять — варн",   callback_data=f"rpt:warn:{target_id}:{idx}"),
        ...
        [InlineKeyboardButton(text="🔨 Принять — бан",    callback_data=f"rpt:ban:{target_id}:{idx}"),
         InlineKeyboardButton(text="[ERR] Отклонить",         callback_data=f"rpt:reject:{target_id}:{idx}")],
    ])
...
async def cb_report_action(call: CallbackQuery):
    ...
    queue = report_queue.get(cid, [])
    if idx >= len(queue):
        await call.answer("[ERR] Жалоба уже обработана", show_alert=True); return
    report = queue[idx]
    if action == "reject":
        queue.pop(idx)
        await call.message.edit_text("[ERR] Жалоба отклонена.")
        asyncio.create_task(auto_delete(call.message))
...
async def cmd_gift(message: Message, command: CommandObject):
    ...
    target = message.reply_to_message.from_user
    if target.id == uid:
        await reply_auto_delete(message, "[ERR] Нельзя дарить самому себе!"); return
    if not command.args:
        await reply_auto_delete(message, "[!] Укажи что дарить. Пример: <code>/gift b1</code>", parse_mode="HTML"); return
    ...
    if arg == "artifact":
        rep = reputation[cid].get(uid, 0)
        if rep < 150:
            await reply_auto_delete(message, "[ERR] Нужно 150 репы!"); return
        ...
        rep = reputation[cid].get(uid, 0)
        if rep < b["price"]:
            await reply_auto_delete(message, f"[ERR] Нужно {b['price']} репы!"); return
        ...
        await reply_auto_delete(message, "[ERR] Неверный подарок. /gift — список", parse_mode="HTML")
...
async def cmd_give_role(message: Message):
    ...
    save_data()
    await reply_auto_delete(message,
        f"[OK] {target.mention_html()} получил роль <b>{MOD_ROLE_LABELS[role_arg]}</b>",
        parse_mode="HTML")
    await log_action(
        f"🔐 <b>Роль выдана</b>\n"
        f"👤 {target.full_name}\n"
...
async def cmd_take_role(message: Message):
    ...
    if target.id in mod_roles[cid]:
        ...
        save_data()
        await reply_auto_delete(message,
            f"[ERR] У {target.mention_html()} забрана роль <b>{old}</b>", parse_mode="HTML")
    else:
...
async def cb_plugin(call: CallbackQuery):
    ...
    status = "[OK] включён" if plugins[cid][key] else "[ERR] выключен"
    # Обновить клавиатуру
    p = plugins[cid]
    rows = []
    for k, label in PLUGIN_LABELS.items():
...
async def cmd_appeal(message: Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="[OK] Разбанить", callback_data=f"appeal:accept:{uid}"),
        InlineKeyboardButton(text="[ERR] Отклонить", callback_data=f"appeal:reject:{uid}"),
    ]])
    await bot.send_message(OWNER_ID,
        f"🔁 <b>Апелляция</b>\n"
    ...
    await message.answer("[OK] Апелляция отправлена! Ожидай решения владельца.")
...
async def cb_appeal(call: CallbackQuery):
    ...
    if action == "accept":
        ...
        save_data()
        try: await bot.send_message(uid, "[OK] Твоя апелляция <b>одобрена</b>! Ты разбанен.", parse_mode="HTML")
        except: pass
        await call.message.edit_text(f"[OK] Апелляция одобрена — ID{uid} разбанен")
    else:
        try: await bot.send_message(uid, "[ERR] Твоя апелляция <b>отклонена</b>.", parse_mode="HTML")
        except: pass
        await call.message.edit_text(f"[ERR] Апелляция отклонена — ID{uid}")
...
async def cb_mypanel(call: CallbackQuery):
    ...
    if action == "close":
        ...
        await call.message.edit_text(
            ...
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="[ERR] Отмена", callback_data=f"mypanel:chat:{cid}")
            ]]))
    elif action == "reports":
        cid = int(val)
        queue = [r for r in report_queue.get(cid, []) if r.get("status") == "new"]
        if not queue:
            await call.answer("[OK] Новых репортов нет!", show_alert=True); return
        r = queue[0]
        await call.message.edit_text(
...
async def cmd_unblacklist(message: Message):
    ...
    save_data()
    await reply_auto_delete(message,
        f"[OK] {target.mention_html()} удалён из чёрного списка", parse_mode="HTML")
...
async def cmd_reset_chat(message: Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        ...
        InlineKeyboardButton(text="[ERR] Отмена",         callback_data=f"resetchat:no:{cid}"),
    ]])
    await reply_auto_delete(message,
...
async def cmd_sos_off(message: Message):
    ...
    await reply_auto_delete(message,
        f"[OK] <b>Все чаты разблокированы</b> ({unlocked})", parse_mode="HTML")
...
async def cmd_quarantine(message: Message):
    ...
    if cid in quarantine_chats:
        ...
        await reply_auto_delete(message, "[OK] Карантин снят — новые участники могут писать", parse_mode="HTML")
    else:
...
async def cmd_cleanup(message: Message):
    ...
    if not inactive_uids:
        await reply_auto_delete(message, "[OK] Неактивных за 30 дней нет!"); return
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text=f"💥 Удалить {len(inactive_uids)} юзеров",
                             callback_data=f"cleanup:yes:{cid}:{len(inactive_uids)}"),
        InlineKeyboardButton(text="[ERR] Отмена", callback_data="cleanup:no:0:0"),
    ]])
    sent = await message.answer(
        f"🧹 <b>Зачистка</b>\n\n"
...
async def cmd_clone_chat(message: Message):
    ...
    save_data()
    await reply_auto_delete(message,
        f"[OK] <b>Настройки скопированы</b>\n"
...
LANGS = {
    "ru": {
        ...
        "lang_changed":"[OK] Язык изменён на Русский 🇷🇺",
    },
    "en": {
        ...
        "lang_changed":"[OK] Language changed to English 🇬🇧",
    },
    "uk": {
        ...
        "lang_changed":"[OK] Мова змінена на Українську 🇺🇦",
    },
}
...
async def cmd_done_task(message: Message):
    ...
    if not row:
        conn.close(); await reply_auto_delete(message, "[ERR] Задача не найдена"); return
    conn.execute("UPDATE mod_tasks SET done=1 WHERE id=?", (task_id,))
    conn.commit(); conn.close()
    await reply_auto_delete(message, f"[OK] Задача #{task_id} выполнена!")
    try:
        await bot.send_message(OWNER_ID,
            f"[OK] <b>Задача выполнена!</b>\n"
...
async def cmd_tasks(message: Message):
    ...
    conn.close()
    if not rows:
        await reply_auto_delete(message, "[OK] Активных задач нет!"); return
    import time as _tsk
...
async def cmd_add_reply(message: Message):
    ...
    conn.commit(); conn.close()
    await reply_auto_delete(message, f"[OK] Быстрый ответ <b>!{key}</b> сохранён", parse_mode="HTML")
...
async def cmd_vip(message: Message):
    ...
    if existing:
        ...
        await reply_auto_delete(message,
            ...
            f"[OK] Иммунитет к анти-спаму и анти-мату", parse_mode="HTML")
        try:
            await bot.send_message(target.id,
                f"💎 <b>Ты получил VIP статус</b> в {message.chat.title}!\n"
                f"[OK] Особые привилегии активированы", parse_mode="HTML")
        except: pass
    ...
    while True:
        try:
            ...
            if _os2.path.exists(db_path):
                ...
                print(f"[OK] Автобэкап отправлен ({size // 1024} KB)")
        except Exception as e:
...
async def cmd_restore_db(message: Message):
    ...
    try:
        ...
        await status.edit_text(f"[ERR] Ошибка восстановления: {e}")
...
async def cmd_backup_now(message: Message):
    ...
    if not _osbn.path.exists(db_path):
        await reply_auto_delete(message, "[ERR] База данных не найдена"); return
    ...
    await bot.send_document(
        ...
        parse_mode="HTML")
    await reply_auto_delete(message, "[OK] Бэкап отправлен в ЛС!", parse_mode="HTML")
...
async def cmd_set_welcome(message: Message):
    ...
    if message.reply_to_message:
        ...
        if message.reply_to_message.photo:
            ...
            if text: s["text"] = text
            welcome_save(cid, s)
            await reply_auto_delete(message, "[OK] Welcome гифка обновлена!", parse_mode="HTML"); return
    ...
    if not text:
        ...
        await reply_auto_delete(message,
            ...
            f"Статус: {status}\n"
            f"Медиа: {'[OK] есть' if s['photo'] else '[ERR] нет'}\n\n"
            f"📝 Текст:\n{s['text']}\n\n"
            f"<b>Команды:</b>\n"
    ...
    welcome_save(cid, s)
    await reply_auto_delete(message,
        f"[OK] <b>Welcome текст обновлён!</b>\n\n{text}",
        parse_mode="HTML")
...
async def cmd_welcome_off(message: Message):
    ...
    await reply_auto_delete(message, "[ERR] Welcome выключен")
...
async def cmd_welcome_on(message: Message):
    ...
    s["enabled"] = True
    welcome_save(message.chat.id, s)
    await reply_auto_delete(message, "[OK] Welcome включён")
...
async def cmd_set_bio(message: Message):
    ...
    if not bio:
        await reply_auto_delete(message,
            ...
            parse_mode="HTML"); return
    if len(bio) > 150:
        await reply_auto_delete(message, "[!] Макс 150 символов"); return
    conn = db_connect()
    ...
    conn.commit(); conn.close()
    await reply_auto_delete(message, f"[OK] Био обновлено!", parse_mode="HTML")
...
async def cb_mood(call: CallbackQuery):
    ...
    conn.commit(); conn.close()
    await call.message.edit_text(f"[OK] Настроение: {mood}")
    await call.answer()
...
async def cmd_my_profile(message: Message):
    ...
    anon = "[OK] вкл" if p and p["anon_enabled"] else "[ERR] выкл"
    rel_text = ""
    if rel:
        try:
...
async def cb_profile_edit(call: CallbackQuery):
    ...
    if field == "mood":
        ...
        conn.execute("INSERT OR REPLACE INTO user_profiles (uid, anon_enabled, anon_nick) VALUES (?,?,?) "
                     ...
                     (uid, new_val, nick))
        conn.commit(); conn.close()
        status = "[OK] включён" if new_val else "[ERR] выключен"
...
async def cmd_add_friend(message: Message):
    ...
    if existing:
        conn.close()
        await reply_auto_delete(message, f"[OK] {target.full_name} уже в твоём списке друзей!"); return
    # Отправить запрос
    conn.execute("INSERT OR REPLACE INTO friend_requests VALUES (?,?,?,?)",
                 (uid, target.id, message.from_user.full_name, int(_taf.time())))
    conn.commit(); conn.close()
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="[OK] Принять", callback_data=f"friend:accept:{uid}"),
        InlineKeyboardButton(text="[ERR] Отклонить", callback_data=f"friend:reject:{uid}"),
    ]])
    try:
        await bot.send_message(target.id,
...
async def cb_friend(call: CallbackQuery):
    ...
    if action == "accept":
        ...
        conn.commit(); conn.close()
        await call.message.edit_text(f"[OK] Теперь вы с {req['from_name']} друзья! 👥")
        try:
            await bot.send_message(from_uid,
                f"[OK] <b>{call.from_user.full_name}</b> принял(а) твой запрос в друзья!",
                parse_mode="HTML")
        except: pass
    else:
        conn.commit(); conn.close()
        await call.message.edit_text(f"[ERR] Запрос от {req['from_name']} отклонён")
    await call.answer()
...
async def cmd_anon_msg(message: Message):
    ...
    try:
        ...
        await reply_auto_delete(message, "[ERR] Не удалось отправить — юзер закрыл ЛС"); return
    await reply_auto_delete(message, f"[OK] Анонимное сообщение отправлено!", parse_mode="HTML")
...
async def cmd_scan_qr(message: Message):
    ...
    if not qr:
        conn.close(); await reply_auto_delete(message, "[ERR] Код не найден"); return
    if qr["used"]:
...
async def cmd_translate_msg(message: Message):
    ...
    try:
        ...
        await reply_auto_delete(message, f"[ERR] Ошибка перевода: {e}")
...
async def cmd_music(message: Message):
    ...
    try:
        ...
        await reply_auto_delete(message, f"[ERR] Ошибка поиска: {e}")
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(url) as resp:
                if resp.status == 200:
                    ...
                    await status.edit_text("[ERR] Не удалось сгенерировать")
    except Exception as e:
        await status.edit_text(f"[ERR] Ошибка: {e}")
...
async def cmd_delnote_mod(message: Message, command: CommandObject):
    ...
    try:
        ...
        if notes and num <= len(notes):
            ...
            conn.commit()
            await reply_auto_delete(message, f"[OK] Заметка #{num} удалена")
        else:
            await reply_auto_delete(message, "[ERR] Заметка не найдена")
    except:
...
async def _vc_execute(parsed: dict, admin_uid: int, admin_name: str) -> str:
    ...
    chats = list(known_chats.keys())
    if not chats:
        return "[ERR] Бот не добавлен ни в один чат"
    ...
    if action == "top":
        ...
        lines = [f"🏆 <b>[~] top.fetch :: LEADERBOARD чата {known_chats.get(cid, cid)}</b>\n"]
    ...
    if action in ("ban", "mute", "unmute", "warn", "unban", "kick"):
        if not target:
            return f"[ERR] Укажи цель: «{action} ID 123456789»"
        ...
        for cid in chats:
            ...
            try:
                ...
                executed.append(f"[ERR] {known_chats.get(cid, cid)}: {e}")

        return "\n".join(executed) if executed else f"[ERR] Пользователь {target} не найден ни в одном чате"
    ...
    if action == "announce":
        if not arg:
            return "[ERR] Укажи текст объявления"
        sent = 0
        for cid in chats:
            try:
                await bot.send_message(cid,
...
async def handle_voice_command(message: Message):
    ...
    try:
        ...
        await processing.edit_text(f"[ERR] Ошибка загрузки файла: {e}")
        return
    ...
    if not transcript:
        await processing.edit_text(
            "[ERR] Не удалось распознать речь.\n"
...
async def cmd_techwork(message: Message):
    ...
    if techwork_active:
        ...
        try:
            ...
            await reply_auto_delete(message, f"[ERR] Ошибка: {e}"); return
        await log_action(f"[OK] <b>ТЕХ.РАБОТЫ ЗАВЕРШЕНЫ</b>\n💬 {message.chat.title}\n👑 {message.from_user.full_name}")
    else:
        # Запускаем
...
async def cmd_violators(message: Message):
    ...
    if message.reply_to_message:
        ...
        if not records:
            await reply_auto_delete(message,
                f"[OK] <b>{target.full_name}</b> — нарушений не найдено!",
                parse_mode="HTML"); return
        lines = [
...
async def cmd_setq(message: Message):
    ...
    if len(args) < 2:
        ...
        for slot in [1, 2, 3]:
            ...
            lines.append(f"▸ /q{slot} — {text[:50] if text != 'не задана' else '[ERR] не задана'}")
    ...
    await reply_auto_delete(message,
        f"[OK] <b>Горячая команда /q{slot} сохранена!</b>\n\n"
        f"📝 Текст: {text[:100]}",
        parse_mode="HTML")
...
async def cmd_deleteme(message: Message):
    uid = message.from_user.id
    kb = InlineKeyboardMarkup(inline_keyboard=[[
        InlineKeyboardButton(text="[OK] Да, удалить", callback_data=f"deleteme:confirm:{uid}"),
        InlineKeyboardButton(text="[ERR] Отмена",      callback_data=f"deleteme:cancel:{uid}"),
    ]])
    await reply_auto_delete(message,
        "🗑 <b>Удаление данных</b>\n\n"
        "Будут удалены:\n"
...
async def cb_deleteme(call: CallbackQuery):
    ...
    if action == "cancel":
        await call.message.edit_text("[OK] Отменено — данные сохранены")
        return
    try:
        conn = db_connect()
        ...
        await call.message.edit_text(f"[ERR] Ошибка: {e}")
    await call.answer()
...
async def cmd_top(message: Message):
    ...
    medals = ["🥇","🥈","🥉"] + ["▸"]*10
    lines = ["🏆 <b>[~] top.fetch :: LEADERBOARD чата по XP</b>\n"]
    for i, r in enumerate(rows):
        try:
...
async def cmd_calc(message: Message):
    ...
    try:
        ...
        await reply_auto_delete(message, "[ERR] Делить на ноль нельзя!")
    except Exception:
        await reply_auto_delete(message, "[ERR] Неверное выражение")
        ...
        async with aiohttp.ClientSession() as sess:
            async with sess.get(qr_url) as resp:
                if resp.status == 200:
                    ...
                    await reply_auto_delete(message, "[ERR] Не удалось создать QR-код")
    except Exception as e:
        await reply_auto_delete(message, f"[ERR] Ошибка: {e}")

@dp.message(Command("ask"))
async def cmd_ask(message: Message):
    question = message.text.replace("/ask", "").strip() if message.text else ""
    answers = [
        "[OK] Определённо да!", "[OK] Скорее всего да", "[OK] Всё указывает на это",
        "🤔 Не уверен...", "🤔 Спроси позже", "🤔 Сложно сказать",
        "[ERR] Очень сомнительно", "[ERR] Скорее нет", "[ERR] Определённо нет!",
...
async def cmd_count(message: Message):
    ...
    lines   = text.count("\n") + 1
    await reply_auto_delete(message,
        f"📊 <b>[~] stats.fetch :: ANALYTICS текста</b>\n\n"
...
async def cmd_banid(message: Message, command: CommandObject):
    ...
    try:
        ...
        await reply_auto_delete(message, f"[ERR] Ошибка: <code>{e}</code>", parse_mode="HTML")
...
async def cmd_alerts(message: Message, command: CommandObject):
    ...
    if not alerts_list:
        await reply_auto_delete(message,
            ""
            "[OK] <i>Алертов нет.</i>", parse_mode="HTML"); return
    limit = 10
...
async def cmd_alerts_clear(message: Message):
    ...
    await reply_auto_delete(message,
        ""
        "[OK] <i>Все алерты очищены.</i>", parse_mode="HTML")
...
async def cmd_appeals_list(message: Message):
    ...
    if not pending:
        await reply_auto_delete(message,
            ""
            "[OK] <i>Активных апелляций нет.</i>", parse_mode="HTML"); return
    ...
    for d in pending[:10]:
        lines.append(
            ...
            f"[OK] /appealapprove {d.get('uid')}  ❌ /appealdeny {d.get('uid')}"
        )
    await reply_auto_delete(message, "\n".join(lines), parse_mode="HTML")
...
async def cmd_appeal_approve(message: Message, command: CommandObject):
    ...
    if not row:
        conn.close()
        await reply_auto_delete(message, "[ERR] Апелляция не найдена."); return
    d = _json.loads(row["data"])
    d["status"] = "approved"
...
async def cmd_appeal_deny(message: Message, command: CommandObject):
    ...
    import json as _json
    row = conn.execute("SELECT data FROM appeals_db WHERE uid=?", (uid,)).fetchone()
    if not row: conn.close(); await reply_auto_delete(message, "[ERR] Апелляция не найдена."); return
    d = _json.loads(row["data"])
...
def _bool_str(val) -> str:
    return "[OK] вкл" if val else "[ERR] выкл"
...
def kb_chatsettings_main(cid: int) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text=f"[OK] Верификация {_bool_str(s.get('verify_enabled'))}", callback_data=f"cs_toggle:{cid}:verify_enabled")],
...
async def cmd_antivpn(message: Message, command: CommandObject):
    ...
    if not command.args:
        status = "[OK] включён" if current["enabled"] else "[ERR] выключен"
        await reply_auto_delete(message,
            f""
            f"Статус: <b>{status}</b>\n"
    ...
    if args[0] == "on":
        ...
        await reply_auto_delete(message,
            ""
            "[ERR] АнтиVPN <b>выключен</b>.", parse_mode="HTML")
...
async def cmd_incidents(message: Message):
    ...
    if not alerts_list:
        await reply_auto_delete(message,
            ""
            "[OK] <i>Критических инцидентов нет.</i>", parse_mode="HTML"); return
...
async def cb_verify_approve(call: CallbackQuery):
    ...
    if cid not in _code_verify or uid not in _code_verify[cid]:
        await call.answer("[OK] Уже верифицирован", show_alert=True)
        try: await call.message.delete()
        except: pass
        return

    data = _code_verify[cid].pop(uid)
    await call.answer(f"[OK] {data['name']} одобрен!")
    try:
        await call.message.edit_text(
            f"[OK] <b>Верификация пройдена</b>\n<code>────────────────────</code>\n\n"
            f"👤 <b>{data['name']}</b>\n"
            f"[OK] Одобрен администратором {call.from_user.full_name}",
            parse_mode="HTML"
        )
...
async def cb_vpn_action(call: CallbackQuery):
    ...
    if action == "vpn_ban":
        ...
        await call.answer("[OK] Проигнорировано")
    try:
...
async def cb_casino(call: CallbackQuery):
    ...
    if uid != target_uid:
        await call.answer("[ERR] Это не твоё казино!", show_alert=True); return
    ...
    if action == "slots":
        bet = int(parts[3])
        if rep < bet:
            await call.answer(f"[ERR] Нужно {bet} 🌟, есть {rep}", show_alert=True); return
    ...
    # ── РУЛЕТКА ────────────────────
        ...
        text = (
            ...
            f"{'[OK] Победа!' if won else '[ERR] Проигрыш.'}\n"
            f""
    ...
    # ── МОНЕТКА ────────────────────
        ...
        text = (
            f""
            f"{icon}!\n\n"
            f"{'[OK] Угадал!' if won else '[ERR] Не угадал.'}\n"
            f""
            f"{'🏆' if won else '💸'}: <b>{sign}{delta} 🌟</b>\n"
            f"💰 Репутация: <b>{new_rep:+d}</b>"
        )
...
async def cb_owner_settings(call: CallbackQuery):
    ...
    # ── Переключатель вкл/выкл ────────────────────
        ...
        await call.answer(f"{'[OK] Включено' if new == '1' else '[ERR] Выключено'}")
    ...
    # ── Редактирование числовых значений 
        ...
        await call.message.edit_text(
            ...
            parse_mode="HTML",
            reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                InlineKeyboardButton(text="[ERR] Отмена", callback_data="os:menu:main")
            ]])
        )
        await call.answer()
        return
        ...
        if param == "reset_xp":
            await call.message.edit_text(
                ...
                reply_markup=InlineKeyboardMarkup(inline_keyboard=[[
                    ...
                    InlineKeyboardButton(text="[ERR] Отмена",        callback_data="os:menu:rating"),
                ]])
            )
    ...
    # ── Подтверждения опасных действий ──
        ...
        if param == "reset_xp":
            ...
            xp_data.clear()
            levels.clear()
            await call.answer("[OK] XP и уровни всех участников сброшены!", show_alert=True)
            s = _owner_settings_get()
            await call.message.edit_text(
                "🏆 <b>Настройки Рейтинга</b>",
                parse_mode="HTML", reply_markup=kb_os_rating(s))
            ...
            conn.close()
            reputation.clear()
            await call.answer("[OK] Репутация всех участников сброшена!", show_alert=True)
            s = _owner_settings_get()
            await call.message.edit_text(
                "🏆 <b>Настройки Рейтинга</b>",
                parse_mode="HTML", reply_markup=kb_os_rating(s))
    ...
    # ── Сброс настроек ─
        ...
        if param == "confirm":
            ...
            conn.commit()
            conn.close()
            await call.answer("[OK] Настройки сброшены к дефолту!", show_alert=True)
            s = _owner_settings_get()
            await call.message.edit_text(
...
async def handle_os_input(message: Message):
    ...
    try:
        ...
        await message.answer(
            f"[ERR] Неверный формат. Попробуй снова.\n<code>{e}</code>",
            parse_mode="HTML",
            reply_markup=kb_owner_settings_main()
        )
